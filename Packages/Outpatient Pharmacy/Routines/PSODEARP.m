...
EN  ; Main Routine Entry Point
 N DIROUT,DTOUT,DUOUT,PSOQ,PSOPAGE,POP,PSHEADER,PSCPRSSA,PSOEDS,PSOSCR,PSOOUT,PSOTYP
 S PSOPAGE=0
 ...
 S DIR(0)="S^A:Active;D:DISUSERed;B:Both",DIR("A")="CPRS System Access" D ^DIR K DIR Q:$D(DUOUT)!$D(DTOUT)!$D(DIROUT)
 S PSCPRSSA=Y,PSHEADER=PSHEADER_$S(Y="A":"Active ",Y="D":"DISUSERed ",Y="B":"Active, DISUSERed, ",1:"")_"and "
 ;
 ...
 S PSOEDS=Y,PSHEADER=PSHEADER_$S(Y="E":"EXPIRED.",Y="N":"NO EXPIRATION DATE.",Y="3":"Expired/Expiring within next 30 days.",Y="9":"Expired/Expiring within next 90 days.",1:"")
...
GUI ; Entry point for ePCS GUI Report
 ...
 ;
 ; CPRS System Access {Active, DISUSERed, or Both}
 S PSHEADER=PSHEADER_$S(EPCSCPRS="A":"Active ",EPCSCPRS="D":"DISUSERed ",EPCSCPRS="B":"Active, DISUSERed, ",1:"")_"and "
 ;
 ; Expiration Date Status {EXPIRED, NO EXP DATE, <30-DAYS, <90-DAYS}
 S PSHEADER=PSHEADER_$S(EPCSSTAT="E":"EXPIRED.",EPCSSTAT="N":"NO EXPIRATION DATE.",EPCSSTAT="3":"Expired/Expiring within next 30 days.",EPCSSTAT="9":"Expired/Expiring within next 90 days.",1:"")
 ;
...
DEVICE  ; Request Device Information
 ...
 Q
 ;
COMPILE(PSCPRSSA,PSOEDS)  ; -- Compile the report lines into the sort global
 ...
 . . Q:'$$TEST(.PSONPOBJ,PSCPRSSA,PSOEDS,NPIEN)
 . . S PSOLINE=""
 ...
 . . . S PSOLINE=PSOLINE_"REMARKS: "_$$LJ^XLFSTR(PSONPOBJ(53.9),"110T")     ; REMARKS FIELD          #200,    #53.9
 ...
 Q
 ;
RUN(PSHEADER,PSCPRSSA,PSOEDS)  ; Run Report
 N PSCOUNT2,PSOTD,PSONAME
 K ^TMP($J,"PSODEARP") ; Clear the temporary accumulator
 D COMPILE(PSCPRSSA,PSOEDS)
 U IO
 D HDR(PSHEADER)
 ...
 ... I $D(^TMP($J,"PSODEARP",PSOTD,PSONAME,PSCOUNT2,2)) D
...
TITLES()  ; -- Create the header TITLES.
 ...
 Q TITLES
 ;
TEST(PSONPOBJ,PSCPRSSA,PSOEDS,NPIEN)  ; -- Perform the requested test for screening critera
 N DEAEXPDT D DT^DILF("",PSONPOBJ(747.44),.DEAEXPDT)
 N RESP S RESP=0
 ;
 ; Provider Active and DEA is expired.
 I ((PSCPRSSA="A")!(PSCPRSSA="B")),PSOEDS="E",(PSONPOBJ(7)'="YES"),PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,-0) Q 1
 ;
 ; Provider Active and does not have a DEA expiration date.
 I ((PSCPRSSA="A")!(PSCPRSSA="B")),PSOEDS="N",(PSONPOBJ(7)'="YES"),PSONPOBJ(747.44)="" Q 1
 ;
 ; Provider Active and DEA is Expired/Expiring within the next 30 days.
 I ((PSCPRSSA="A")!(PSCPRSSA="B")),PSOEDS="3",(PSONPOBJ(7)'="YES"),PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,30) Q 1
 ;
 ; Provider Active and DEA is Expired/Expiring within the next 90 days.
 I ((PSCPRSSA="A")!(PSCPRSSA="B")),PSOEDS="9",(PSONPOBJ(7)'="YES"),PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,90) Q 1
 ;
 ; Provider disusered and DEA is expired.
 I ((PSCPRSSA="D")!(PSCPRSSA="B")),PSOEDS="E",PSONPOBJ(7)="YES",PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,-0) Q 1
 ;
 ; Provider disusered and does not have a DEA expiration date.
 I ((PSCPRSSA="D")!(PSCPRSSA="B")),PSOEDS="N",PSONPOBJ(7)="YES",PSONPOBJ(747.44)="" Q 1
 ;
 ; Provider disusered and DEA is Expired/Expiring within the next 30 days.
 I ((PSCPRSSA="D")!(PSCPRSSA="B")),PSOEDS="3",PSONPOBJ(7)="YES",PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,30) Q 1
 ;
 ; Provider disusered and DEA is Expired/Expiring within the next 90 days.
 I ((PSCPRSSA="D")!(PSCPRSSA="B")),PSOEDS="9",PSONPOBJ(7)="YES",PSONPOBJ(747.44)'="",DEAEXPDT<$$FMADD^XLFDT(DT,90) Q 1
 ;
 Q RESP
 ;
