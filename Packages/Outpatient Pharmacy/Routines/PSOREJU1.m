PSOREJU1 ;BIRM/MFR - BPS (ECME) - Clinical Rejects Utilities (1) ;10/15/04
 ;;7.0;OUTPATIENT PHARMACY;**148,247,260,287,289,358,359,385,403,421,482,562,702**;DEC 1997;Build 14
 ...
 ; Reference to File 200 - NEW PERSON in ICR #10060
 ; Reference to SIG^XUSESIG in ICR #10050
 ; Reference to AMT^BPSBUTL in ICR #4719
 ;
...
ASK ;
 ...
 ;
 ; If TRICARE/CHAMPVA, require confirmation before ignoring.
...
DC(RX,ACTION,DC1) ; Discontinue inside and outside call
 ...
 S DA=RX,RXNUM=""
 ; Set PSOTRIC, if not already set.  PSOTRIC is used by NOOR^PSOCAN4,
 ; which is called by COM^PSOCAN1.
 ...
 D CAN^PSOCAN
 ;
 ; These two lines may not be needed. It seems premature to remove the
 ; RX from the label list when the Rx probably hasn't been added yet.
 ; In addition, PSORXL has code to remove discontinued RXs from the
 ; label list (using the ECME^PSORXL1 call).
...
HDLG(RX,RFL,CODES,FROM,OPTS,DEF) ; - REJECT Handling
 ...
 ;       
 N ACTION,NEWDATA,PSOTRIC,REJ,REJDATA,REJI,REJIEN,RESP,RESPI,RESPREJ,RRR
 S ACTION="",RRR=0
 ...
 ;
 ; Check for TRICARE/CHAMPVA open rejects
 ...
 . . ;
 . . ;For TRICARE/CHAMPVA, override OPTS and DEF if necessary
 . . I PSOTRIC D HDLGTC(.NEWDATA,.OPTS,.DEF,.CODES,DUZ)
 . . ;
 . . ;For Vet, override OPTS, DEF and set up RRR variable if necessary
...
HDLGRRR(RRRDATA,OPTS,DEF,RRR,CODES)  ; Check for VET with open RRR rejects, new tag PSO*421
 ...
 . ;
 . ; Find RRR reject
 . I RRRDATA(REJIEN,"RRR FLAG")="YES" D  Q
 . . S RRR=1_U_RRRDATA(REJIEN,"RRR THRESHOLD AMT")_U_RRRDATA(REJIEN,"RRR GROSS AMT DUE")
 . . S CODES=RRRDATA(REJIEN,"CODE")_","_CODES
 . . Q
 . ;
...
OVRX(TYPE,CODE) ; Returns the extended code/description of the NCPDP DUR override codes
 ...
 I TYPE Q CODE_" - "_$$GET1^DIQ(FILE,+Y,1)
 Q ""
 ;
 ;
