APSPT16 ; GENERATED FROM 'PSO INTERVENTIONS' PRINT TEMPLATE (#1407) ; 09/17/98 ; (FILE 9009032.4, MARGIN=80)
 G BEGIN
CP G CP^DIO2
C S DQ(C)=Y
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1407,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
