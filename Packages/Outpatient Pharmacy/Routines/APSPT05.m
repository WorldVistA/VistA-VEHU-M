APSPT05 ; GENERATED FROM 'PSO INTERVENTION NEW' INPUT TEMPLATE(#1380), FILE 9009032.4;09/17/98
 D DE G BEGIN
DE S DIE="^APSPQA(32.4,",DIC=DIE,DP=9009032.4,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^APSPQA(32.4,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,8) S:%]"" DE(9)=% S %=$P(%Z,U,9) S:%]"" DE(13)=% S %=$P(%Z,U,16) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="APSPT05",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1380,U="^"
...
X14 I X=1 S Y="@3"
 Q
15 D:$D(DG)>9 F^DIE17 G ^APSPT051
