PSOERRX0 ;BIRM/MFR - All Rxs eRx Queue - ListManager ;08/28/22
 ;;7.0;OUTPATIENT PHARMACY;**700,746,770,817**;DEC 1997;Build 7
 ;
...
REP ; Re-Entry Point
 ...
 S PSOFPICK=+$G(Y)
 I PSOFPICK=1 D EPATFLTR^PSOERPC0 S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1) G REP
 I PSOFPICK=2 D DOBFLTR^PSOERPC0 S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1) G REP
 I PSOFPICK=3 D REDTFLTR S CHANGE=1 G REP
 I PSOFPICK=4 D PRVFLTR S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1)
 I PSOFPICK=5 D STSFLTR S CHANGE=1 G REP
 I PSOFPICK=6 D DRGFLTR S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1)
 ...
 . S ERXPTIEN=$$GET1^DIQ(52.49,ERXFLTR,.04,"I") Q:'ERXPTIEN
 . D UL^PSOERX1A(ERXPTIEN)
 . S CHANGE=1 ;I PSOFPICK=10 D VPATFLTR^PSOERPC0 S PSOALLST=1,CHANGE=1 G REP
 I PSOFPICK=10 D VPATFLTR^PSOERPC0 S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1)  G REP
 I PSOFPICK=11 D VPRVFLTR^PSOERRX1 S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1) G REP
 I PSOFPICK=12 D MATFLTR^PSOERPC0 S CHANGE=1 G REP
 I PSOFPICK=13 D VDRGFLTR^PSOERRX1 S CHANGE=1 S PSOALLST=$S('MBMSITE:0,1:1)  G REP
 D REF S VALMBG=1
 Q 
 ;
