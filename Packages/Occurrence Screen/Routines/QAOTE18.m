QAOTE18 ; ;10/05/93
 D DE G BEGIN
DE S DIE="^QA(741,D0,""REVR"",D1,2,",DIC=DIE,DP=741.15,DL=3,DIEL=2,DU="" K DG,DE,DB Q:$O(^QA(741,D0,"REVR",D1,2,DA,""))=""
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="QAOTE18",DQ=1+D G B
1 S DW="0;1",DV="M*P741.7'O",DU="",DLB="ACTION",DIFLD=.01
 S DQ(1,2)="S Y(0)=Y S:+Y(0) Y=$P(^QA(741.7,+Y(0),0),""^"")_""  ""_$P(^(0),""^"",3)"
 S DE(DW)="C1^QAOTE18"
 S DU="QA(741.7,"
 S X=$S(QAOSQUIT:1,1:"")
 S Y=X
...
C1 G C1S:$D(DE(1))[0 K DB S X=DE(1),DIC=DIE
 K ^QA(741,DA(2),"REVR",DA(1),2,"B",$E(X,1,30),DA)
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^QA(741,DA(2),"REVR",DA(1),2,"B",$E(X,1,30),DA)=""
 Q
X1 S DIC("S")="D EN7^QAOSDDUT" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X3 S:(QAOSACTN=1)!(QAOSACTN=23) QAOSQUIT=1
 Q
4 G 1^DIE17
