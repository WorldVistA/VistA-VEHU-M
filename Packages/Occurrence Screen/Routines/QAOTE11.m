...
DE S DIE="^QA(741,D0,""REVR"",",DIC=DIE,DP=741.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^QA(741,D0,"REVR",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(4)=% S %=$P(%Z,U,5) S:%]"" DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X2 S QAOSLEVL=$S($D(^QA(741,DA(1),"REVR",DA,0))#2:+^(0),1:X) S:QAOSLEVL=2 QAOSLEVL("PEER")=1
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S (QAOSREAS,QAOSQUIT,QAOSFIND,QAOSNEWF,QAOSACTN)=""
 Q
4 S DW="0;2",DV="R*P200'",DU="",DLB="REVIEWER NAME",DIFLD=.02
 S DU="VA(200,"
 G RE
X4 S DIC("S")="S QA=$P(^QA(741,DA(1),""REVR"",DA,0),""^""),QA=$P(^QA(741.2,QA,0),""^"",2) I ((QA=1)&($D(^XUSEC(""QAOSCLIN"",+Y))))!(QA>1)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 S Y=$S(QAOSLEVL=2:"@2",QAOSLEVL>2:"@6",1:Y)
 Q
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X6 S:$O(^QA(741.5,"C",+^QA(741,D0,"SCRN"),0))']"" Y="@2"
 Q
7 S D=0 K DE(1) ;2
 S DIFLD=2,DGO="^QAOTE16",DC="1^741.12PA^1^",DV="741.12M*P741.5'",DW="0;1",DOW="REASON FOR EXCEPTION",DLB="Select "_DOW S:D DC=DC_D
 S DU="QA(741.5,"
 G RE:D I $D(DSC(741.12))#2,$P(DSC(741.12),"I $D(^UTILITY(",1)="" X DSC(741.12) S D=$O(^(0)) S:D="" D=-1 G M7
 S D=$S($D(^QA(741,D0,"REVR",DA,1,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M7 I D>0 S DC=DC_D I $D(^QA(741,D0,"REVR",DA,1,+D,0)) S DE(7)=$P(^(0),U,1)
 G RE
R7 D DE
 S D=$S($D(^QA(741,D0,"REVR",DA,1,0)):$P(^(0),U,3,4),1:1) G 7+1
 ;
...
X10 S QAOSFIND=$P(^QA(741,D0,"REVR",D1,0),"^",5)
 Q
11 S DW="0;5",DV="*P741.6'O",DU="",DLB="FINDINGS",DIFLD=4
 S DQ(11,2)="S Y(0)=Y S:+Y(0) Y=$P(^QA(741.6,+Y(0),0),""^"")_""  ""_$P(^(0),""^"",2)"
 S DU="QA(741.6,"
 G RE
X11 S DIC("S")="D EN8^QAOSDDUT" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X13 S:(QAOSFIND]"")&(QAOSFIND'=QAOSFIND("X")) QAOSNEWF=1
 Q
14 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=14 D X14 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X14 S:$S(QAOSFIND("X")="":0,'$D(^QA(741.6,QAOSFIND("X"),0)):0,($P(^(0),U)=1)!($P(^(0),U)=3)!($P(^(0),U)=11):1,1:0) Y="@6",QAOSQUIT=1
 Q
15 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=15 D X15 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X15 S:QAOSLEVL'=1 Y="@6"
 Q
16 D:$D(DG)>9 F^DIE17 G ^QAOTE17
