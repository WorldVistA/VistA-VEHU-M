GMRVONIT ; ; 07-SEP-1995
 ;;3.0;Vitals/Measurements;;Jan 24, 1996
 ;
 K DIF,DIK,DDF,DDT,DTO,D0,DLAYGO,DIC,DIR,DA,ORVROM,DFR,DTN,DIX,DZ
 S DIFQ=0,ORVROM=22 W !!,"This version of 'GMRVONIT' was created on 07-SEP-1995"
 W !?9,"(at GLRISC, by OE/RR V.2.5)",!
 G Q:DIFQ D ^GMRVONI1 G Q:'$D(DIFQ) S DIK(0)="B"
 D ^GMRVONI2,^GMRVONI3
 L  W !,*7,"OK, Protocol Installation is Complete.",!
 K %ZW,%,%H,D0,DA,DIF,DIFQ,DIG,DIH,DIK,DIU,DIV,DSEC,I,J,KEY,DIY,N,NM,NO,ORVROM,R,X,X0
 Q
 ;
Q W *7,!!,"NO UPDATING HAS OCCURRED!" Q
 ;
IXF ;;GMRV
