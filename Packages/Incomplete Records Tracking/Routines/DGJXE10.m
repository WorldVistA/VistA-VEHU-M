...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^VAS(393,DA,-1))<0
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,2) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGJXE10",DQ=1
1 S DW="DT;2",DV="P200'",DU="",DLB="DICTATED BY",DIFLD=10.02
 S DU="VA(200,"
 G RE
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 I X="" S $P(^VAS(393,D0,"DT"),"^",1)="",$P(^("DT"),"^",3)="",$P(^("DT"),"^",4)="",$P(^("DT"),"^",5)="",$P(^("DT"),"^",6)="",$P(^("DT"),"^",7)="",$P(^("DT"),"^",8)="" S DGJTST="INCOMPLETE",Y="@2"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S DGJTST="DICTATED"
 Q
4 S DQ=5 ;@9
5 D:$D(DG)>9 F^DIE17 G ^DGJXE11
