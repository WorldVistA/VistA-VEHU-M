...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^VAS(393,DA,-1))<0
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,3) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGJXE11",DQ=1
1 S DW="DT;3",DV="DX",DU="",DLB="DATE TRANSCRIBED",DIFLD=10.03
 G RE
...
X2 I X="" S $P(^VAS(393,D0,"DT"),"^",4)=""
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 I X]"",X<$P(^VAS(393,D0,"DT"),"^",1) W !!?5,"Date must be after the DATE DICTATED" S $P(^VAS(393,D0,"DT"),"^",3)="" S Y="@9"
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 I X="" S DGJTST="DICTATED",Y="@2"
 Q
5 D:$D(DG)>9 F^DIE17 G ^DGJXE12
