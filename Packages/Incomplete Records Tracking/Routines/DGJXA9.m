...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^VAS(393,DA,-1))<0
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,1) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGJXA9",DQ=1
1 S DW="DT;1",DV="DX",DU="",DLB="DATE DICTATED",DIFLD=10.01
 G RE
...
X2 I X="" S DGJTST="INCOMPLETE",Y="@2"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S DGJTDDT=$S($P(^VAS(393,D0,0),"^",4)]"":$P(^DGPM($P(^VAS(393,D0,0),"^",4),0),"^",1),1:$P(^VAS(393,D0,0),"^",3))
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 S DGJTDDT=$S(DGJTDDT[".":$P(DGJTDDT,".",1),1:DGJTDDT) I X<DGJTDDT S $P(^VAS(393,D0,"DT"),"^",1)="" W !,"Date must be equal to or after EVENT DATE...no time required." S Y="@8"
 Q
5 D:$D(DG)>9 F^DIE17 G ^DGJXA10
