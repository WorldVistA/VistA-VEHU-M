...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^VAS(393,DA,-1))<0
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,5) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGJXA13",DQ=1
1 S DW="DT;5",DV="DX",DU="",DLB="DATE SIGNED",DIFLD=10.05
 G RE
...
X2 I X]"",X<$P(^VAS(393,D0,"DT"),"^",1) W !!?5,"Date must be after the DATE DICTATED" S $P(^VAS(393,D0,"DT"),"^",5)="" S Y="@6"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 I X="" S DGJTST="TRANSCRIBED",Y="@2"
 Q
4 D:$D(DG)>9 F^DIE17 G ^DGJXA14
