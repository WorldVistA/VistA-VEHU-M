...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^VAS(393,DA,""))=""
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,6) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(6)=% S %=$P(%Z,U,8) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X4 I $P(DGJTDEL,"^",3)'=1 S DGJTST="SIGNED NO REVIEW",Y="@2"
 Q
5 S DQ=6 ;@7
...
X7 I X]"",X<$P(^VAS(393,D0,"DT"),"^",1) W !!?5,"Date must be after the DATE DICTATED" S $P(^VAS(393,D0,"DT"),"^",7)="",Y="@7"
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 I X="" S DGJTST="SIGNED",Y="@2"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="DT;8",DV="P200'",DU="",DLB="REVIEWED BY",DIFLD=10.08
 S DE(DW)="C9^DGJXA4"
 S DU="VA(200,"
 G RE
