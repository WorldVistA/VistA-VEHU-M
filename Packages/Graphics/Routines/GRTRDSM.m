%GRTR ;27-Feb-84;Receive routines/globals from GRTS;DSM V3 ; 08 FEB 85  10:52 AM
...
R S GET="F K=1:1 R X:T1 S RSLT=$S('$T:-1,X=ENQ:0,X=ETX:2,X=""(%)ERROR(%)"":3,X=EOT:4,1:1),ERROR=RSLT=3 Q:RSLT>1  W:'RSLT XX,RET I RSLT R CHK1:T1 X CHK S XX=$S(OK:ACK,1:NAK)_""^""_J X BUFLUSH W XX,RET Q:OK"
 ...
 C DEV O DEV:(0:255::255:1) U DEV G @(DIR_MOD)
RR ; Receive Routines
 ...
 X "F I=1:1 X RSYNC Q:K=MAX  S J=0 X GET Q:ERROR  S N=X ZR  X:'DSP RB B  X RTGET B  ZR:ERROR  Q:ERROR  X:'DSP END ZS @N Q:RSLT=4"
 ;
