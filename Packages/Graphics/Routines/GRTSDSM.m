%GRTS ;27-Feb-84;Send Routines or Globals to another CPU with GRTR;DSM V3 ; 25 FEB 85  9:11 AM
...
S S END="U ME W "" **Sent**"",! U DEV",NOSEND="Nothing chosen to send"
 ...
 W ! C DEV O DEV:(0:255::255:1) U DEV G @(DIR_MOD)
SR ; Routine Send
 ...
 X "S N=$O(^UTILITY($J,"""")) Q:N=""""  F I=1:1 S EOR=0,M=$O(^UTILITY($J,N)),DONE=M="""" X TSYNC Q:K=MAX  ZL @N S X=N,J=0 X SEND X:'DSP RB X RTSEND X:'DSP END Q:DONE  S N=M"
 ;
...
SG ;GLOBAL SEND
 ...
 F I=1:1 B  S M=$O(^UTILITY($J,N)),DONE=M="" S:N["(" OS=$E(N,1,$L(N)-1) X TSYNC Q:K=MAX  X:'DSP GB D GLSND Q:Y'=""  X:'DSP END Q:DONE  S N=M
 ;
