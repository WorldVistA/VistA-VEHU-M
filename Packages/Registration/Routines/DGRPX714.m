...
BEGIN S DNM="DGRPX714",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,D=0 K DE(1) ;.3731
 S DIFLD=.3731,DGO="^DGRPX715",DC="2^2.05A^.373^",DV="2.05MFX",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 G RE:D I $D(DSC(2.05))#2,$P(DSC(2.05),"I $D(^UTILITY(",1)="" X DSC(2.05) S D=$O(^(0)) S:D="" D=-1 G M1
 S D=$S($D(^DPT(DA,.373,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M1 I D>0 S DC=DC_D I $D(^DPT(DA,.373,+D,0)) S DE(1)=$P(^(0),U,1)
 G RE
R1 D DE
 S D=$S($D(^DPT(DA,.373,0)):$P(^(0),U,3,4),1:1) G 1+1
 ;
2 S DQ=3 ;@705
...
X4 D:'$G(DGEMHCNVT) YN^DGPPSYCH(DFN) S Y="@706"
 Q
5 S DQ=6 ;@706
6 S DQ=7 ;@99
7 G 0^DIE17
