...
BEGIN S DNM="DGRPTX15",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".53;2",DV="S",DU="",DIFLD=.532,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^DGRPTX15"
...
C1F1 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".53;4",DV="P4'",DU="",DIFLD=.535,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(4,"
 S X=$$DIV^DGRPLE()
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X3 S Y="@53"
 Q
4 S DQ=5 ;@533
5 S DW=".53;3",DV="S",DU="",DIFLD=.533,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C5^DGRPTX15"
...
C5F1 Q
X5 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".53;4",DV="P4'",DU="",DIFLD=.535,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(4,"
 S X=$$DIV^DGRPLE()
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X6 Q
7 S DQ=8 ;@53
8 S DW=".321;2",DV="RSX",DU="",DIFLD=.32102,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C8^DGRPTX15"
...
X8 S DFN=DA D SV^DGLOCK
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".321;3",DV="RSX",DU="",DIFLD=.32103,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C9^DGRPTX15"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X9 S DFN=DA D SV^DGLOCK
 Q
 ;
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW=".322;13",DV="RSX",DU="",DIFLD=.322013,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C10^DGRPTX15"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X10 S DFN=DA D SV^DGLOCK
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".36;2",DV="RSX",DU="",DIFLD=.362,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C11^DGRPTX15"
...
X11 S DFN=DA D SV^DGLOCK
 Q
 ;
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW=".3;1",DV="RSXa",DU="",DIFLD=.301,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^DGRPTX15"
 S DU="Y:YES;N:NO;"
 G RE
...
X13 S:X'="Y" Y="@50"
 Q
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW=".3;2",DV="NJ3,0Xa",DU="",DIFLD=.302,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C14^DGRPTX15"
 G RE
...
X14 S DFN=DA D EV^DGLOCK Q:'$D(X)  K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X),$D(^DPT(DA,.3)),$P(^(.3),U,1)'="Y" W !?4,*7,"Only applies to service-connected applicants." K X
 ...
 ;
15 S DQ=16 ;@50
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW=".362;12",DV="SX",DU="",DIFLD=.36205,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C16^DGRPTX15"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X16 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".362;13",DV="SX",DU="",DIFLD=.36215,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C17^DGRPTX15"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X17 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".362;14",DV="SX",DU="",DIFLD=.36235,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C18^DGRPTX15"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X18 S DFN=DA D MV^DGLOCK
 Q
 ;
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".36;1",DV="*P8'Xa",DU="",DIFLD=.361,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C19^DGRPTX15",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 G RE
...
C19S S X="" G:DG(DQ)=X C19F1 K DB
 D ^DGRPTX18
C19F1 N X,X1,X2 S DIXR=852 D C19X1(U) K X2 M X2=X D C19X1("O") K X1 M X1=X
 D
...
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPTX19",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
...
R20 D DE
 ...
 ;
21 D:$D(DG)>9 F^DIE17 G ^DGRPTX20
