...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.21)) S %Z=^(.21) S %=$P(%Z,U,1) S:%]"" DE(7)=% S %=$P(%Z,U,2) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(13)=% S %=$P(%Z,U,10) S:%]"" DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X3 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 ...
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".13;2",DV="FaXJ17",DU="",DIFLD=.132,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C4^DGRPTX5"
 G RE
...
C4F1 Q
X4 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
