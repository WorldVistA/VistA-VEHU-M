...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.21)) S %Z=^(.21) S %=$P(%Z,U,1) S:%]"" DE(7)=% S %=$P(%Z,U,2) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(13)=% S %=$P(%Z,U,4) S:%]"" DE(15)=% S %=$P(%Z,U,5) S:%]"" DE(17)=% S %=$P(%Z,U,6) S:%]"" DE(18)=% S %=$P(%Z,U,10) S:%]"" DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X16 S:X="" Y=.216
 Q
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".21;5",DV="FX",DU="",DIFLD=.215,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X17 K:$L(X)>30!($L(X)<3) X I $D(X) S DFN=DA D K1^DGLOCK2
 ...
 Q
 ;
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".21;6",DV="FX",DU="",DIFLD=.216,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X18 K:$L(X)>30!($L(X)<3) X I $D(X) S DFN=DA D K1^DGLOCK2
 ...
 Q
 ;
19 D:$D(DG)>9 F^DIE17 G ^DGRPTX10
