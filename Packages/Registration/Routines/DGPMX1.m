DGPMX1 ; GENERATED FROM 'DGPM ADMIT' INPUT TEMPLATE(#1098), FILE 405;11/22/24
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(14)=% S %=$P(%Z,U,5) S:%]"" DE(17)=% S %=$P(%Z,U,6) S:%]"" DE(19)=% S %=$P(%Z,U,7) S:%]"" DE(22)=% S %=$P(%Z,U,10) S:%]"" DE(18)=% S %=$P(%Z,U,11) S:%]"" DE(11)=%
 I  S %=$P(%Z,U,12) S:%]"" DE(4)=%
 I $D(^("DIR")) S %Z=^("DIR") S %=$P(%Z,U,1) S:%]"" DE(3)=%
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,1) S:%]"" DE(24)=%
 I $D(^("PTF")) S %Z=^("PTF") S %=$P(%Z,U,4) S:%]"" DE(5)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(27)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGPMX1",DQ=1
 ...
 M DIEZAR=^DIE(1098,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=1098,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S:$S(DGPMN:1,DGPMY=+^DGPM(DA,0):1,1:0) Y=41
 Q
2 S DW="0;1",DV="RDX",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^DGPMX1",DE(DW,"INDEX")=1
 S X=DGPMY
 S Y=X
...
X2 S %DT="ST" D ^%DT K %DT S X=Y K:Y<1 X I $D(X),'$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="DIR;1",DV="SXR",DU="",DIFLD=41,DLB="DOES THE PATIENT WISH TO BE EXCLUDED FROM THE FACILITY DIRECTORY?"
 S DE(DW)="C3^DGPMX1"
 S DU="0:NO;1:YES;"
 G RE
...
X3 I $D(X),'$D(DGPMT) D EN^DDIOL("USE BED CONTROL MOVEMENT OPTIONS!",,"!") K X
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="0;12",DV="R*P43.4'",DU="",DIFLD=.12,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(43.4,"
 G RE
X4 S DIC("S")="I '$P(^(0),""^"",4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 S DW="PTF;4",DV="P35.2'",DU="",DIFLD=54,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DG(35.2,"
...
X7 I $$ELIG^DGCOMPACTELIG(DFN,$T(+0))="NOT ELIGIBLE" S Y="@10"
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 D ADMIT^DGCOMPACT(DFN,$G(PTF))
 Q
9 S DQ=10 ;@10
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 I $S('$D(^DPT(DFN,.3)):1,$P(^(.3),"^",1)'="Y":1,1:0) S Y="@11"
 Q
11 S DW="0;11",DV="S",DU="",DIFLD=.11,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="1:YES;0:NO;"
 G RE
X11 Q
12 S DQ=13 ;@11
13 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=13 D X13 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X13 S ^DISV(DUZ,"^DG(405.1,")=$S($D(^DISV(DUZ,"DGPM1")):^("DGPM1"),1:"")
 Q
14 S DW="0;4",DV="R*P405.1'X",DU="",DIFLD=.04,DLB="TYPE OF ADMISSION"
 S DE(DW)="C14^DGPMX1"
 S DU="DG(405.1,"
 G RE
...
C14F1 Q
X14 S DIC("S")="I $D(DGPMT),($P(^(0),""^"",2)=DGPMT),$P(^(0),""^"",4) S DGER=0,DGPMTYP=$P(^(0),""^"",3) D:DGPMT<4!(DGPMT=6)!(DGPMT=5) @(""DICS^DGPMV3""_DGPMT) I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
15 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=15 D X15 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X15 S ^DISV(DUZ,"DGPM1")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
16 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=16 D X16 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X16 I $P(^DGPM(DA,0),"^",18)'=9 S Y=.1
 Q
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW="0;5",DV="RP4'X",DU="",DIFLD=.05,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C17^DGPMX1"
 S DU="DIC(4,"
 G RE
...
C17S S X="" G:DG(DQ)=X C17F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C17F1 Q
X17 I '$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="0;10",DV="RFX",DU="",DIFLD=.1,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X18 K:$L(X)>30!($L(X)<3)!(X[";") X
 I $D(X),X'?.ANP K X
 Q
 ;
19 S DW="0;6",DV="R*P42'X",DU="",DIFLD=.06,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C19^DGPMX1"
 S DU="DIC(42,"
 G RE
...
C19F1 Q
X19 S DIC("S")="I $S($D(^(""ORDER"")):^(""ORDER""),1:0)" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
...
X21 I ($$UPPER^DGUTL($P($G(^DIC(42,X,0)),"^",1))["OBS"),($D(^UTILITY($J,"PXCOMPACT"))) S $P(^UTILITY($J,"PXCOMPACT"),"^",3)="P"
 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="0;7",DV="*P405.4'X",DU="",DIFLD=.07,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C22^DGPMX1"
 S DU="DG(405.4,"
 G RE
...
X23 D DFN^DGYZODS S:'DGODS Y="@12"
 Q
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW="ODS;1",DV="S",DU="",DIFLD=11500.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="1:YES;0:NO;"
 S Y="1"
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
...
X26 I DGPMP=^DGPM(DA,0) S Y=""
 Q
27 S DW="USR;3",DV="RP200'",DU="",DIFLD=102,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
