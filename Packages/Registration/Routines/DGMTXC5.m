...
DE S DIE="^DGMT(408.31,",DIC=DIE,DP=408.31,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGMT(408.31,DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,15) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGMTXC5",DQ=1
1 S DW="0;15",DV="NJ10,2",DU="",DLB="DEDUCTIBLE EXPENSES",DIFLD=.15
 S X=DGDET
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X1 Q
2 S DW="0;5",DV="NJ10,2",DU="",DLB="NET WORTH",DIFLD=.05
 S X=DGNWT
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 Q
3 D:$D(DG)>9 F^DIE17 G ^DGMTXC6
