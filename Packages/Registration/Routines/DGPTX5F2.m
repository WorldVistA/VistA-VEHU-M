...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^DGPT(D0,"M",DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,4) S:%]"" DE(1)=% S %=$P(%Z,U,5) S:%]"" DE(12)=% S %=$P(%Z,U,18) S:%]"" DE(6)=%,DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX5F2",DQ=1
1 S DW="0;4",DV="NJ7,0",DU="",DLB="PASS DAYS",DIFLD=4
 G RE
X1 K:+X'=X!(X>9999999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
2 S DQ=3 ;@17
...
X5 I $D(^DPT(+^DGPT(DGPTF,0),.3)),$P(^(.3),U)="Y" S (DGNFLD,Y)="@25"
 Q
6 S DW="0;18",DV="S",DU="",DLB="TREATED FOR SC CONDITION",DIFLD=18
 S DU="1:YES;2:NO;"
 S X=2
 S Y=X
...
X7 S (DGNFLD,Y)="@27"
 Q
8 S DQ=9 ;@25
9 S DW="0;18",DV="S",DU="",DLB="TREATED FOR SC CONDITION",DIFLD=18
 S DU="1:YES;2:NO;"
 S Y="NO"
 G Y
X9 Q
...
X11 S DGNFLD="@30"
 Q
12 S DW="0;5",DV="*P80'X",DU="",DLB="ICD 1",DIFLD=5
 S DE(DW)="C12^DGPTX5F2"
 S DU="ICD9("
 G RE
