DGMTXC6 ; ;06/24/93
 D DE G BEGIN
DE S DIE="^DGMT(408.31,",DIC=DIE,DP=408.31,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGMT(408.31,DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(2)=% S %=$P(%Z,U,18) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGMTXC6",DQ=1
1 S DW="0;18",DV="NJ2,0",DU="",DLB="TOTAL DEPENDENTS",DIFLD=.18
 S X=DGND
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X1 Q
2 S DW="0;6",DV="P200'",DU="",DLB="COMPLETED BY",DIFLD=.06
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X3 S:$P(^DGMT(408.31,DA,0),U,7) Y="@30"
 Q
4 D:$D(DG)>9 F^DIE17 G ^DGMTXC7
