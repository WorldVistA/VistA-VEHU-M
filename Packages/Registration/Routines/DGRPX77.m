...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,9) S:%]"" DE(38)=% S %=$P(%Z,U,11) S:%]"" DE(25)=%
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,3) S:%]"" DE(40)=%
 ...
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,2) S:%]"" DE(42)=% S %=$P(%Z,U,3) S:%]"" DE(43)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X7 D EN^DDIOL("other assistance, use the HEC Alert process.","","!")
 Q
8 S DQ=9 ;@7023
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".385;1",DV="DX",DU="",DIFLD=.3851,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C9^DGRPX77"
 G RE
...
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW=".385;2",DV="*P27.18'X",DU="",DIFLD=.3852,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^DGRPX77"
 S DU="DG(27.18,"
 S X="@"
 S Y=X
...
X13 S Y="@7026"
 Q
14 S DQ=15 ;@7024
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW=".385;2",DV="*P27.18'X",DU="",DIFLD=.3852,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C15^DGRPX77"
 S DU="DG(27.18,"
 G RE
...
X23 D EN^DDIOL("other assistance, use the HEC Alert process.","","!")
 Q
24 S DQ=25 ;@7026
25 D:$D(DG)>9 F^DIE17,DE S DQ=25,DW=".3;11",DV="SX",DU="",DIFLD=.3025,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C25^DGRPX77"
...
X26 S:(X'="Y")&($P($G(^DPT(DA,.362)),U,12,14)'["Y") Y=.36265
 Q
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW=".362;20",DV="NJ8,2X",DU="",DIFLD=.36295,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C27^DGRPX77"
 G RE
...
X27 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>99999)!(X<0) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),('$$TOTCHK^DGLOCK2(DFN)) D TOTCKMSG^DGLOCK2 K X
 Q
 ;
28 D:$D(DG)>9 F^DIE17,DE S DQ=28,DW=".362;17",DV="SX",DU="",DIFLD=.36265,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C28^DGRPX77"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X29 S:X'="Y" Y="@703"
 Q
30 D:$D(DG)>9 F^DIE17,DE S DQ=30,DW=".362;6",DV="NJ8,2X",DU="",DIFLD=.3626,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
X32 S:DGDR'["703" Y="@704"
 Q
33 S DW=".36;1",DV="*P8'Xa",DU="",DIFLD=.361,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C33F1 N X,X1,X2 S DIXR=852 D C33X1(U) K X2 M X2=X D C33X1("O") K X1 M X1=X
 D
...
X34 I $$GET1^DIQ(2,DFN_",",.361,"E")'["EXPANDED MH CARE NON-ENROLLEE" S Y="@7031"
 Q
35 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=35 D X35 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X35 D XPANDED^DGOTHD1(DFN) S:DGEMHCNVT Y="@704"
 Q
36 S DQ=37 ;@7031
37 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=37 D X37 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X37 D AAC1^DGLOCK2 S:DGAAC(1)']"" Y=361
 Q
38 D:$D(DG)>9 F^DIE17,DE S DQ=38,DW=".3;9",DV="*P35'X",DU="",DIFLD=.309,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(35,"
 G RE
X38 S DFN=DA D AAC^DGLOCK2
 Q
 ;
39 S D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPX78",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
 G RE:D I $D(DSC(2.0361))#2,$P(DSC(2.0361),"I $D(^UTILITY(",1)="" X DSC(2.0361) S D=$O(^(0)) S:D="" D=-1 G M39
 S D=$S($D(^DPT(DA,"E",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M39 I D>0 S DC=DC_D I $D(^DPT(DA,"E",+D,0)) S DE(39)=$P(^(0),U,1)
 G RE
R39 D DE
 S D=$S($D(^DPT(DA,"E",0)):$P(^(0),U,3,4),1:1) G 39+1
 ;
40 S DW=".32;3",DV="*P21'Xa",DU="",DIFLD=.323,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C40^DGRPX77"
 S DU="DIC(21,"
 G RE
...
C40S S X="" G:DG(DQ)=X C40F1 K DB
 D ^DGRPX79
C40F1 Q
X40 S DFN=DA D POS^DGLOCK1
 Q
 ;
41 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=41 D X41 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X41 D ^DGYZODS S:'DGODS Y="@704"
 Q
42 D:$D(DG)>9 F^DIE17,DE S DQ=42,DW="ODS;2",DV="S",DU="",DIFLD=11500.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C42^DGRPX77"
...
C42F1 Q
X42 Q
43 D:$D(DG)>9 F^DIE17,DE S DQ=43,DW="ODS;3",DV="*P25002.1'",DU="",DIFLD=11500.03,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C43^DGRPX77"
 S DU="DIC(25002.1,"
 G RE
...
C43S S X="" G:DG(DQ)=X C43F1 K DB
 D ^DGRPX713
C43F1 Q
X43 S DIC("S")="I '$P(^(0),""^"",4),(""^e^c^""[(""^""_$P(^(0),""^"",2)_""^""))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
44 S DQ=45 ;@704
45 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=45 D X45 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X45 S:DGDR'["704" Y="@705"
 Q
46 D:$D(DG)>9 F^DIE17,DE S DQ=46,D=0 K DE(1) ;.3731
 S DIFLD=.3731,DGO="^DGRPX714",DC="2^2.05A^.373^",DV="2.05MFX",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 G RE:D I $D(DSC(2.05))#2,$P(DSC(2.05),"I $D(^UTILITY(",1)="" X DSC(2.05) S D=$O(^(0)) S:D="" D=-1 G M46
 S D=$S($D(^DPT(DA,.373,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M46 I D>0 S DC=DC_D I $D(^DPT(DA,.373,+D,0)) S DE(46)=$P(^(0),U,1)
 G RE
R46 D DE
 S D=$S($D(^DPT(DA,.373,0)):$P(^(0),U,3,4),1:1) G 46+1
 ;
47 S DQ=48 ;@705
...
X49 D:'$G(DGEMHCNVT) YN^DGPPSYCH(DFN) S Y="@706"
 Q
50 S DQ=51 ;@706
51 S DQ=52 ;@99
52 G 0^DIE17
