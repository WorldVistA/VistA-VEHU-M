...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,9) S:%]"" DE(11)=% S %=$P(%Z,U,11) S:%]"" DE(1)=%
 I $D(^(.36)) S %Z=^(.36) S %=$P(%Z,U,1) S:%]"" DE(9)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,6) S:%]"" DE(6)=% S %=$P(%Z,U,17) S:%]"" DE(4)=% S %=$P(%Z,U,20) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGRPX711",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".3;11",DV="SX",DU="",DLB="RECEIVING VA DISABILITY?",DIFLD=.3025
 S DE(DW)="C1^DGRPX711"
...
C1S S X="" G:DG(DQ)=X C1F1 K DB
 ...
 D EVENT^IVMPLOG(DA)
C1F1 Q
X1 S DFN=DA D MV^DGLOCK I $D(X),X="Y" D EC^DGLOCK1
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 S:(X'="Y")&($P($G(^DPT(DA,.362)),U,12,14)'["Y") Y=.36265
 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".362;20",DV="NJ8,2X",DU="",DLB="TOTAL ANNUAL VA CHECK AMOUNT",DIFLD=.36295
 S DE(DW)="C3^DGRPX711"
 G RE
...
C3S S X="" G:DG(DQ)=X C3F1 K DB
 ...
 D AUTOUPD^DGENA2(DA)
C3F1 Q
X3 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>99999)!(X<0) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),('$$TOTCHK^DGLOCK2(DFN)) D TOTCKMSG^DGLOCK2 K X
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".362;17",DV="SX",DU="",DLB="GI INSURANCE POLICY?",DIFLD=.36265
 S DE(DW)="C4^DGRPX711"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
C4F1 Q
X4 S DFN=DA D MV^DGLOCK Q
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 S:X'="Y" Y="@703"
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".362;6",DV="NJ8,2X",DU="",DLB="AMOUNT OF GI INSURANCE",DIFLD=.3626
 G RE
X6 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>999999)!(X<1) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),$D(^DPT(DA,.362)),$P(^(.362),U,17)'="Y" W !?4,*7,"Applicant doesn't have GI Insurance." K X
 Q
 ;
7 S DQ=8 ;@703
...
X8 S:DGDR'["703" Y="@704"
 Q
9 S DW=".36;1",DV="*P8'Xa",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C9^DGRPX711",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 G RE
...
C9F1 N X,X1,X2 S DIXR=852 D C9X1(U) K X2 M X2=X D C9X1("O") K X1 M X1=X
 D
...
C9X1(DION) K X
 ...
 Q
C9F2 Q
X9 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 D AAC1^DGLOCK2 S:DGAAC(1)']"" Y=361
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".3;9",DV="*P35'X",DU="",DLB="AGENCY/ALLIED COUNTRY",DIFLD=.309
 S DU="DIC(35,"
 G RE
X11 S DFN=DA D AAC^DGLOCK2
 Q
 ;
12 S D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPX712",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW="ELIGIBILITY",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
 G RE:D I $D(DSC(2.0361))#2,$P(DSC(2.0361),"I $D(^UTILITY(",1)="" X DSC(2.0361) S D=$O(^(0)) S:D="" D=-1 G M12
 S D=$S($D(^DPT(DA,"E",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M12 I D>0 S DC=DC_D I $D(^DPT(DA,"E",+D,0)) S DE(12)=$P(^(0),U,1)
 G RE
R12 D DE
 S D=$S($D(^DPT(DA,"E",0)):$P(^(0),U,3,4),1:1) G 12+1
 ;
13 D:$D(DG)>9 F^DIE17 G ^DGRPX713
