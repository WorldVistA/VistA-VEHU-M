DGYPGLO2 ;ALB/LM - TREATING SPECIALTY INPATIENT LISTING BY TS ;2-2-93
...
START Q:'PTLTS
 ...
 D SUBHEAD
 ;
DIV S DIV="" F DIV1=0:0 S DIV=$O(^TMP($J,"PTLTS",DIV)) Q:DIV=""  D:$Y+8>IOSL HEAD^DGYPGLO,SUBHEAD Q:END  W !?5,"DIVISION: ",$S($D(^DG(40.8,DIV,0)):$P(^(0),"^"),1:"EMPTY") D TREAT Q:END  S SUBCOUNT=^TMP($J,"PTLTS",DIV) D TOTAL Q:END
 ;
 G:END END
...
TREAT S TREAT="" F TREAT1=0:0 S TREAT=$O(^TMP($J,"PTLTS",DIV,TREAT)) Q:TREAT=""  Q:END  D DGTS
 Q
 ;
DGTS S DGTS="" F DGTS1=0:0 S DGTS=$O(^TMP($J,"PTLTS",DIV,TREAT,DGTS)) Q:DGTS=""  D:$Y+8>IOSL HEAD^DGYPGLO,SUBHEAD Q:END  W !!?10,"FACILITY TREATING SPECIALTY:  ",TREAT D PTNM Q:END  S SUBCOUNT=^TMP($J,"PTLTS",DIV,TREAT,DGTS) S SUBNAME="TS" D SUB
 Q
 ;
