DGINTEG ;ALB/MRL - MAS INTEGRITY CHECKER ; DEC 12, 1987
 ;;MAS VERSION 5.1;
 G EN
...
CC S DGCC="F DGC=1:1:DGLN S DGA=$A(DGL,DGC) S DGD=DGA_""^""_$P(DGD,""^"",1,2) S DGT=(DGA*DGC)+DGT I $S(DGLN>2:$P(DGD,""^"",2,3),1:$P(DGD,""^"",1,2))=""59^32"",DGA'=59!(DGLN=2) X DGSB Q"
 ...
 S DGWR="W "" ASCII Value ===> "",$J(DGT,11),""-Now"" S Y=$N(^DG(43,1,""ROU"",""B"",DGR,0)) I Y>0,$D(^DG(43,1,""ROU"",Y,0)) S X=$P(^(0),""^"",2) I X W $J(X,11),""-Orig."" X DGWR1"
