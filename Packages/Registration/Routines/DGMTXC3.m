...
DE S DIE="^DGMT(408.31,",DIC=DIE,DP=408.31,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGMT(408.31,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,20) S:%]"" DE(1)=% S %=$P(%Z,U,21) S:%]"" DE(2)=% S %=$P(%Z,U,22) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGMTXC3",DQ=1
1 S DW="0;20",DV="S",DU="",DLB="HARDSHIP?",DIFLD=.2
 S DE(DW)="C1^DGMTXC3"
 S DU="1:YES;0:NO;"
 S Y="@"
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;21",DV="RD",DU="",DLB="HARDSHIP REVIEW DATE",DIFLD=.21
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 Q
3 S DW="0;22",DV="RP200'",DU="",DLB="APPROVED BY",DIFLD=.22
 S DU="VA(200,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
