...
EXPORT ; Called from TaskMan job
 ...
 L +^DGAUDIT2(0):2 I '$T I DGDEBUGON D
...
EXPORT3(DGABORT) ;JPN ADDED FOR BREAKING UP DIA GLOBAL
 ...
 . S DGRESP=$$RESTPOST(BATCHID,BATSIZE,.DGERRARR,DGAUDWRCNT)
 ...
 . D PROCRESP(.DGRESP,BATCHID,.DGERRARR,DGAUDWRCNT,DGAUDWRMAX,.DGCONSERR)
 . I DGAUDFAIL>DGAUDWRMAX S DGABORT=1 Q
 . S DGAUDSTOP=$$S^%ZTLOAD,DGAUDSHUT=$$GET1^DIQ(46.5,1,.02,"I")
 Q
 ;
RESTPOST(BATCHID,BATSIZE,DGERRARR,DGAUDWRCNT) ; Build batch containing BATSIZE JSON records from ^DGAUDIT
 ...
 S DGRESTOBJ.SSLCheckServerIdentity=0  ; Older versions of xobw.WebServer.cls don't set this value. Setting here to prevent Error #6156 during the POST below.
 ;
 ...
 . N DGJSON,FDA,DGDATA,DGERR
 . S DGJSON=$G(^DGAUDIT(DGAUDCNT,1))
 . D DECODE^XLFJSON("DGJSON","DGDATA","DGERR")
 . I '$L($G(DGDATA("id"))) D BADJSON^DGAUDIT1(DGAUDCNT,+$G(DGAUDKPX)) Q
 . S ^TMP($J,"DGAUDIT",BATCHID,DGAUDCNT)=DGDATA("id")
 ...
 Q DGRESPONSE
 ; 
PROCRESP(DGRESP,BATCHID,DGERRARR,DGAUDWRCNT,DGAUDWRMAX,DGCONSERR) ; process response
 ...
 .. N DGERRTXT S DGERRTXT=$S($L($G(DGERRARR("statusLine"))):$G(DGERRARR("statusLine")),1:$P($G(DGRESP),"^",2))
 .. D APPERROR^%ZTER(DGERRTXT)
 ...
 . N DGEXCEPT,DGAUDCNT
 . S DGEXCEPT=0 F  S DGEXCEPT=$O(DGRESP("failedIds",DGEXCEPT)) Q:DGEXCEPT=""  D
 .. ;  DGRESP("failedIds",exceptionSequence)=queueSequence.batchId
 ...
 .. ;    DGRESP("failedIds",1)=10.3211117.13122601
 .. ;    DGRESP("failedIds",2)=50.3211117.13122601
 .. S DGAUDCNT=$P(DGRESP("failedIds",DGEXCEPT),".") Q:'DGAUDCNT   ; Need ^DGAUDIT queue IEN to retrieve JSON
 .. S DGAUDERR=1
 .. ;
...
SNDMSG(DGAUDMSG,DGAUDGRP,DGALTSUB) ; Send mail message to mail group
 ...
 S XMY("G."_DGEMAIL)=""                                    ;JPN ADDED 3/31/21
