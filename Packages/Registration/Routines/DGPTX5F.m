DGPTX5F ; GENERATED FROM 'DG501F' INPUT TEMPLATE(#87), FILE 45;06/24/93
 D DE G BEGIN
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGPT(DA,-1))<0
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX5F",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=87,U="^"
...
X3 S DGNFLD="@10"
 Q
4 S D=0 K DE(1) ;50
 S DIFLD=50,DGO="^DGPTX5F1",DC="33^45.02AI^M^",DV="45.02MNJ6,1X",DW="0;1",DOW="MOVEMENT RECORD",DLB="Select "_DOW S:D DC=DC_D
 G RE:D I $D(DSC(45.02))#2,$P(DSC(45.02),"I $D(^UTILITY(",1)="" X DSC(45.02) S D=$N(^(0)) G M4
 S D=$S($D(^DGPT(DA,"M",0)):$P(^(0),U,3,4),1:$N(^(0)))
