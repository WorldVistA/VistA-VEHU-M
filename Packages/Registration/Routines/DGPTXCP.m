DGPTXCP ; GENERATED FROM 'DGPT QUICK PROFILE' PRINT TEMPLATE (#1053) ; 12/20/96 ; (FILE 45.86, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1053,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
