DGPTXB ; GENERATED FROM 'DG PTF PT BRIEF LIST' PRINT TEMPLATE (#1097) ; 03/27/98 ; (FILE 45, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1097,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
