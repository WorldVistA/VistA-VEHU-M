...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPMXAS3",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;12",DV="*P43.4'",DU="",DLB="ADMITTING REGULATION",DIFLD=.12
 S DU="DIC(43.4,"
 G RE
X1 S DIC("S")="I '$P(^(0),""^"",4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 I $S('$D(^DPT(DFN,.3)):1,$P(^(.3),"^",1)'="Y":1,1:0) S Y="@1"
 Q
3 D:$D(DG)>9 F^DIE17 G ^DGPMXAS4
