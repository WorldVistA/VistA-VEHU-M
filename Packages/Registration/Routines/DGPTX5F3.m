...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^DGPT(D0,"M",DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,8) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX5F3",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;6",DV="*P80'X",DU="",DLB="ICD 2",DIFLD=6
 S DE(DW)="C1^DGPTX5F3"
 S DU="ICD9("
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 K K S DIC("S")="S DGI=6 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 I X K DGPTIT S DGNFLD="@40",Y="@800",DGPTIT(X_$C(59)_"ICD9(")=""
 Q
3 S DQ=4 ;@40
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 S DGNFLD="@50"
 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;7",DV="*P80'X",DU="",DLB="ICD 3",DIFLD=7
 S DE(DW)="C5^DGPTX5F3"
 S DU="ICD9("
 G RE
...
C5S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^DGPT(DA(1),"M","AC",$E(X,1,30),DA)=""
 Q
X5 K K S DIC("S")="S DGI=7 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X6 I X K DGPTIT S DGNFLD="@50",Y="@800",DGPTIT(X_$C(59)_"ICD9(")=""
 Q
7 S DQ=8 ;@50
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S DGNFLD="@60"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;8",DV="*P80'X",DU="",DLB="ICD 4",DIFLD=8
 S DE(DW)="C9^DGPTX5F3"
 S DU="ICD9("
 G RE
