DGPTXC ; GENERATED FROM 'DG PTF CREATE PTF ENTRY' INPUT TEMPLATE(#855), FILE 45;03/11/23
 D DE G BEGIN
...
BEGIN S DNM="DGPTXC",DQ=1
 ...
 M DIEZAR=^DIE(855,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=855,U="^"
...
C1F1 S DIEZRXR(45,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1147,1148,1149,1150,1151,1152,1153,1154,1155,1156,1157,1158,1159,1160,1161,1238,1239,1240,1241,1242,1243,1244,1245,1246,1247,1248 S DIEZRXR(45,DIXR)=""
 Q
X1 Q
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,D=0 K DE(1) ;50
 S DIFLD=50,DGO="^DGPTXC1",DC="80^45.02AI^M^",DV="45.02MNJ6,1X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
C5F1 S DIEZRXR(45,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1147,1148,1149,1150,1151,1152,1153,1154,1155,1156,1157,1158,1159,1160,1161,1238,1239,1240,1241,1242,1243,1244,1245,1246,1247,1248,1320,1699 S DIEZRXR(45,DIXR)=""
 Q
X5 Q
6 G 0^DIE17
