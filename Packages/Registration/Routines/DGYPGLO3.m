DGYPGLO3 ;ALB/LM - TREATING SPECIALTY INPATIENT COUNTS BY TS ;2-2-93
...
START Q:'PTCTS
 ...
 D SUBHEAD
 ;
DIV S DIV="" F DIV1=0:0 S DIV=$O(^TMP($J,"PTCTS",DIV)) Q:DIV=""  D:$Y+8>IOSL HEAD^DGYPGLO,SUBHEAD Q:END  W !?5,"DIVISION: ",$S($D(^DG(40.8,DIV,0)):$P(^(0),"^"),1:"EMPTY") D TREAT Q:END  D SUB Q:END
 ;
 G:END END
...
DGTS S DGTS="" F DGTS1=0:0 S DGTS=$O(^TMP($J,"PTCTS",DIV,TREAT,DGTS)) Q:DGTS=""  D SV Q:END
 Q
 ;
SV S SV="" F SV1=0:0 S SV=$O(^TMP($J,"PTCTS",DIV,TREAT,DGTS,SV)) Q:SV=""  D:$Y+8>IOSL HEAD^DGYPGLO,SUBHEAD Q:END  D INFO Q:END
 Q
 ;
