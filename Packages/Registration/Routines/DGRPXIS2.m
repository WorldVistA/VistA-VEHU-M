...
DE S DIE="^DGMT(408.21,",DIC=DIE,DP=408.21,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGMT(408.21,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,16) S:%]"" DE(1)=% S %=$P(%Z,U,17) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGRPXIS2",DQ=1
1 S DW="0;16",DV="NJ8,2X",DU="",DLB="WORKERS COMP. OR BLACK LUNG",DIFLD=.16
 G RE
X1 S:X["$" X=$P(X,"$",2) X:X["*" "S X=X*12 W ""  "",X" K:X'?.N.1".".2N!(X>99999)!(X<0) X
 Q
 ;
2 S DQ=3 ;@910
...
X3 S:DGDR'[910 Y="@998"
 Q
4 S DW="0;17",DV="NJ9,2X",DU="",DLB="ALL OTHER INCOME",DIFLD=.17
 G RE
X4 S:X["$" X=$P(X,"$",2) X:X["*" "S X=X*12 W ""  "",X" K:X'?.N.1".".2N!(X>999999)!(X<0) X
 Q
 ;
5 S DQ=6 ;@998
