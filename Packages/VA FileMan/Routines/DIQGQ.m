...
DDENTRY N DIQGQE S DIQGQE=0
 ...
 K DIERR,^TMP("DIERR",$J) N DIERR ;p25
 ...
 S:$D(@DIQGSI@(0)) DIQGXDDN=+$P(^(0),"^",2),DIQGXDD="^DD("_DIQGXDDN_")" I '$D(DIQGXDD) N X S X("FILE")=DIQGR D BLD^DIALOG(401,.X) G ERROUT ;p25
...
AUDIT I DIQGIPAR'["R" N DIQGAUDR,DIQGAUDD S DIQGAUDD=+$P(DIQGPARM,"A",2) I DIQGAUDD D GET^DIAUTL(DIQGXDDN,DA_",",DIQGAUDD,"DIQGAUDR") ;is there an AUDIT TRAIL??
 ...
 .I DIQGXDN?.N,'$D(@DIQGXDD@(DIQGXDN,0)) D BLD^DIALOG(200,.X) G ERROUT ;check that field exists ;p25
...
ERROUT I $G(DIERR) S $P(DIQGQERR,U)=$P($G(DIQGQERR),U)+DIERR,$P(DIQGQERR,U,2)=$P($G(DIQGQERR),U,2)+$P(DIERR,U,2) K DIERR S DIQGQE=DIQGQE+1 ;p25
OUT Q 
