DIQGQ ;SFISC/DCL-DATA RETRIEVAL ; Jun 08, 2023@08:54:56
 ;;22.2;VA FileMan;**28**;Jan 05, 2016;Build 2
...
DDENTRY N DIQGQE S DIQGQE=0
 ...
 S:$D(@DIQGSI@(0)) DIQGXDDN=+$P(^(0),"^",2),DIQGXDD="^DD("_DIQGXDDN_")" I '$D(DIQGXDD) N X S X("FILE")=DIQGR D BLD^DIALOG(401,.X) G ERROUT ;p28
...
AUDIT I DIQGIPAR'["R" N DIQGAUDR,DIQGAUDD S DIQGAUDD=+$P(DIQGPARM,"A",2) I DIQGAUDD D GET^DIAUTL(DIQGXDDN,DA_",",DIQGAUDD,"DIQGAUDR") ;is there an AUDIT TRAIL??
 ...
 .I DIQGXDN?.N,'$D(@DIQGXDD@(DIQGXDN,0)) D BLD^DIALOG(200,.X) G ERROUT ;check that field exists ;p28
...
ERROUT I $G(DIERR) S $P(DIQGQERR,U)=$P($G(DIQGQERR),U)+DIERR,$P(DIQGQERR,U,2)=$P($G(DIQGQERR),U,2)+$P(DIERR,U,2) K DIERR S DIQGQE=DIQGQE+1 ;p28
OUT Q 
