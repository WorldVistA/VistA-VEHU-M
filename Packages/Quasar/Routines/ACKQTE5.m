...
DE S DIE="^ACK(509850.6,",DIC=DIE,DP=509850.6,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^ACK(509850.6,DA,""))=""
 I $D(^(2)) S %Z=^(2) S %=$P(%Z,U,2) S:%]"" DE(26)=% S %=$P(%Z,U,9) S:%]"" DE(23)=%
 I $D(^(5)) S %Z=^(5) S %=$P(%Z,U,4) S:%]"" DE(1)=% S %=$P(%Z,U,5) S:%]"" DE(6)=% S %=$P(%Z,U,6) S:%]"" DE(11)=% S %=$P(%Z,U,9) S:%]"" DE(15)=% S %=$P(%Z,U,10) S:%]"" DE(19)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X9 I ACKENV=0 S Y="@90"
 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 I ACKSC=2 S Y="@90"
 Q
...
X14 I ACKHNC=0 S Y="@95"
 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="5;9",DV="RS",DU="",DLB="Was care related to Head and/or Neck Cancer ?",DIFLD=40
 S DU="0:NO;1:YES;"
 S X=""
 S Y=X
 G Y
...
X16 S ACKHNC=$S(ACKHNC=0:0,ACKSC'=1:ACKHNC,X=1:2,1:1)
 Q
17 S DQ=18 ;@95
18 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=18 D X18 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X18 I ACKCV=0 S Y="@100"
 Q
19 S DW="5;10",DV="RS",DU="",DLB="Was care related to COMBAT ?",DIFLD=45
 S DU="0:NO;1:YES;"
 S X="YES"
 S Y=X
 G Y
...
X20 S ACKCV=$S(ACKCV=0:0,ACKSC'=1:ACKCV,X=1:2,1:1)
 Q
21 S DQ=22 ;@100
22 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=22 D X22 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X22 I '$$MST^ACKQUTL4(ACKPCE,ACKVD,ACKPAT) S Y="@105"
 Q
23 S DW="2;9",DV="RS",DU="",DLB="Was care related to MST ?",DIFLD=90
 S DE(DW)="C23^ACKQTE5"
 S DU="1:YES;0:NO;"
 S X=""
 S Y=X
...
X24 I '$$MST^ACKQUTL4(ACKPCE,ACKVD,ACKPAT) D CLEAR^ACKQUTL4(ACKVIEN,"90")
 Q
25 S DQ=26 ;@105
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,DW="2;2",DV="*P509850'",DU="",DLB="CDR ACCOUNT",DIFLD=5
 S DE(DW)="C26^ACKQTE5"
 S DU="ACK(509850,"
 S X=$$CDR^ACKQUTL6()
 S Y=X
...
X36 W !!,"Enter Audiometrics :",!
 Q
37 D:$D(DG)>9 F^DIE17 G ^ACKQTE8
