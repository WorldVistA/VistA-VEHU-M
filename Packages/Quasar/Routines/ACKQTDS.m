ACKQTDS ; GENERATED FROM 'ACKQTDS' PRINT TEMPLATE (#2011) ; 10/28/96 ; (FILE 509850.6, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2011,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
