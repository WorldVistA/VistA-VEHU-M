MCOBPF ; GENERATED FROM 'MCPFTBRPR1' PRINT TEMPLATE (#2106) ; 07/14/98 ; (FILE 700, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2106,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
