MCAROH2 ; GENERATED FROM 'MCARHOLT2' PRINT TEMPLATE (#2048) ; 07/22/97 ; (FILE 691.6, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2048,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
