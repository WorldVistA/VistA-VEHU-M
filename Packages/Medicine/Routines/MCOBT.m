MCOBT ; GENERATED FROM 'MCARETTBRPR' PRINT TEMPLATE (#2092) ; 11/14/97 ; (FILE 691.7, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2092,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
