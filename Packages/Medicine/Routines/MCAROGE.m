MCAROGE ; GENERATED FROM 'MCGENERICPRINT' PRINT TEMPLATE (#2086) ; 08/21/97 ; (FILE 699.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2086,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
