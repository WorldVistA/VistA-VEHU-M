LEX2037 ; ISL/KER - Environment Check/Pre/Post Install ; 09/02/2005
 ...
 ;                            
ENV ; LEX*2.0*37 Environment Check
 ;
 ;   General
 ;
 N LEXBUILD,LEXIGHF,LEXREQP,LEXLREV,LEXG,LEXE
 W !," Code Set Update for FY06 1st Quarter - includes ICD Annual Release",!
 D IMP S U="^"
 ;     No user
...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*37")=1
...
CPD(X) ;  Check Current Patched Data is installed
 ...
 S:'$D(^LEX(757.1,"B",183396,260563)) INS=0
 S:$D(^LEX(757.02,"ACODE","V64.0 ",306079)) INS=0
 ;    Check Last ICD Diagnosis Set/Kill
 S:'$D(^ICD9("D","WORK",14095)) INS=0
 S:$D(^ICD9("ACC",13976,9243)) INS=0
 ;    Check Last ICD Procedures Set/Kill
 S:'$D(^ICD0("E","SUBTALR JT ARTHROEREISIS",1959)) INS=0
 S:$D(^ICD0("E","OTHER FUSION OF TOE",1959)) INS=0
 ;    Check Last CPT/HCPCS Procedures Set
 S:'$D(^ICPT("F","S3626",108117)) INS=0
 ;    Check for V3
 S:'$D(^ICD9(9243,66,2,0)) INS=0
 S X=+($G(INS))
 Q X
 ;
 ; Error messages
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=37,LEXREQP="LEX*2.0*36",LEXBUILD="LEX*2.0*37"
 S LEXIGHF="LEX_2_37.GBL"
 Q
