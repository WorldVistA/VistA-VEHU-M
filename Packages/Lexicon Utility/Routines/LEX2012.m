LEX2012 ; ISA/CJE-LEX*2.0*12 Environment Check/Pre/Post Install    ; 11-09-98
 ;;2.0;Lexicon Utility;**12**;Sept 23, 1996
 ;
ENV ; LEX*2.0*12 Environment Check
 ;           
 ;   General
 ;
...
UR(X) ;   Check User variables
 Q:'$L($G(DUZ(0))) 0 Q:+($G(DUZ))=0!('$D(^VA(200,+($G(DUZ)),0))) 0 Q 1
REV(X) ;   Check File Revision Level
 D IMP N LEXLRV S LEXLRV=+($$LRV) Q:LEXLRV<+($G(LEXLREV)) 0 Q:LEXLRV'<+($G(LEXLREV)) 1
LRV(X) ;   Check Last Installed Revision Number
 N LEXFI,LEXR S X=0,LEXFI=756.999999 F  S LEXFI=$O(^DD(LEXFI)) Q:+LEXFI=0!($E(LEXFI,1,3)'="757")  S LEXR=+($G(^DD(LEXFI,0,"VRRV"))) S:LEXR>X X=LEXR
 Q X
...
LPD(X) ;   Check Last Patched Data is installed
 Q:$D(^LEX(757.21,"C","VESTIBULOPLASTY, RIDGE EXTENSION (SECOND EPITHELIALIZATION)",7717))&('$D(^LEX(757.21,"C","GENERAL ANESTHESIA",107))) 1 Q 0
 Q
 ;                   
 ; Error messages
 ;
...
IMP ;   Import names
 ;
 S LEXLREV=7,LEXLAST="LEX*2.0*10",LEXBUILD="LEX*2.0*12"
 S LEXIGHF="LEX_2_P12.GBL"
 Q
