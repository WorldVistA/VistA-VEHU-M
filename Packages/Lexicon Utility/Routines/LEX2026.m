LEX2026 ;ISL/KER - Environment Check/Pre/Post Install ; 10/15/2003
 ...
 ;   DBIA 10015  EN^DIQ1
 ;                            
ENV ; LEX*2.0*26 Environment Check
 ;           
 ;   General
 ;
...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*26")=1
...
CPD(X) ;   Check Current Patched Data is installed
 N INS S INS=1
 ; Last Lexicon SET     ^LEXM(757.1,656)=S ^LEX(757.1,"B",180726,257893)=""
 S:'$D(^LEX(757.1,"B",180726,257893)) INS=0
 ; Last Lexicon KILL    ^LEXM(757.02,4260)=K ^LEX(757.02,"CODE","607.89 ",96518)
 S:$D(^LEX(757.02,"CODE","607.89 ",96518)) INS=0
 ; Last ICD Diagnosis SET
 ;  ^LEXM(80,11467)=S ^ICD9("D","WITHOUT",13860)=""
 S:'$D(^ICD9("D","WITHOUT",13860)) INS=0
 ; Last ICD Diagnosis KILL
 ;  ^LEXM(80,13538)=K ^ICD9("D","WITHOUT",12481)
 S:$D(^ICD9("D","WITHOUT",12481)) INS=0
 ; Last ICD Procedure SET
 ;   ^LEXM(80.1,186)=S ^ICD0("E","REPL/REP THORAC UNIT HRT",4207)=""
 S:'$D(^ICD0("E","REPL/REP THORAC UNIT HRT",4207)) INS=0
 ; Last ICD Procedure KILL
 ;   ^LEXM(80.1,206)=K ^ICD0("E","REPLACE FEMORAL HEAD NEC",1983)
 S:$D(^ICD0("E","REPLACE FEMORAL HEAD NEC",1983)) INS=0
 S X=INS
 Q X
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=16,LEXLAST="LEX*2.0*25",LEXBUILD="LEX*2.0*26"
 S LEXIGHF="LEX20_26.GBL"
 Q
