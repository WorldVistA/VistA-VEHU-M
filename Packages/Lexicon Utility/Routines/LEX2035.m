LEX2035 ; ISL/KER - Environment Check/Pre/Post Install ; 02/16/2005
 ...
 ;                            
ENV ; LEX*2.0*35 Environment Check
 ;
 ;   General
 ;
...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*35")=1
...
CPD(X) ;  Check Current Patched Data is installed
 ...
 S:'$D(^LEX(757.11,"F","ORGANISM",12)) INS=0
 S:$D(^LEX(757.11,"F","ORGANISM",0)) INS=0
 ;    Check Last CPT/HCPCS Procedures Set/Kill
 S:'$D(^ICPT("F","S9097",108013)) INS=0
 S:$D(^ICPT("C","SUPPLIES",106588,3)) INS=0
 ;    Check Last CPT Modifiers Set/Kill
 S:'$D(^DIC(81.3,"M",865362,865364,370)) INS=0
 S:$D(^DIC(81.3,"C","REPEAT DX LAB TST SAME DAY",362)) INS=0
 ;    V2
 S:$L($P($G(^LEX(757.02,325204,0)),"^",8,12))>0 INS=0
 ;    V3
 S:$G(^LEX(757.01,332250,0))["S0lution" INS=0
 S X=+($G(INS))
 Q X
 ;
 ; Error messages
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=35,LEXREQP="LEX*2.0*34",LEXBUILD="LEX*2.0*35"
 S LEXIGHF="LEX_2_35.GBL"
 Q
