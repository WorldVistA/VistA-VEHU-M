LEX2097 ;ISL/KER - LEX*2.0*97 Env Check ;10/02/2014
 ...
 ;     
 ; <<<<<<<<<<<<<<<<<<<<<<<<<< WARNING >>>>>>>>>>>>>>>>>>>>>>>
 ; Do not use the environment check and post-install routines
 ; for LEX*2.0*97 as a model for other patches.
 ; >>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ; 
...
IMP ;   Import names
 ...
 S LEXREQP(1)="LEX*2.0*96 SEQ #86^3140701^Code Set FY14 4th Qtr"
 S LEXREQP(2)="PXRM*2.0*26 SEQ #27^3140724^Clinical Reminders ICD-10 Chgs"
 S LEXREQP(3)="LEX*2.0*87 SEQ #87^3140807^ICD-10 Update/Date Change"
 ;     This Build Name
 S LEXBUILD="LEX*2.0*97"
 ;     This Build's Export Global Host Filename
 S LEXIGHF="LEX_2_97.GBL"
 ;     Fiscal Year
 S LEXFY="FY15"
 ;     Quarter
 S LEXQTR="1st"
