PRCATF ; GENERATED FROM 'PRCA FMS STATUS' PRINT TEMPLATE (#1872) ; 05/03/96 ; (FILE 347, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1872,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
