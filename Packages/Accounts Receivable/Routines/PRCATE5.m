...
DE S DIE="^PRCA(430,",DIC=DIE,DP=430,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRCA(430,DA,""))=""
 I $D(^(202)) S %Z=^(202) S %=$P(%Z,U,1) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCATE5",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="202;1",DV="F",DU="",DLB="INSURED NAME",DIFLD=239
 G RE
X1 K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
