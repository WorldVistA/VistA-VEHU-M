PRCATA ; GENERATED FROM 'PRCA OLD SET' INPUT TEMPLATE(#722), FILE 430;04/29/10
 D DE G BEGIN
DE S DIE="^PRCA(430,",DIC=DIE,DP=430,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRCA(430,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,7) S:%]"" DE(9)=% S %=$P(%Z,U,8) S:%]"" DE(2)=% S %=$P(%Z,U,9) S:%]"" DE(11)=% S %=$P(%Z,U,16) S:%]"" DE(4)=% S %=$P(%Z,U,21) S:%]"" DE(6)=%,DE(13)=%
 I $D(^(202)) S %Z=^(202) S %=$P(%Z,U,1) S:%]"" DE(14)=% S %=$P(%Z,U,2) S:%]"" DE(16)=% S %=$P(%Z,U,4) S:%]"" DE(17)=% S %=$P(%Z,U,5) S:%]"" DE(18)=% S %=$P(%Z,U,6) S:%]"" DE(19)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="PRCATA",DQ=1
 ...
 M DIEZAR=^DIE(722,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=722,U="^"
