PRCATSE ; GENERATED FROM 'PRCASV REL' INPUT TEMPLATE(#732), FILE 430;01/27/04
 D DE G BEGIN
...
BEGIN S DNM="PRCATSE",DQ=1
 ...
 M DIEZAR=^DIE(732,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=732,U="^"
...
C4S S X="" G:DG(DQ)=X C4F1 K DB
 S X=DG(DQ),DIC=DIE
 I $P($G(^PRCA(430,+DA,0)),"^",8)=16 S ^PRCA(430,"AJK1",X,DA)=""
C4F1 Q
X4 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="104;3",DV="D",DU="",DLB="DATE SIGNED (APPROVED)",DIFLD=113
 S X=DT
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
...
X8 S:'$D(PRCASV("2NDINS")) Y="@1"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;19",DV="P36'",DU="",DLB="SECONDARY INSURANCE CARRIER",DIFLD=19
 S DU="DIC(36,"
 S X=PRCASV("2NDINS")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X10 S:'$D(PRCASV("3RDINS")) Y="@1"
 Q
11 S DW="0;20",DV="P36'",DU="",DLB="TERTIARY INSURANCE CARRIER",DIFLD=19.1
 S DU="DIC(36,"
 S X=PRCASV("3RDINS")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X11 Q
12 S DQ=13 ;@1
13 S DW="0;8",DV="R*P430.3'X",DU="",DLB="CURRENT STATUS",DIFLD=8
 S DE(DW)="C13^PRCATSE"
 S DU="PRCA(430.3,"
