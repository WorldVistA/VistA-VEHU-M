PRCATP1 ; GENERATED FROM 'PRCA PROFILE' PRINT TEMPLATE (#765) ; 05/26/95 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(765,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
