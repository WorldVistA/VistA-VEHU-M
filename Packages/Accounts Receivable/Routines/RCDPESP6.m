RCDPESP6 ;AITC/CJE - ePayment Lockbox Site Parameters - Notify Changes;29 Jan 2019 18:00:14
 ;;4.5;Accounts Receivable;**326,332,345**;Mar 20, 1995;Build 34
...
EN ; On entry into parameter edit, save a snapshot of the files
 ...
 K ^TMP($T(+0),$J)
 M ^TMP($T(+0),$J,344.6)=^RCY(344.6)  ; RCDPE AUTO-PAY EXCLUSION
 M ^TMP($T(+0),$J,344.61)=^RCY(344.61)  ; RCDPE PARAMETER
 M ^TMP($T(+0),$J,344.62)=^RCY(344.62)  ; RCDPE CARC-RARC AUTO DEC
 Q
 ;
EXIT ; On exit compare snapshots with files
 ...
 ;
 N C,CHANGES,G,LINES,RCMSGTXT,RCSUBJ,TXTLNS,XMINSTR,XMTO
 ;Check for changed parameters, if no changes, don't send message
 Q:'$$CHKCHNG(.TXTLNS)  ; Check for changes in parameters
 ;
 S RCSUBJ=$E("ePayments EDI Lockbox Parameters changed "_$P($$SITE^VASITE,U,2),1,65)
 D HEADER(.RCMSGTXT)
 S G=RCMSGTXT,C=0
 F  S C=$O(TXTLNS(C)) Q:'C  S RCMSGTXT(C+G)=TXTLNS(C)
 ;
 ...
 . K ^TMP("XMERR",$J)
 ;
 K ^TMP($T(+0),$J)  ; Clean up
 Q
 ;
HEADER(MSGTXT)  ; Add Header Lines to mail message text
 ; Output:  Array MSGTXT passed by reference
 N C D XMSGBODY^RCDPESPB(.MSGTXT)
 S C=MSGTXT
 S C=C+1,MSGTXT(C)=" The following EDI Lockbox Site Parameters were changed: "
 S C=C+1,MSGTXT(C)=" "
 S C=C+1,MSGTXT(C)=$J("",50)_$J("OLD VALUE",10)_"  "_$J("NEW VALUE",10)
 S MSGTXT=C
 Q
 ;
...
MEDCHNG(HEAD,COUNT,LINE)  ; Check for Medical site parameter changes - PRCA*4.5*345
 ...
 I XNEW'=XOLD D LNOUT(.HEAD,.LINE,"AUTO-POST MED CLAIMS ENABLED",XOLD,XNEW,"B",.COUNT)
 ;
 ...
 I XNEW'=XOLD D LNOUT(.HEAD,.LINE,"AUTO-DECREASE NO-PAY MED ENABLED",XOLD,XNEW,"B",.COUNT)
 ;
 ; Auto-decrease no-pay med days
 S XOLD=$P(MEDND,U,12),XNEW=$$GET1^DIQ(344.61,"1,",.12,"I")
 I XNEW'=XOLD D LNOUT(.HEAD,.LINE,"AUTO-DECREASE NO-PAY DAYS DEFAULT",XOLD,XNEW,"D",.COUNT)
 ;
...
RXCHNG(HEAD,COUNT,LINE)  ; Check for Rx site parameter changes PRCA*4.5*345
 ...
 Q
 ;
TRICHNG(TRIND,HEAD,COUNT,LINE)  ; Check for TRICARE site parameter changes PRCA*4.5*345
 ;Input, all parameters passed by reference:
 ; TRIND: Original Site parameters from ^TMP($T(+0),$J,344.61,1,1)
 ; HEAD: See LNOUT for details
 ; COUNT: count of parameter changes
 ; LINE: Array of site parameter changes
 N XNEW,XOLD
 ...
 S XOLD=$P(TRIND(0),U,13),XNEW=$$GET1^DIQ(344.61,"1,",.13,"I")
 I XNEW'=XOLD D LNOUT(.HEAD,.LINE,"TRICARE EFT POST PREVENT DAYS",XOLD,XNEW,"D",.COUNT)
 Q
 ;
...
LNOUT(HEAD,LINE,TXT,XOLD,XNEW,TYPE,COUNT)   ; Format a line for the message
 ...
 ;Input:   
 ; HEAD: 0 if header not output into the line array for this section yet, 1 otherwise
 ; HEAD("SIZE"): 10
