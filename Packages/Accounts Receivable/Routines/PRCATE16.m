...
DE S DIE="^PRCA(430,",DIC=DIE,DP=430,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRCA(430,DA,""))=""
 I $D(^(100)) S %Z=^(100) S %=$P(%Z,U,2) S:%]"" DE(1)=%
 I $D(^(104)) S %Z=^(104) S %=$P(%Z,U,1) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCATE16",DQ=1
1 S DW="100;2",DV="RP49'",DU="",DLB="SERVICE",DIFLD=101
 S DU="DIC(49,"
 G RE
X1 Q
2 S DW="104;1",DV="P200'",DU="",DLB="APPROVING OFFICIAL (SERVICE)",DIFLD=111
 S DU="VA(200,"
 G RE
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S $P(^PRCA(430,D0,0),U,12)=$S($D(PRCA("SITE")):PRCA("SITE"),1:"")
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 S $P(^PRCA(430,D0,0),U,4)=$S($D(PRCAGLN):PRCAGLN,1:"")
 Q
