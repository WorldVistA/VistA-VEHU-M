...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCATA21",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;4",DV="FX",DU="",DLB="APPROPRIATION SYMBOL",DIFLD=3
 S DE(DW)="C1^PRCATA21"
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 K:X[""""!($A(X)=45) X I $D(X) D EN1^PRCHPAT
 I $D(X),X'?.ANP K X
 Q
 ;
2 D:$D(DG)>9 F^DIE17 G ^PRCATA22
