...
DE S DIE="^FBAAA(D0,1,",DIC=DIE,DP=161.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^FBAAA(D0,1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(4)=%,DE(13)=% S %=$P(%Z,U,2) S:%]"" DE(19)=% S %=$P(%Z,U,3) S:%]"" DE(24)=% S %=$P(%Z,U,5) S:%]"" DE(26)=% S %=$P(%Z,U,7) S:%]"" DE(39)=% S %=$P(%Z,U,8) S:%]"" DE(51)=%
 I  S %=$P(%Z,U,13) S:%]"" DE(47)=% S %=$P(%Z,U,18) S:%]"" DE(46)=% S %=$P(%Z,U,21) S:%]"" DE(27)=%
...
X2 I FBPRG,FBPRG'=2 W !!,*7,"You are only allowed to edit an outpatient authorization using this option.",!! S Y=""
 Q
3 S DQ=4 ;@1
4 S DW="0;1",DV="DX",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C4F1 S DIEZRXR(161.01,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=394,1183 S DIEZRXR(161.01,DIXR)=""
 Q
...
X12 I FBTODT'>$P(FBAOLD,U,2) S Y="@2"
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW="0;1",DV="DX",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X18 S FBD1=DA
 Q
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW="0;2",DV="RDX",DU="",DIFLD=.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C19F1 S DIEZRXR(161.01,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1183 S DIEZRXR(161.01,DIXR)=""
 Q
...
X23 I $P(^FBAAA(DA(1),1,DA,0),"^",7)]"" K DIE("NO^")
 Q
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW="0;3",DV="RP161.8'",DU="",DIFLD=.03,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C24^FBCTAU1"
 S DU="FBAA(161.8,"
...
X25 S FBTYPE=$S(FBPRG:FBPRG,1:2)
 Q
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,DW="0;5",DV="RP4'",DU="",DIFLD=101,DLB="PRIMARY SERVICE FACILITY"
 S DU="DIC(4,"
 G RE
X26 Q
27 S DW="0;21",DV="*P200'",DU="",DIFLD=104,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="VA(200,"
 G RE
...
X37 S Y="@2"
 Q
38 S DQ=39 ;@4
39 S DW="0;7",DV="R*P161.82'",DU="",DIFLD=.07,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X44 S Y="@4"
 Q
45 S DQ=46 ;@5
46 D:$D(DG)>9 F^DIE17,DE S DQ=46,DW="0;18",DV="S",DU="",DIFLD=.065,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 ...
 G RE
X46 Q
47 S DW="0;13",DV="R*S",DU="",DIFLD=.095,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X50 K DIE("NO^")
 Q
51 D:$D(DG)>9 F^DIE17,DE S DQ=51,DW="0;8",DV="F",DU="",DIFLD=.08,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
X52 S:X="" Y=.021
 Q
53 S DW="3;1",DV="F",DU="",DIFLD=.085,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
X54 S:X="" Y=.021
 Q
55 S DW="3;2",DV="F",DU="",DIFLD=.086,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X55 K:$L(X)>60!($L(X)<2) X
 I $D(X),X'?.ANP K X
 Q
 ;
56 D:$D(DG)>9 F^DIE17 G ^FBCTAU2
