ONCOX4 ; GENERATED FROM 'ONCOX4' PRINT TEMPLATE (#1275) ; 01/21/97 ; (FILE 165.5, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1275,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
