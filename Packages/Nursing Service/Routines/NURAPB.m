NURAPB ; GENERATED FROM 'NURA-P-EXCEPTION' PRINT TEMPLATE (#1206) ; 08/27/97 ; (FILE 213.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1206,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
