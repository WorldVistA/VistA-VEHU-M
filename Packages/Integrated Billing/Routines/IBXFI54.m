...
DE S DIE="^IBE(357.5,D0,2,",DIC=DIE,DP=357.52,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.5,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(12)=% S %=$P(%Z,U,8) S:%]"" DE(10)=% S %=$P(%Z,U,9) S:%]"" DE(1)=%,DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="IBXFI54",DQ=1
1 S DW="0;9",DV="NJ1,0X",DU="",DLB="DATA",DIFLD=.09
 S X=1
 S Y=X
...
X5 D HELP1^IBDFU5
 Q
6 S DW="0;9",DV="NJ1,0X",DU="",DLB="Select Subfield's Data",DIFLD=.09
 G RE
X6 K:+X'=X!(X>7)!(X<1)!(X?.E1"."1N.N)!($P($G(^IBE(357.6,+$P($G(^IBE(357.5,DA(1),0)),U,3),2)),U,(2*X)-1)="") X
 Q
 ;
...
X8 S IBP=X
 Q
9 S DQ=10 ;@9
10 S DW="0;8",DV="NJ3,0",DU="",DLB="LENGTH OF DATA",DIFLD=.08
 S X=$G(IBLEN(IBP))
 S Y=X
 G Y
X10 K:+X'=X!(X>150)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 S IBW=+X
 Q
12 S DW="0;6",DV="NJ3,0XO",DU="",DLB="STARTING ROW FOR DATA",DIFLD=.06
 S DQ(12,2)="S Y(0)=Y S:Y=+Y Y=Y+1"
 S X=+IBY
 S Y=X
 G Y
X12 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
13 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=13 D X13 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X13 S IBY=X+1
 Q
14 D:$D(DG)>9 F^DIE17 G ^IBXFI55
