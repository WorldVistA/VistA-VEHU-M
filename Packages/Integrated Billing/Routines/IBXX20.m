IBXX20 ; COMPILED XREF FOR FILE #399 ; 05/22/20
 ; 
 S DIKZK=1
 ...
 I X'="" D
 .N DIK,DIV,DIU,DIN
 .K DIV S DIV=X,D0=DA,DIV(0)=D0 S Y(0)=X S X=Y(0),X=X S X=X=9 I X S X=DIV S Y(1)=$S($D(^DGCR(399,D0,500001)):^(500001),1:"") S X=$P(Y(1),U,1),X=X S DIU=X K Y S X=DIV S X=$$ADDCODE^ZZZZ(DA) X ^DD(399,.07,1,1000,1.4)
 S DIKZ(0)=$G(^DGCR(399,DA,0))
 S X=$P($G(DIKZ(0)),U,8)
 I X'="" D
 .N DIK,DIV,DIU,DIN
