IB20P808 ;MNTVBB/RFS - POST-INIT FOR IB*2.0*808 ; DEC 2, 2024@13:00
...
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P808")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P808")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P808")
 Q
 ;
THRESH ; Pension Threshold
 ...
 S IBA(354.3,"+1,",.03)="16695" ;  base rate for veteran
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*808 CY 2025 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*808. If you have received this"
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*808 patch description for additional information."
