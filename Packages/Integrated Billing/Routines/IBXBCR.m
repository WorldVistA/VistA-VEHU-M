IBXBCR ; GENERATED FROM 'IB BILLING CLOCK INQ' PRINT TEMPLATE (#1214) ; 05/23/94 ; (FILE 351, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1214,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
