...
DE S DIE="^DGCR(399,D0,""RC"",",DIC=DIE,DP=399.042,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"RC",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(2)=% S %=$P(%Z,U,9) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="IBXSC626",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;9",DV="NJ8,2",DU="",DLB="NON-COVERED CHARGE",DIFLD=.09
 G RE
X1 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>99999)!(X<0) X
 Q
 ;
2 S DW="0;6",DV="P81'",DU="",DLB="PROCEDURE",DIFLD=.06
 S DE(DW)="C2^IBXSC626"
 S DU="ICPT("
 G RE
...
X3 I '$P(^DGCR(399,DA(1),"RC",DA,0),U,6) S Y="@658"
 Q
4 D:$D(DG)>9 F^DIE17 G ^IBXSC627
