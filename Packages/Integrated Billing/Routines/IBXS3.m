IBXS3 ; GENERATED FROM 'IB SCREEN3' INPUT TEMPLATE(#1250), FILE 399;09/15/16
 D DE G BEGIN
...
BEGIN S DNM="IBXS3",DQ=1
 ...
 M DIEZAR=^DIE(1250,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=1250,U="^"
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C7F1 N X,X1,X2 S DIXR=918 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 K X M X=X2 D
...
C7X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C7F2 S DIXR=919 D C7X2(U) K X2 M X2=X D C7X2("O") K X1 M X1=X
 K X M X=X2 D
...
C11S S X="" G:DG(DQ)=X C11F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C11F1 N X,X1,X2 S DIXR=918 D C11X1(U) K X2 M X2=X D C11X1("O") K X1 M X1=X
 K X M X=X2 D
 . N IBLNPRV I $$LNPRVFT^IBCEU7(X,.IBLNPRV) D FILE^DIE("E","IBLNPRV") Q
 G C11F2
C11X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C11F2 S DIXR=919 D C11X2(U) K X2 M X2=X D C11X2("O") K X1 M X1=X
 K X M X=X2 D
 . N DIEXARR M DIEXARR=X S DIEZCOND=1
 . S X=X(1)=3
 . S DIEZCOND=$G(X) K X M X=DIEXARR Q:'DIEZCOND
...
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="M;12",DV="FXO",DU="",DLB="PRIMARY INSURANCE POLICY",DIFLD=112
 S DQ(22,2)="S Y(0)=Y S Y=$$TRANS^IBCNS2(DA,Y)"
...
X27 K:+X'=X!(X>9999999999)!(X<1000000000)!(X?.E1"."1N.N)!$$HOD^IBCNHUT1(X) X
 Q
 ;
28 D:$D(DG)>9 F^DIE17,DE S DQ=28,DW="M;13",DV="FOX",DU="",DLB="SECONDARY INSURANCE POLICY",DIFLD=113
 S DQ(28,2)="S Y(0)=Y S Y=$$TRANS^IBCNS2(DA,Y)"
...
C28F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=190 S DIEZRXR(399,DIXR)=""
 Q
X28 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>4!($L(X)<1) X D:$D(X) DD^IBCNS2(X,DA,1)
 I $D(X),X'?.ANP K X
 Q
 ;
