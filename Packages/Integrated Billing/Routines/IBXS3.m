IBXS3 ; GENERATED FROM 'IB SCREEN3' INPUT TEMPLATE(#1250), FILE 399;09/25/15
 D DE G BEGIN
...
33 S DW="M;14",DV="FOX",DU="",DLB="TERTIARY INSURANCE POLICY",DIFLD=114
 S DQ(33,2)="S Y(0)=Y S Y=$$TRANS^IBCNS2(DA,Y)"
...
C33F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=190 S DIEZRXR(399,DIXR)=""
 Q
X33 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>4!($L(X)<1) X D:$D(X) DD^IBCNS2(X,DA,1)
 I $D(X),X'?.ANP K X
 Q
 ;
