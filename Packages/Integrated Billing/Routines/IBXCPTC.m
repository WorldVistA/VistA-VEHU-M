IBXCPTC ; GENERATED FROM 'IB CPT CP DISPLAY' PRINT TEMPLATE (#1209) ; 05/23/94 ; (FILE 350.71, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1209,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
