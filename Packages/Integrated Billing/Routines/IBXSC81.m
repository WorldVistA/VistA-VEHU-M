...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("U")) S %Z=^("U") S %=$P(%Z,U,13) S:%]"" DE(7)=%
 I $D(^("U1")) S %Z=^("U1") S %=$P(%Z,U,13) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="IBXSC81",DQ=1
1 S DW="U1;13",DV="F",DU="",DLB="FORM LOCATOR 92",DIFLD=213
 G RE
X1 K:$L(X)>32!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
...
X3 S:IBDR20'["87" Y="@88"
 Q
4 S DW="U1;14",DV="F",DU="",DLB="FORM LOCATOR 93",DIFLD=214
 G RE
X4 K:$L(X)>32!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
...
X6 S:IBDR20'["88" Y="@89"
 Q
7 S DW="U;13",DV="F",DU="",DLB="TREATMENT AUTHORIZATION CODE",DIFLD=163
 G RE
X7 K:$L(X)>18!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
