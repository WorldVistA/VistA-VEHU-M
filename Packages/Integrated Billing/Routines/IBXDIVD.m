IBXDIVD ; GENERATED FROM 'IB DIVISION DISPLAY' PRINT TEMPLATE (#1211) ; 05/23/94 ; (FILE 40.8, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1211,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
