...
X2 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".13;1",DV="FaXJ17",DU="",DIFLD=.131,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^IBXS111"
 G RE
...
C3F1 Q
X3 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
4 S DQ=5 ;@155
...
X6 S:X="N" Y="@915" S:X="Y" DIE("NO^")=""
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".121;7",DV="RDX",DU="",DIFLD=.1217,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X17 K:X[""""!($A(X)=45) X I $D(X) S DFN=DA D TAD^DGLOCK I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".121;10",DV="FXJ17",DU="",DIFLD=.1219,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C18^IBXS111"
 G RE
...
C18S S X="" G:DG(DQ)=X C18F1 K DB
 ...
 D EVENT^IVMPLOG(DA,1)
C18F1 Q
X18 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X I $D(X) S DFN=DA D TAD^DGLOCK
 I $D(X),X'?.ANP K X
 Q
 ;
19 S DQ=20 ;@915
