IBXCPTG ; GENERATED FROM 'IB CPT PG DISPLAY' PRINT TEMPLATE (#1208) ; 05/23/94 ; (FILE 350.7, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1208,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
