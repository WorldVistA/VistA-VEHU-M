IBXS6 ; GENERATED FROM 'IB SCREEN6' INPUT TEMPLATE(#1253), FILE 399;09/15/16
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("U")) S %Z=^("U") S %=$P(%Z,U,5) S:%]"" DE(32)=% S %=$P(%Z,U,6) S:%]"" DE(35)=% S %=$P(%Z,U,7) S:%]"" DE(34)=%
...
BEGIN S DNM="IBXS6",DQ=1
 ...
 M DIEZAR=^DIE(1253,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=1253,U="^"
...
C17F1 N X,X1,X2 S DIXR=924 D C17X1(U) K X2 M X2=X D C17X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . D CMAEDALL^IBCU9(DA)
...
C22S S X="" G:DG(DQ)=X C22F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C22F1 N X,X1,X2 S DIXR=918 D C22X1(U) K X2 M X2=X D C22X1("O") K X1 M X1=X
 K X M X=X2 D
...
C22X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C22F2 S DIXR=919 D C22X2(U) K X2 M X2=X D C22X2("O") K X1 M X1=X
 K X M X=X2 D
...
C26S S X="" G:DG(DQ)=X C26F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C26F1 N X,X1,X2 S DIXR=918 D C26X1(U) K X2 M X2=X D C26X1("O") K X1 M X1=X
 K X M X=X2 D
 . N IBLNPRV I $$LNPRVFT^IBCEU7(X,.IBLNPRV) D FILE^DIE("E","IBLNPRV") Q
 G C26F2
C26X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C26F2 S DIXR=919 D C26X2(U) K X2 M X2=X D C26X2("O") K X1 M X1=X
 K X M X=X2 D
 . N DIEXARR M DIEXARR=X S DIEZCOND=1
 . S X=X(1)=3
 . S DIEZCOND=$G(X) K X M X=DIEXARR Q:'DIEZCOND
