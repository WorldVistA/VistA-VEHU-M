IB20P780 ;MNTVBB/RXD - POST-INIT FOR IB*2.0*780 ; Nov 10, 2023@14:00
...
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P780")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P780")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P780")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK,IBADLDEP
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2024 Pension Threshold rates.")
 S IBX=3231200 ;set IBX so that it will pick up all record on or after the new effective date
 ...
 S IBA(354.3,"+1,",.01)=3231201 ; effective date for CY 2024 values
 S IBA(354.3,"+1,",.02)=1 ;     internal value 1 = BASIC PENSION
 S IBA(354.3,"+1,",.03)="16551" ;  base rate for veteran
 S IBA(354.3,"+1,",.04)="21674" ; 1 dependent
 S IBADLDEP="2831" ;  additional dependent amount
 ...
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2024
 I $D(IBERRM) D
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3221201))  ; quit if the "APRIOR" x-ref is not set for 12/1/22.
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*780 CY 2024 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*780. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2024 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*780 patch description for additional information."
