IBXSC7H1 ; ;08/24/92
 D DE G BEGIN
DE S DIE="^DGCR(399,D0,""OP"",",DIC=DIE,DP=399.043,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^DGCR(399,D0,"OP",DA,-1))<0
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="IBXSC7H1",DQ=1+D G B
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 S DGRVRCAL=1
 Q
X1 S %DT="E" D ^%DT S X=Y K:Y<1!('$D(DGCRIFN)) X I $D(X) D DDAT2^DGCRU4 I $D(X) S DINUM=X I $D(^DGCR(399,+DGCRIFN,"OP",0)),$P(^(0),"^",4)>29 W !?4,*7,"Maximum of 30 visit dates allowed per bill!" K X,DINUM
 Q
 ;
2 G 1^DIE17
