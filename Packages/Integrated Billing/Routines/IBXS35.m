...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="M1;14",DV="NJ10,0X",DU="",DLB="SECONDARY INSURANCE HPID",DIFLD=472
 S DE(DW)="C1^IBXS35"
 S X=IBHPID
 S Y=X
 G Y
...
X1 K:+X'=X!(X>9999999999)!(X<1000000000)!(X?.E1"."1N.N)!$$HOD^IBCNHUT1(X) X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="M;14",DV="FOX",DU="",DLB="TERTIARY INSURANCE POLICY",DIFLD=114
 S DQ(2,2)="S Y(0)=Y S Y=$$TRANS^IBCNS2(DA,Y)"
 S DE(DW)="C2^IBXS35",DE(DW,"INDEX")=1
 G RE
...
C2F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=190 S DIEZRXR(399,DIXR)=""
 Q
X2 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>4!($L(X)<1) X D:$D(X) DD^IBCNS2(X,DA,1)
 I $D(X),X'?.ANP K X
 Q
 ;
...
X5 S IBHPID=$$HPD^IBCNHUT1(IBINS)
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="M1;15",DV="NJ10,0X",DU="",DLB="TERTIARY INSURANCE HPID",DIFLD=473
 S DE(DW)="C6^IBXS35"
 S X=IBHPID
 S Y=X
 G Y
...
X17 I '$$SUPPPT^IBCEP7B(DA,1) S Y="@3212"
 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C18^IBXS35"
 S Y="@"
...
X18 K:$L(X)>13!($L(X)<3)!($TR(X," ")="") X
 ...
 Q
 ;
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X28 S IBPSDAT=$$GETID^IBCEP7B(DA,1),IBPSID=$P(IBPSDAT,U),IBPSQUAL=$P(IBPSDAT,U,2) I IBPSID="" S Y="@321"
 Q
29 S DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C29^IBXS35"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X29 K:$L(X)>13!($L(X)<3)!($TR(X," ")="") X
 ...
 Q
 ;
30 D:$D(DG)>9 F^DIE17,DE S DQ=30,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
35 S DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C35^IBXS35"
...
39 D:$D(DG)>9 F^DIE17,DE S DQ=39,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
...
X40 S Y="@3215"
 Q
41 S DQ=42 ;@3214
42 S DW="M1;10",DV="*P355.97'",DU="",DLB="Primary ID Qualifier",DIFLD=128
 S DU="IBE(355.97,"
 G RE
