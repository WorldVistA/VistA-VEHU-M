...
DE S DIE="^IBE(357.5,",DIC=DIE,DP=357.5,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^IBE(357.5,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,13) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="IBXFI52",DQ=1
1 S DW="0;13",DV="SR",DU="",DLB="HOW SHOULD THE TEXT LINES BE SPACED?",DIFLD=.13
 S DU="1:SINGLE SPACED;2:DOUBLE SPACED;3:SINGLE, BUT DOUBLE IF BLANK;"
 G RE
X1 Q
2 S DW="0;14",DV="NJ3,0R",DU="",DLB="HOW MANY CHARACTERS ACROSS SHOULD THE TEXT LINES BE?",DIFLD=.14
 G RE
X2 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
...
X7 S (IBY,IBX)=1
 Q
8 S D=0 K DE(1) ;2
 S DIFLD=2,DGO="^IBXFI53",DC="8^357.52I^2^",DV="357.52MF",DW="0;1",DOW="SUBFIELD LABEL",DLB="Select "_DOW S:D DC=DC_D
 G RE:D I $D(DSC(357.52))#2,$P(DSC(357.52),"I $D(^UTILITY(",1)="" X DSC(357.52) S D=$O(^(0)) S:D="" D=-1 G M8
 S D=$S($D(^IBE(357.5,DA,2,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M8 I D>0 S DC=DC_D I $D(^IBE(357.5,DA,2,+D,0)) S DE(8)=$P(^(0),U,1)
 G RE
R8 D DE
 S D=$S($D(^IBE(357.5,DA,2,0)):$P(^(0),U,3,4),1:1) G 8+1
 ;
9 S DQ=10 ;@99
