...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGCR(399,DA,-1))<0
 I $D(^("U")) S %Z=^("U") S %=$P(%Z,U,1) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,6) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="IBXSC7H2",DQ=1
1 S DW="U;5",DV="RFOX",DU="",DLB="IS THIS A SENSITIVE RECORD?",DIFLD=155
...
4 S DW="U;6",DV="RFOX",DU="",DLB="ASSIGNMENT OF BENEFITS",DIFLD=156
 ...
 G RE
X4 I $D(X) D YN^DGCRU I $D(X) X:X=0 ^DD(399,156,9.3) K DGCRRATY
 I $D(X),X'?.ANP K X
 Q
 ;
...
C9S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 S DGRVRCAL=1
 Q
X9 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT^DGCRU4 K DGCR00
 Q
 ;
10 D:$D(DG)>9 F^DIE17 G ^IBXSC7H3
