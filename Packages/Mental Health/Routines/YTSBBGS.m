YTSBBGS ;NTX/LGM - Score BBGS ; 3/25/22 2:02pm
...
SCORESV ; calculate the score
 ...
 N I,J,CHOICE,QUES,VAL,SCORES,TOT ; Removed SKPCNT (all questions are required)
 S TOT=0,VAL=0
 S I=2 F  S I=$O(YSDATA(I)) Q:'I  D
 . S VAL=""
 . S QUES=$P(YSDATA(I),U)
 . S CHOICE=$P(YSDATA(I),U,3)
 . I CHOICE="5761" S VAL=0
 . I CHOICE="5760" S VAL=1
 . S TOT=TOT+VAL
 ;
 ...
 . S J=J+1
 . I SCLID=1553 S ^TMP($J,"YSCOR",J)=SCLNM_"="_TOT
 Q
REPORT(SCORES) ; build the scoring display for the report
 ; expects ^TMP($J,"YSCOR",...) and ^TMP($J,"YSG") from DLLSTR
 ;         YSDATA from DLLSTR
 N I,VALUE
 S SCORES="BBGS screen is NEGATIVE for gambling-related problems."
 S I=1 F  S I=$O(^TMP($J,"YSCOR",I)) Q:'I  D
 . S VALUE=$P(^TMP($J,"YSCOR",I),"=",2)
 I VALUE>0 S SCORES="BBGS screen is POSITIVE indicating potential gambling-related| problems and additional evaluation is warranted."
 Q
