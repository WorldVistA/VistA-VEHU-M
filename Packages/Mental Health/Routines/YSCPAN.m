YSCPAN ; GENERATED FROM 'YSSR REVIEW ACTION HEADER' PRINT TEMPLATE (#916) ; 03/10/95 ; (FILE 615.2, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(916,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
