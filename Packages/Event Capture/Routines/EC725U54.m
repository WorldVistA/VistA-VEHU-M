EC725U54 ;ALB/GTS/JAP/GT - EC National Procedure Update; 12/16/2008
 ;;2.0; EVENT CAPTURE ;**99**;8 May 96;Build 2
 ;
 ;this routine is used as a post-init in a KIDS build 
 ;to modify the EC National Procedure file #725
 ;
ADDPROC ;* add national procedures
 ;
 ;  ECXX is in format:
 ;   NAME^NATIONAL NUMBER^CPT CODE^FIRST NATIONAL NUMBER SEQUENCE
 ;   LAST NATIONAL NUMBER SEQUENCE
 ;
 N ECX,ECXX,ECDINUM,NAME,CODE,CPT,COUNT,X,Y,DIC,DIE,DA,DR,DLAYGO,DINUM
 N ECADD,ECBEG,ECEND,CODX,NAMX,ECSEQ,LIEN,STR,CPTN,STR
 D MES^XPDUTL(" ")
 D BMES^XPDUTL("Adding new procedures to EC NATIONAL PROCEDURE File (#725)...")
 D MES^XPDUTL(" ")
 S ECDINUM=$O(^EC(725,9999),-1),COUNT=$P(^EC(725,0),U,4)
 F ECX=1:1 S ECXX=$P($T(NEW+ECX),";;",2) Q:ECXX="QUIT"  D
 .S NAME=$P(ECXX,U,1),CODE=$P(ECXX,U,2),CPTN=$P(ECXX,U,3),CODX=CODE
 .S CPT=""
 .I CPTN'="" S CPT=$$FIND1^DIC(81,"","X",CPTN) I +CPT<1 D  Q
 ..S STR="   CPT code "_CPTN_" not a valid code in CPT File."
 ..D MES^XPDUTL(" ")
 ..D BMES^XPDUTL("   ["_CODE_"] "_STR)
 .S ECBEG=$P(ECXX,U,4),ECEND=$P(ECXX,U,5),NAMX=NAME
 .I ECBEG="" S X=NAME D FILPROC Q
 .F ECSEQ=ECBEG:1:ECEND D
 ..S ECADD="000"_ECSEQ,ECADD=$E(ECADD,$L(ECADD)-2,$L(ECADD))
 ..;S NAME=NAMX_ECADD,X=NAME,CODE=CODX_ECADD
 ..I $E(CODX,1,3)'="RCM" S NAME=NAMX_ECSEQ,X=NAME,CODE=CODX_ECADD
 ..E  S NAME=NAMX_$E(ECADD,2,99),X=NAME,CODE=CODX_$E(ECADD,2,99)
 ..D FILPROC
 S $P(^EC(725,0),U,4)=COUNT,X=$O(^EC(725,999999),-1),$P(^EC(725,0),U,3)=X
 Q
 ;
FILPROC ;File national procedures
 I '$D(^EC(725,"D",CODE)) D
 .S ECDINUM=ECDINUM+1,DINUM=ECDINUM,DIC(0)="L",DLAYGO=725,DIC="^EC(725,"
 .S DIC("DR")="1////^S X=CODE;4////^S X=CPT"
 .D FILE^DICN
 .I +Y>0 D
 ..S COUNT=COUNT+1
 ..D MES^XPDUTL(" ")
 ..S STR="   Entry #"_+Y_" for "_$P(Y,U,2)
 ..S STR=STR_$S(CPT'="":" [CPT: "_CPT_"]",1:"")_" ("_CODE_")"
 ..D BMES^XPDUTL(STR_"  ...successfully added.")
 .I Y=-1 D
 ..D MES^XPDUTL(" ")
 ..D BMES^XPDUTL("ERROR when attempting to add "_NAME_" ("_CODE_")")
 I $D(^EC(725,"DL",CODE)) D
 .S LIEN=$O(^EC(725,"DL",CODE,""))
 .D MES^XPDUTL(" ")
 .D BMES^XPDUTL("   Your site has a local procedure (entry #"_LIEN_") in File #725")
 .D BMES^XPDUTL("   which uses "_CODE_" as its National Number.")
 .D BMES^XPDUTL("   Please inactivate this local procedure.")
 .K Y
 Q
NEW ;national procedures to add;;descript^nation #^CPT code^beg seq^end seq
 ;;TEAM W/O FAM/PT V30M^SW093^99368
 ;;WARD ROUNDS V15M^SW094^99499
 ;;DIALYSIS SUPPORT V15M^SW095^99499
 ;;GUARDIANSHIP V15M^SW096^99499
 ;;HOME HEALTH V15M^SW097^99499
 ;;HOMEMAKER HHA V15M^SW098^99499
 ;;HOSPICE IN-HOME V15M^SW099^99499
 ;;HOSPICE IP V15M^SW100^99499
 ;;HBPC V15M^SW101^99499
 ;;LODGING V15M^SW102^99499
 ;;FISHER HOUSE V15M^SW103^99499
 ;;MEDICAID V15M^SW104^99499
 ;;TRANSPORTATION V15M^SW105^99499
 ;;VA BENEFITS V15M^SW106^99499
 ;;SA SCREEN/INTER V15M^SW107^99408
 ;;HOMELESS REF V15M^SW108^99499
 ;;REFERRALS/OTHER V15M^SW109^99499
 ;;MEDICAID NH V15M^SW110^99499
 ;;MEDICARE NH V15M^SW111^99499
 ;;VA CONTRACT NH V15M^SW112^99499
 ;;PRIVATE PAY NH V15M^SW113^99499
 ;;STATE VETS HOME V15M^SW114^99499
 ;;COM LIVING CTR V15M^SW115^99499
 ;;HOSPICE RES V15M^SW116^99499
 ;;ASSIS LIV FAC V15M^SW117^99499
 ;;CRC V15M^SW118^99499
 ;;TRANSPLANT V15M^SW119^99499
 ;;SUICIDE RISK V15M^SW120^99499
 ;;SPECIAL ASSES V15M^SW121^99499
 ;;AD DIRECTIVES V15M^SW122^S0257
 ;;COMMUN RESOURCE V15M^SW123^99499
 ;;DISCHCONF FAM/SOV15M^SW124^99499
 ;;FINANCIAL V15M^SW125^99499
 ;;COPING WITH ILL V15M^SW126^99499
 ;;END OF LIFE V15M^SW127^S0257
 ;;SA COUNSELING V15M^SW128^99499
 ;;INDIV  COUNSEL V15M^SW129^99499
 ;;FAMCOUNSEL WPT V15M^SW130^99499
 ;;FAM COUNSEL W/O PT V15M^SW131^99499
 ;;FAM SUPPORT GP V15M^SW132^99499
 ;;FAM BEREAVEMENT V15M^SW133^99499
 ;;MARITAL COUNSEL V15M^SW134^99499
 ;;MDS ASSESS, V 15M^SW135^99499
 ;;QUIT
NAMECHG ;* change national procedure names
 ;
 ;  ECXX is in format:
 ;   NATIONAL NUMBER^NEW NAME
 ;
 N ECX,ECXX,ECDA,DA,DR,DIC,DIE,X,Y,STR
 D MES^XPDUTL(" ")
 D BMES^XPDUTL("Changing names in EC NATIONAL PROCEDURE File (#725)...")
 D MES^XPDUTL(" ")
 F ECX=1:1 S ECXX=$P($T(CHNG+ECX),";;",2) Q:ECXX="QUIT"  D
 .I $D(^EC(725,"D",$P(ECXX,U,1))) D
 ..S ECDA=+$O(^EC(725,"D",$P(ECXX,U,1),0))
 ..I $D(^EC(725,ECDA,0)) D
 ...S DA=ECDA,DR=".01////^S X=$P(ECXX,U,2)",DIE="^EC(725," D ^DIE
 ...D MES^XPDUTL(" ")
 ...D MES^XPDUTL("   Entry #"_ECDA_" for "_$P(ECXX,U,1))
 ...D BMES^XPDUTL("      ... field (#.01) updated to  "_$P(ECXX,U,2)_".")
 .I '$D(^EC(725,"D",$P(ECXX,U,1))) D
 ..D MES^XPDUTL(" ")
 ..S STR="Can't find entry for "_$P(ECXX,U,1)
 ..D BMES^XPDUTL(STR_" ...field (#.01) not updated.")
 Q
 ;
CHNG ;name changes -national code #^new procedure name
 ;;RC009^99368 TEAMEETCAREPLAN 30M
 ;;RC010^99366 TEAMEET CP F2F 30M
 ;;RC011^98961 IDT GRP 2-4 30M
 ;;RC056^97533 MUSTHER EMOTIND 15M
 ;;RC081^S9445 LEIS EDUC IND 15M
 ;;SW001^CASE MANG IP/OP V15M
 ;;SW002^CONSULT IP V15M
 ;;SW003^DISCHPLAN IP/OP V15M
 ;;SW004^FAMPSYTHERW/OPT V15M
 ;;SW006^PREV COUNSEL V15M
 ;;SW009^TEAM F2F FAM/PT V30M
 ;;SW010^PHONE CONTACT, OP, 5-10M
 ;;SW011^CNH FOLLOWUP OP V15M
 ;;SW012^PHONDCNONMHFU11-20M
 ;;SW013^OFFICE VISIT 5M
 ;;SW014^PSYCHSOC ASSES V15M
 ;;SW019^GP 2-5 COUNSEL V30M
 ;;SW020^GPPSYTHERAPY2-5 V30M
 ;;SW025^GP 6-8 COUNSEL V30M
 ;;SW026^GP 9+ COUNSEL V30M
 ;;SW027^GPPSYTHERAPY6-8 V30M
 ;;SW028^GPPSYTHERAPY 9+ V30M
 ;;SW031^INELIGIBLE-VET V15M
 ;;SW032^INELIGNON-VET V15M
 ;;SW035^FAMPSYTHER W/PT V15M
 ;;SW036^MUTIFAMGPPSYTHERV15M
 ;;SW044^PHONE CONTACT 11-20M
 ;;SW045^PHONE CONTACT 21-30M
 ;;SW046^CNH FOLLOW UP 30 MIN
 ;;SW047^CNH FOLLOW UP 45 MIN
 ;;SW048^CRC FOLLOWUP OP V15M
 ;;SW054^PHONDCNONMHFU21-30M
 ;;SW056^OFFICE VISIT 10M
 ;;SW057^OFFICE VISIT 15M
 ;;SW058^OFFICE VISIT 25M
 ;;SW059^OFFICE VISIT 40M
 ;;SW060^COMP/PENS EXAM V15M
 ;;SW061^PSYTHERINDIV V20M
 ;;SW063^UNLIST PSY SERV V15M
 ;;SW072^IND FUNCTIONAL IMPVMT@15M
 ;;SW073^SELFCARE OP V15M
 ;;SW074^COMWORKREIN OP V15M
 ;;SW075^HIGH RISK ASSESSMENT V15M
 ;;SW076^HOME VISIT V15M
 ;;SW077^PSYCHSOC ASSESS OPT 60M
 ;;SW081^PSYCHTHER INDIV V20M
 ;;SW083^PREVCNSEL ED REF G1-5 60M
 ;;SW084^PREVCNSEL ED REF G6-8 60M
 ;;SW085^PREVCNSEL ED REF GP9+ 60M
 ;;SW087^HOMEHEALTHF/U V15M
 ;;SW088^NONEMERTRANSPORV15M
 ;;SW089^PHONED/CMHF/U 11-20M
 ;;SW090^PSYCH INTER V15M
 ;;QUIT
