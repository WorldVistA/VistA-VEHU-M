ECPROV5 ;BIR/MAM,JPW-Event Capture Provider Summary (cont'd) ;7 May 96
 ;;2.0; EVENT CAPTURE ;**5,8,18**;8 May 96
 ; This routine is used when printing the report for
 ; a specific DSS Unit.
 S %H=$H D YX^%DTC S ECRDT=Y
 I ECL S ECC=+$P(^ECD(ECD,0),U,11) D LOC,PRINT Q
 S ECL=0 F I=0:0 S ECL=$O(^ECH("ADT",ECL)) Q:'ECL  D
 .S ECLN=$P(^DIC(4,ECL,0),"^")
 .S ECC=+$P(^ECD(ECD,0),U,11) D LOC
PRINT S (ECLN,ECPN,ECUN)=0,ECCN=""
 F I=0:0 S ECLN=$O(^TMP($J,ECLN)) Q:ECLN=""!(ECOUT)!(ECLN["^")  D
 .D PAGE S ECPG=1,ECUN=""
 .D NOUNIT
 .F I=0:0 S ECUN=$O(^TMP($J,ECLN,ECUN)) Q:ECUN=""!(ECOUT)  D
 ..;Naked reference below refers to ^TMP( global above.
 ..S ECINZ="^"_$O(^(ECUN,0))
 ..D CATS
 Q
CATS ; continue looping
 I $O(^TMP($J,ECLN,ECUN,""))']"" D PAGE W !!!,?12,"NO PROCEDURES",! S ECPG=1 Q
 I $Y+10>IOSL D PAGE I ECOUT Q
 W !!,ECUN S ECCN=0 F I=0:0 S ECCN=$O(^TMP($J,ECINZ,ECCN)) D:ECCN="" TOTP Q:ECCN=""!(ECOUT)  D:$Y+7>IOSL PAGE Q:ECOUT  D PRO
 Q
 ;
 ;ALB/ESD - Loop through to get procedure reason and print
PRO W !,?3,ECCN S ECPN=0,ECPRSN=""
 F  S ECPN=$O(^TMP($J,ECINZ,ECCN,ECPN)) Q:ECPN=""!(ECOUT)  S ECAT=1 D:$Y+7>IOSL PAGE S ECAT=0 Q:ECOUT  F  S ECPRSN=$O(^TMP($J,ECINZ,ECCN,ECPN,ECPRSN)) Q:ECPRSN=""!(ECOUT)  DO
 ...
 Q
LOC S (ECDFN,ECNOPE,ECOUT,^TMP($J,ECLN))=0
 F I=0:0 S ECDFN=$O(^ECH("ADT",ECL,ECDFN)) Q:'ECDFN  S MM=ECSD F I=0:0 S MM=$O(^ECH("ADT",ECL,ECDFN,ECD,MM)) Q:'MM!(MM>ECED)  S ECFN=0 F I=0:0 S ECFN=$O(^ECH("ADT",ECL,ECDFN,ECD,MM,ECFN)) Q:'ECFN  D UTL
 Q
UTL ; set ^TMP($J,"ECPROV"
 ...
 I '$D(^TMP($J,ECLN,ECUN)) S ECINC=ECINC+1,ECINZ="^"_ECINC,^(ECUN)=0,^(ECUN,ECINC)=0
 S ECINZ="^"_$O(^TMP($J,ECLN,ECUN,0))
 ...
 S ^TMP($J,ECLN,ECUN)=^TMP($J,ECLN,ECUN)+ECV
...
HDR ; print heading
 ...
 Q
TOTP Q:ECOUT  W !,?105,"------",!,"Total Procedures for "_ECUN,?105,$J(^TMP($J,ECLN,ECUN),6)
 Q
NOUNIT ;No procedures
 I $O(^TMP($J,ECLN,ECUN))']"" W !!!,?12,"NO PROCEDURES",! S ECPG=1
 Q
