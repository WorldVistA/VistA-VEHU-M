LRBLDPK ; GENERATED FROM 'LRBL DONOR TESTING SUPPLEMENT' PRINT TEMPLATE (#948) ; 09/30/96 ; (FILE 65.5, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(948,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
