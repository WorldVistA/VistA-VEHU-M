LRBLDPT ; GENERATED FROM 'LRBL DONOR TESTING REPORT' PRINT TEMPLATE (#943) ; 03/01/96 ; (FILE 65.5, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(943,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
