LR571 ;SLC/PBB - LR*5.2*571 PATCH ENVIRONMENT CHECK ROUTINE ;01/21/06
 ;;5.2;LAB SERVICE;**571**;Sep 27, 1994;Build 2
 ;;
EN ; Does not prevent loading of the transport global.
 ...
 Q
 ;
PRE ; KIDS Pre install for LR*5.2*571
 ;
 N XQA,XQAMSG
 ;K ^XTEMP($J,"LR571") ;Keep field 80 identifier
 ...
 Q
 ;
POST ; KIDS Post install for LR*5.2*571
 ;
 N XQA,XQAMSG
...
NLT1 ;
 ...
 S ^DD(95.3099,.01,9)="^"
 S ^DD(95.3,0,"VRRV")=2.77 ;LAB LOINC
 S ^DD(95.31,0,"VRRV")=2.77 ;LAB LOINC COMPONENT
 S ^DD(64,0,"VRRV")=2.77 ;WKLD CODE
 S ^DD(64.061,0,"VRRV")=2.77 ;LAB ELECTRONIC CODES 
 S ^DD(64.2,0,"VRRV")=2.77 ;WKLD SUFFIX CODES
 S ^DD(64.21,0,"VRRV")=2.77 ;WKLD CODE LAB SECT
 ;S ^DD(64.22,0,"VRRV")=2.77
 ;S ^DD(64.3,0,"VRRV")=2.77
 ;S Y=$$NOW^XLFDT\1 D DD^%DT S Y="2.52 ;;5.2;LAB SERVICE;**571**;"_Y_";Build 01"
 ;S Y="5.2;LAB SERVICE;**571**;MAY 05, 2016;Build 1"
 S Y="2.77;;5.2;LAB SERVICE;**571**;Sep 27, 1994"
 S ^LAB(95.3,"VR")=Y
 S ^LAB(95.31,"VR")=Y
 S ^LAM("VR")=Y
 S ^LAB(64.061,"VR")=Y
...
REINDEX ;Reindex LAM to fire new x-refs
 ...
 . S $P(^LAM(0),U,3)=99999
 K ^XTMP("LRNLTERR","LR571") S ^XTMP("LRNLTERR",0)=$$HTFM^XLFDT($H+90,1)_"^"_DT_"^LR52 571 Error Messages"
 K ^XTMP("LRNLT","LR571")
 S ^XTMP("LRNLT",0)=$$HTFM^XLFDT($H+90,1)_"^"_DT_"^LR52 571 Messages"
...
POST1 ;TRANSPORT FILE 64.81 INTO FILE 64 IF REQUIRED
 N LRREC,LRREC9
 K ^XTMP("LRNLT","LR571 ") D
 . S ^XTMP("LRNLT",0)=$$HTFM^XLFDT($H+90,1)_U_DT_U_"LR571  Added NLT Codes List"
 . S ^XTMP("LRNLT","LR571 ",0)=""
 ;D DSS W !
P1 F  L +^LAM:10 Q:$T  D BMES^XPDUTL("Attempting to Lock ^LAM Global.")
 ...
 S $P(^LAM(0),U,3)=99999
 D:'$G(LRDBUG)
 .S $ZE="LR571 : Application Error! please disregard NO need to report it" D ^%ZTER ;MAIL^LR571POA
...
SCR(LRMSG) ;Store message in ^XTMP("LRNLT" Global
 S LRSCR=$G(^XTMP("LRNLT","LR571 ",1,0))+1,^(0)=LRSCR
 S ^XTMP("LRNLT","LR571 ",1,LRSCR)=LRSCR_"|"_LRMSG
 Q
