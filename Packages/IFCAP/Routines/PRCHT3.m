PRCHT3 ; GENERATED FROM 'PRCHNREQ' INPUT TEMPLATE(#780), FILE 442;11/25/98
 D DE G BEGIN
DE S DIE="^PRC(442,",DIC=DIE,DP=442,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRC(442,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(10)=%
...
BEGIN S DNM="PRCHT3",DQ=1
 S DR(99,1,9.2)="S I(1,0)=$S($D(D1):D1,1:""""),I(0,0)=$S($D(D0):D0,1:""""),Y(1)=$S($D(^PRC(442,D0,0)):^(0),1:"""") S X=$P(Y(1),U,1) K DIC S DIC=""^PRC(442.8,"",DIC(0)=""NMFWLE"" D ^DIC S (D,D0,D(0))=+Y"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=780,U="^"
...
X2 S PRCHDUZ=$P(^VA(200,DUZ,0),U,1),PRCHIEN=DA
 Q
...
X8 I $D(^PRCS(410,+$P(^PRC(442,DA,0),"^",12),0)) I X'=OSUB W !,?5,"Sub-station cannot be changed because the attached 2237",!,?5,"would then have a different sub-station.",! S $P(^PRC(442,DA,23),U,7)=OSUB S Y=31
 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 S PRCX=$O(^PRC(411,X,1,0)) S:$G(PRCX)]"" PRCY=$P($G(^PRC(411,X,1,PRCX,0)),U) K PRCX
 Q
10 S DW="0;2",DV="R*P442.5'X",DU="",DLB="METHOD OF PROCESSING",DIFLD=.02
 S DE(DW)="C10^PRCHT3"
 S DU="PRCD(442.5,"
 S X="REQUISITION"
 S Y=X
...
C10S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X10 S DIC("S")="I $P(^(0),U,5)=1,"_$S($D(PRCHNRQ):"Y=8!(Y=25)",1:"Y'=8") S:$D(PRCHDELV) DIC("S")="I Y=1!(Y=26)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 I $P(^PRC(442,DA,0),U,2)'=25 S Y=.08
 Q
12 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=12 D X12 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X12 D LOOK^PRCSPC
 Q
13 D:$D(DG)>9 F^DIE17 G ^PRCHT31
