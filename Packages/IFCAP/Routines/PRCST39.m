...
X4 I $D(PRCSERR),PRCSERR S Y=PRCSERR K PRCSERR
 Q
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 D QRB^PRCSCK
 Q
6 S D=0 K DE(1) ;12
 S DIFLD=12,DGO="^PRCST310",DC="2^410.212I^2^",DV="410.212MNJ2,0",DW="0;1",DOW="DELIVERY SCHEDULE",DLB="Select "_DOW S:D DC=DC_D
 G RE:D I $D(DSC(410.212))#2,$P(DSC(410.212),"I $D(^UTILITY(",1)="" X DSC(410.212) S D=$O(^(0)) S:D="" D=-1 G M6
 S D=$S($D(^PRCS(410,D0,"IT",DA,2,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M6 I D>0 S DC=DC_D I $D(^PRCS(410,D0,"IT",DA,2,+D,0)) S DE(6)=$P(^(0),U,1)
 G RE
R6 D DE
 S D=$S($D(^PRCS(410,D0,"IT",DA,2,0)):$P(^(0),U,3,4),1:1) G 6+1
 ;
