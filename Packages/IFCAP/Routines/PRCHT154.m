...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCHT154",DQ=1
1 S DW="0;9",DV="RNJ12,4XO",DU="",DLB="ACTUAL UNIT COST",DIFLD=5
 S DQ(1,2)="S Y(0)=Y S Y=$S(Y=""N/C"":""N/C"",1:""$""_$J(Y,0,4))"
 S DE(DW)="C1^PRCHT154"
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 S:X["$" X=$P(X,"$",2) S X=$TR(X,"nc","NC") K:(X'?1.N)&(X'?.N1".".4N)&(X'?1"N/C")!(X?.E1"."5N.N)!(X>9999999.9999)!(X<0) X D EN9^PRCHNPO5 I $D(X) W $S(X="N/C":"     N/C",1:"     $"_$J(X,0,4))
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 I $D(^PRC(442,DA(1),1)),$P(^(1),U,20)="Y" S Y="@6"
 Q
3 D:$D(DG)>9 F^DIE17 G ^PRCHT155
