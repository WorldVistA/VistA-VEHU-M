...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCHT128",DQ=1
1 S DW="0;13",DV="NJ7,2XO",DU="",DLB="EST. SHIPPING AND/OR HANDLING",DIFLD=13
 S DQ(1,2)="S Y(0)=Y S Y="" $""_$J(Y,0,2)"
 S DE(DW)="C1^PRCHT128"
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 S:X["$" X=$P(X,"$",2) K:(X?.N1"."3N.N)!(X>9999.99)!(X<0) X I $D(X),$D(^PRC(442,DA,1)),$P(^(1),U,6)="D" D EN4^PRCHNPO5 I $D(X) W " $",$J(X,0,2)
 Q
 ;
...
X4 I $P(^PRC(442,DA,0),U,19)'=2 S Y="@98"
 Q
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 S PRCHSBOC=$P($G(^PRCD(420.2,2299,0)),U)
 Q
6 D:$D(DG)>9 F^DIE17 G ^PRCHT129
