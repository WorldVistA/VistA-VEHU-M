PRCHT1 ; GENERATED FROM 'PRCH2138' INPUT TEMPLATE(#273), FILE 442;11/25/98
 D DE G BEGIN
...
BEGIN S DNM="PRCHT1",DQ=1
 ...
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=273,U="^"
...
X2 S (PRCHN("SVC"),PRCHN("CC"),PRCHN("SC"),PRCHN("INV"))=""
 Q
...
X14 S X=$P($G(^PRC(442,DA,1)),U) I X>0,$P($G(^PRC(440,X,3)),U,2)="Y" S PRCHEDI=""
 Q
15 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=15 D X15 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X15 I '$D(^PRC(411,"UP",PRC("SITE"))) S Y=.1
 Q
16 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=16 D X16 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X16 S OSUB=$P($G(^PRC(442,DA,23)),U,7)
 Q
17 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=17 D X17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X17 I $D(^PRCS(410,+$P(^PRC(442,DA,0),"^",12),0)) S X=$P($G(^(0)),U,10) I +X=OSUB W !,"SUBSTATION: ",$P(^PRC(411,$P(^PRC(442,DA,23),U,7),0),U) S Y=.1
 Q
18 D:$D(DG)>9 F^DIE17 G ^PRCHT11
