...
DE S DIE="^PRC(442,",DIC=DIE,DP=442,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRC(442,DA,""))=""
 ...
 I $D(^(18)) S %Z=^(18) S %=$P(%Z,U,10) S:%]"" DE(17)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X7 W:0 "The jump Y=@20 is set in routine EN9^PRCHNPO7"
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 D EN9^PRCHNPO7
 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 S I(0,0)=D0 S Y(1)=$S($D(^PRC(442,D0,1)):^(1),1:"") S X=$P(Y(1),U,1),X=X S D(0)=+X S X=$S(D(0)>0:D(0),1:"")
...
X11 I '$D(PRCHEDI) S:$D(^PRC(442,DA,12)) $P(^(12),U,13,14)="^" S Y="@10"
 Q
...
X13 I X'="Y" S:$D(^PRC(442,DA,12)) $P(^(12),U,14)="" S Y="@10"
 Q
...
X16 S:PRCHN("SC")'=1 Y=8
 Q
17 S DW="18;10",DV="RFX",DU="",DLB="REQUISITION NO.(SUPPLY)",DIFLD=102.4
 S DE(DW)="C17^PRCHT33"
 G RE
...
C17S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X17 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>10!($L(X)<9)!'((X?3N1"-"3N1"-"2N)!(X?3N1"-"5UN)) X I $D(X) K:+X'=+^PRC(442,DA,0) X
 I $D(X),X'?.ANP K X
 Q
 ;
18 D:$D(DG)>9 F^DIE17 G ^PRCHT35
