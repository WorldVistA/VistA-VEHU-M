...
DE S DIE="^PRC(442,",DIC=DIE,DP=442,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRC(442,DA,""))=""
 I $D(^(17)) S %Z=^(17) S %=$P(%Z,U,17) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCHT338",DQ=1
1 S DW="17;17",DV="F",DU="",DLB="SPECIAL CODE",DIFLD=73.4
 S X="Y"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X1 K:$L(X)>1!($L(X)<1)!'(X?1"Y") X
 I $D(X),X'?.ANP K X
 Q
 ;
