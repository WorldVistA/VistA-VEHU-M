PSSOPKI ;BHAM ISC/MHA-New API's to CPRS for DEA/PKI Pilot Project ;03/11/02
 ;;1.0;PHARMACY DATA MANAGEMENT;**61,69,166,183,209,219,246**;9/30/97;Build 2
...
DETOX(PSSDIEN) ;Returns 1 if the drug is a detox drug - drug name contains "BUPREN" & the dosage form is not found in one of the XPAR parameters
 ...
 ; p246 BUPRENORPHINE/NALOXONE (FILM,BUCCAL) (Bunavail) requires Detox/Maintenance number
 N PSSGN,PSSDF S PSSGN="",PSSDF=""
 S:+$P($G(^PSDRUG(PSSDIEN,"ND")),"^",3) PSSNDF=$P(^("ND"),"^",3)
 S:$G(PSSNDF) PSSGN=$P($G(^PSNDF(50.68,PSSNDF,0)),"^",2),PSSDF=$P($G(^PSNDF(50.68,PSSNDF,0)),"^",3) ; VA GENERIC NAME, DOSAGE FORM
 Q:$$BUNAVAIL(PSSGN,PSSDF) 1 ; if drug is BUNAVAIL return DETOX as true -end of p246 changes
...
BUNAVAIL(PSSGNP,PSSDFP) ; VA GENERIC NAME, DOSAGE FORM iens ;p246
 Q:('PSSGNP)!('PSSDFP) 0
 Q:($G(^PSNDF(50.6,PSSGNP,0))="BUPRENORPHINE/NALOXONE")&($G(^PS(50.606,PSSDFP,0))="FILM,BUCCAL") 1
 Q 0
DETINDEX(LIST) ;
 N I,IEN
