RACTWR ; GENERATED FROM 'RA REPORT EDIT' INPUT TEMPLATE(#235), FILE 74;03/19/10
 D DE G BEGIN
DE S DIE="^RARPT(",DIC=DIE,DP=74,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RARPT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,8) S:%]"" DE(8)=%
...
BEGIN S DNM="RACTWR",DQ=1
 ...
 M DIEZAR=^DIE(235,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=235,U="^"
