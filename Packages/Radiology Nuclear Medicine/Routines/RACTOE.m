RACTOE ; GENERATED FROM 'RA ORDER EXAM' INPUT TEMPLATE(#663), FILE 75.1;03/19/19
 D DE G BEGIN
...
BEGIN S DNM="RACTOE",DQ=1
 ...
 M DIEZAR=^DIE(663,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=663,U="^"
...
C72F1 N X,X1,X2 S DIXR=1487 D C72X1(U) K X2 M X2=X D C72X1("O") K X1 M X1=X
 I $G(X(1))]"" D
...
C75F1 N X,X1,X2 S DIXR=1487 D C75X1(U) K X2 M X2=X D C75X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^RAO(75.1,"BDD",$E(X,1,30),DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^RAO(75.1,"BDD",$E(X,1,30),DA)=""
...
C79F1 N X,X1,X2 S DIXR=1487 D C79X1(U) K X2 M X2=X D C79X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^RAO(75.1,"BDD",$E(X,1,30),DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^RAO(75.1,"BDD",$E(X,1,30),DA)=""
