...
1 S DW="0;17",DV="D",DU="",DIFLD=17,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X4 S:$D(RAWHEN)#2 Y="@140"
 Q
5 S DW="0;21",DV="DR",DU="",DIFLD=21,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C5^RACTOE3",DE(DW,"INDEX")=1
 G RE
...
C5F1 N X,X1,X2 S DIXR=1487 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 I $G(X(1))]"" D
...
C5X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C5F2 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1424 S DIEZRXR(75.1,DIXR)=""
 Q
...
X6 S Y="@150"
 Q
7 S DQ=8 ;@140
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW="0;21",DV="DR",DU="",DIFLD=21,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C8^RACTOE3",DE(DW,"INDEX")=1
 S X=RAWHEN
 S Y=X
 G Y
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^RAO(75.1,"AP",+$P(^RAO(75.1,DA,0),U),+$P(^(0),U,2),9999999.9999-$E(X,1,30),DA)=""
C8F1 N X,X1,X2 S DIXR=1487 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^RAO(75.1,"BDD",$E(X,1,30),DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^RAO(75.1,"BDD",$E(X,1,30),DA)=""
...
C8X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C8F2 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1424 S DIEZRXR(75.1,DIXR)=""
 Q
...
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="0;21",DV="DR",DU="",DIFLD=21,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^RACTOE3",DE(DW,"INDEX")=1
 S X=RAWHEN
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
C12S S X="" G:DG(DQ)=X C12F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^RAO(75.1,"AP",+$P(^RAO(75.1,DA,0),U),+$P(^(0),U,2),9999999.9999-$E(X,1,30),DA)=""
C12F1 N X,X1,X2 S DIXR=1487 D C12X1(U) K X2 M X2=X D C12X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^RAO(75.1,"BDD",$E(X,1,30),DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^RAO(75.1,"BDD",$E(X,1,30),DA)=""
...
C12X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C12F2 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1424 S DIEZRXR(75.1,DIXR)=""
 Q
...
X14 S:$D(RAEXMUL)#2 RAWHEN=$$FMTE^XLFDT(X,1)
 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="0;5",DV="SX",DU="",DIFLD=5,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C15^RACTOE3",DE(DW,"INDEX")=1
...
C15F1 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1425 S DIEZRXR(75.1,DIXR)=""
 Q
X15 Q
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="0;18",DV="D",DU="",DIFLD=18,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C16^RACTOE3"
 S X="NOW"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
R19 D DE
 ...
 ;
20 S DQ=21 ;@160
21 S DW="0;19",DV="S",DU="",DIFLD=19,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X22 S:$D(RAEXMUL) RAMT=X
 Q
23 S DW="0;24",DV="S",DU="",DIFLD=24,DLB="IS PATIENT ON ISOLATION PROCEDURES?"
 S DU="y:YES;n:NO;"
 S X="NO"
 S Y=X
 G Y
...
X24 S:$D(RAEXMUL) RAIP=X
 Q
25 S DW="0;6",DV="S",DU="",DIFLD=6,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X27 S:$$ORVR^RAORDU()<3 Y="@163"
 Q
28 S DW="0;26",DV="S",DU="",DIFLD=26,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X32 I 'RAREQLOC S Y="@165"
 Q
33 S DW="0;20",DV="*P79.1'",DU="",DIFLD=20,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="RA(79.1,"
 S X=RAREQLOC
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X36 I '$D(RALOCFLG) S Y="@175"
 Q
37 S DW="0;20",DV="*P79.1'R",DU="",DIFLD=20,DLB="SUBMIT REQUEST TO"
 S DU="RA(79.1,"
 G RE
