ASFFUT ;CALLED FROM NSZ REFERRAL - PRINT FUTURE APPTS.
 ;;GDM/SYR 4/93 - PORTION OF  SDROUT0
...
LIN S X=K D TM W !,$J(X,8) I $D(^SC(L,0)) W ?11,$P(^(0),"^",1) D LOC W ?42,SDLOC K SDLOC D:$D(^DPT(J,"S",K,0)) SETP W:'$D(^DPT(J,"S",K,0)) ?70,"*DELETED*" D SCCOND^SDROUT2
 ...
 ;THE NEXT LINE PRINTS THE LOCATION-,UNDEF VAR 'DIV' MEANS NO PHYSICAL LOCATION ENTERED IN FILE #44 FOR THE SELECTED CLINIC.
LOC S SDLOC=$P(^SC(L,0),"^",11) I SDLOC']"",$D(^DIC(4,+$$SITE^VASITE,"DIV")),^("DIV")="Y" S SDLOC=$S($P(^SC(L,0),"^",15)=$G(DIV):"",$D(^DG(40.8,+$P(^SC(L,0),"^",15),0)):$P(^(0),"^",1),1:"")  ; FIXED $G(DIV) RMS 4-21-2000
 Q
FUT S HDFLAG=0 I $N(^DPT(J,"S",SDATE_".9"))>0 D HED2 F M=SDATE_".9":0 S M=$N(^DPT(J,"S",M)) Q:M<0!($Y>(IOSL-5))  I $S($P(^(M,0),"^",2)']"":1,$P(^(0),"^",2)="I":1,1:0) D LIN2
 I SDREP,SDX'["ALL" W !!,"DATE PRINTED  : " S Y=SDSTART D DTS^SDUTL W Y,!,"DATE REPRINTED: ",PRDATE Q
 W:HDFLAG=1 !!?6,"DATE PRINTED: ",PRDATE S HDFLAG=0 Q
LIN2 S SDM=M F SDI=3,4,5 I $P(^DPT(J,"S",SDM,0),"^",SDI)]"" S (X,M)=$P(^(0),"^",SDI) D TM^SDROUT0 S Y=M D DTS^SDUTL W !?6,Y,?17,$J(X,8),?28,$S(SDI=3:"LAB",SDI=4:"XRAY",1:"EKG") Q:($Y>(IOSL-1))
 S M=SDM K SDM,SDI
 S L=+^DPT(J,"S",M,0),X=M D TM S Y=M D DTS^SDUTL W !?6,Y,?17,$J(X,8),?28,$P(^SC(L,0),"^",1) D LOC W ?60,SDLOC K SDLOC
 Q
HED2 W !!,?20,"**FUTURE APPOINTMENTS**" S HDFLAG=1
 W !!?6,"  DATE",?17,"TIME",?28,"CLINIC",?62,"LOCATION",! Q
TM I $P(X,".",2)']"" S X1=""
