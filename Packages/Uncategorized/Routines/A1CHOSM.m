A1CHOSM ;ALB/ - OUTPATIENT WORKLOAD SUMMARY ; 2/28/89 1600
 ;;V 1.3
 S A1X="AS^AN^B^C^N^X^U"
 S HR="Outpatient Workload Summary",^UTILITY("A1CH","T","C")=0 W @IOF,!,?((IOM-$L(HR))/2),HR,?IOM-20,T2,!
 ...
 F K=1:1:DGTN F I=1:1:A2 F DGMT=1:1:7 S DGDV=$E($P(A(I),U,2)_"     ",1,5) I ^UTILITY("A1CH",DGJB,K,"TOT",DGDV)>0 S ^UTILITY("A1CH","T1",K,I,DGMT)=^UTILITY("A1CH","T1",K,I,DGMT)+^UTILITY("A1CH",DGJB,K,"TOT",DGDV,$P(A1X,U,DGMT))
 F K=1:1:DGTN F I=1:1:A2 F DGMT=1:1:7 S DGDV=$E($P(A(I),U,2)_"     ",1,5) I ^UTILITY("A1CH",DGJB,K,"TOT",DGDV)>0 S ^UTILITY("A1CH","T",K,I,DGMT)=^UTILITY("A1CH","T",K,I,DGMT)+^UTILITY("A1CH","T1",K,I,DGMT)
 ...
 D TOT^A1CHOSM1 W ! F I=1:1:4 W !,$P($T(LEG+I),";;",2)
 K A,A1X,A2,DGBD,DGDV,DGJB,DGMT,DGND,DGTN,HDR1,HR,I,J,K
 Q
PRI Q:^UTILITY("A1CH","T",K,I,"R")=0
 S ZRT1="Hit RETURN to continue" I (IOST["C-")&(IO=IO(0))&(IOSL-$Y<4) W !,?IOM-$L(ZRT1)-2,ZRT1 R ZRT:DTIME W @IOF D:$D(ZRT) HDR
