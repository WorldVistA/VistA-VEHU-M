RJPTFD ; DRGs REG-DAK/AVAMC 2/27/83 ; 18 SEP 84  12:54 pm
 ...
 ;RJ MODIFIED CALCULATES DRG BASED ON MOVEMENTS AND SURGERIES FOR JUST A PARTICULAR MOVEMENT
...
CD G Q:AGE'?1N.N K NOR,NSD,NDR,RG,CC,ORG S L=$P(B(70),U,10) S:L="" L=$P(B(70),U,11) G Q:L="" S L1=$S($D(^ICD9(L,0)):^(0),1:"") G Q:L1="" S DXLS=L,MDC=$P(L1,U,5),PD=$P(L1,U,2),RG=0 I MDC=12 S MDC=$S(SEX="F":13,1:12)
 ...
 S (OR,SD)="",SD1=1,NOR=0 F NSD=0:0 S NSD=$N(^DGPT(PTF,"M","AC",NSD)) Q:NSD'>0  D CHECK I L,NSD'="",$D(^ICD9(+NSD,0)),NSD'=DXLS S SD=SD_$P(^(0),U,2) S:$P(^(0),U,2)'["g" SD1=0
OP S L=1 F NO=0:0 S NO=$N(^DGPT(PTF,"S",NO)) Q:NO'>0  D CHECK1 I L'="" F I=1:1:5 S L1=$P(L,U,I+7) I L1'="",$D(^ICD0(+L1,0)) S DA=+L1,L2=^(0),OR=OR_$P(L2,U,2),NOR=NOR+1 F ORG=0:0 S ORG=$N(^ICD0(DA,"DR",ORG)) Q:ORG<1  S ORG($P(^ICD(ORG,0),"^",5),ORG)=""
...
CHECK S L=1,RJCO2=$O(^DGPT(PTF,"M","AC",NSD,0)) S RJDAT1=$P($P(^DGPT(PTF,"M",RJCO2,0),"^",10),".",1),RJDAT2=$P($P(^DGPT(PTF,"M",RJCO1,0),"^",10),".",1) I RJDAT1'>RJDATE S L=0 Q
 I RJDAT1>RJDAT2 S L=0 Q
 Q
CHECK1 S L=^(NO,0),RJDAT1=$P($P(^DGPT(PTF,"S",NO,0),"^",1),".",1),RJDAT2=$P($P(^DGPT(PTF,"M",RJCO1,0),"^",10),".",1) I RJDAT1'>RJDATE S L="" Q
 I RJDAT1>RJDAT2 S L="" Q
 Q
