PRMQE710 ; ;03/20/89
 D DE G BEGIN
DE S DIE="^PRMQ(513.8,D0,1,D1,4,",DIC=DIE,DP=513.804,DL=3,DIEL=2,DU="" K DG,DE,DB Q:$N(^PRMQ(513.8,D0,1,D1,4,DA,-1))<0
 I $D(^(2)) S %Z=^(2) S %=$P(%Z,"^",9) S:%]"" DE(1)=% S %=$P(%Z,"^",10) S:%]"" DE(2)=% S %=$P(%Z,"^",11) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQE710",DQ=1
1 S DW="2;9",DV="S",DU="",DLB="AVAILABLE AT THIS FACILITY",DIFLD=1.72
 S DU="1:YES;2:NO;"
 S Y="YES"
 G Y
X1 Q
2 S DW="2;10",DV="S",DU="",DLB="COVERED BY POLICY",DIFLD=1.73
 S DU="1:YES;2:NO;"
 S Y="NO"
 G Y
X2 Q
3 S DQ=4 ;@3
4 S DW="2;11",DV="D",DU="",DLB="DATE COVERAGE ENDS",DIFLD=1.74
 G RE
X4 S %DT="EX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
5 D:$D(DG)>9 F^DIE17 G ^PRMQE711
