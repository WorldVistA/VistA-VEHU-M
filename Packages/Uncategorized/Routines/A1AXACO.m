A1AXACO ; GENERATED FROM 'A1AX PACO' PRINT TEMPLATE (#1475) ; 06/10/88 ; (FILE 11830, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 S:'$D(DN) DN=1
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1475,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 W ?0 S X="ACTION PLAN AND SURVEY REPORT BY ORGAN" W #,?(IOM-$L(X))\2,X K DIP
 W ?11 S X="" S $P(X,"=",39)="" W !,?(IOM-$L(X))\2,X K DIP
...
A1 ;
 ...
 W ?77 W !,"REVIEWER:  ",A1AXO K DIP
 W ?88 I A1AXO="JCAHO" W !,"PROGRAM:  ",A1AXT K DIP
 W ?99 W !,"VAMC:  ",A1AXF K DIP
