ZZSPGFWD ; B'ham ISC/CML3 - SET ALL SYSTEMS' TIME AHEAD ONE HOUR ; [ 03/28/95  4:04 PM ]
 ;;
 D DT^DICRW
...
LOOP ; loop through nodes
 S $ZT="LOOP^ZZSPGFWD"
 F  S LOOP=$O(^DIZ(521550,LOOP)) Q:'LOOP  S ND=$G(^(LOOP,0)) I $P(ND,"^")?3U,$P(ND,"^",2)?3U,$P(ND,"^",2)_","_$P(ND,"^")'=$ZU(0) J ENFWD^ZZSPGFWD[$P(ND,"^",2),$P(ND,"^")]
 ;
DONE ;
