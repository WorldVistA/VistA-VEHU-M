...
DE S DIE="^PRMQ(513.8,D0,1,",DIC=DIE,DP=513.801,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^PRMQ(513.8,D0,1,DA,-1))<0
 I $D(^(3)) S %Z=^(3) S %=$P(%Z,"^",2) S:%]"" DE(1)=% S %=$P(%Z,"^",3) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQE114",DQ=1
1 S DW="3;2",DV="NJ3,0",DU="",DLB="EXPECTED LOS",DIFLD=6
 G RE
...
X2 S:$D(PRMQDXLS) Y="@1"
 Q
3 S DW="3;3",DV="D",DU="",DLB="DATE DXLS ENTERED",DIFLD=7
 S Y="T"
