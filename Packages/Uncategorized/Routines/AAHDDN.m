%AAHDDN ;402,DJB,11/2/91,EDD**NODE Lookup and Look-up by Global
 ;;GEM III;;
...
ND1 D NDPRT G:FLAGQ EX
 ...
 S FLAGQ=0 G ND1
EX ;
 S:'FLAGE FLAGQ=0 K ^UTILITY($J,"EDD/NP")
 Q
NDGET ;Node get
NDGET1 I FILE'=ZNUM W !!,"Select '",NODE1,"' SUBNODE: "
 E  W !,"Select NODE: "
 R NODE:GEMTIME S:'$T NODE="^" I "^"[NODE S FLAGQ=1 Q
...
NDBLD ;
 ...
 Q
NDPRT ;Print
 S PIECE="" W @GEMIOF D HD
 F  S PIECE=$O(^UTILITY($J,"EDD/NP",NODE,PIECE)) Q:PIECE=""  W !?3,$J(NODE_";"_PIECE,12),?20,$J($P(^UTILITY($J,"EDD/NP",NODE,PIECE),U),7),?32,$P(^(PIECE),U,2) I $Y>GEMSIZE D PAGE Q:FLAGQ=1
 Q
NDSUM ;
 ...
 S FNUM=+Y,FNAM=$P(Y,U,2),LEV=1,FILE(LEV)=FILE D ^%AAHDDI1 Q:FLAGQ
...
GLOBAL ;Find File when user enters global
 ...
 S GEMXX=X F I=1:1 S GEMXX=$O(^UTILITY("EDD/GL",GEMXX)) Q:GEMXX=""!($E(GEMXX,1,$L(X))'=X)  D GLLIST I I#5=0 D GLPAGE Q:FLAGGL!FLAGGL1
 I '$D(^UTILITY($J)) W *7," ??" S FLAGGL=1
 Q:FLAGGL
 I 'FLAGGL1 S I=(I-1) D GLPAGE1 Q:FLAGGL
 I Z1>I F II=(I+1):1:Z1 S GEMXX=$O(^UTILITY("EDD/GL",GEMXX)) Q:GEMXX=""!($E(GEMXX,1,$L(X))'=X)  S ^UTILITY($J,II)=$P(^UTILITY("EDD/GL",GEMXX),U)_"\~"_$P(^(GEMXX),U,2)_"\~"_GEMXX
...
GLLIST ;List Globals
 ...
 W !?3,$J(I,3),"  ",GEMXX,?28,$J($P(^UTILITY("EDD/GL",GEMXX),U),8),?38,$E($P(^(GEMXX),U,2),1,32)
 S ^UTILITY($J,I)=$P(^UTILITY("EDD/GL",GEMXX),U)_"\~"_$P(^(GEMXX),U,2)_"\~"_GEMXX
 Q
GLPAGE ;
 W !,"TYPE '^' TO STOP, OR",!,"CHOOSE NUMBER: "
 R Z1:GEMTIME S:'$T Z1="^" I Z1="?" W "  Enter a number from left hand column.." G GLPAGE
 S:Z1["^" FLAGGL=1 I +Z1>0 S FLAGGL1=1
 Q
GLPAGE1 ;
 W !,"TYPE '^' TO STOP, OR",!,"CHOOSE NUMBER: "
 R Z1:GEMTIME S:'$T Z1="^" I "^"[Z1!(+Z1'>0) S FLAGGL=1
