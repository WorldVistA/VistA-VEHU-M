A4AARPT1 ;HINES-CIOFO/JJM - REPORT 1 ; 7/15/99 19:20
V ;;1.0;**** CLASS III ****;;JUN 30, 1998
 ;
EBILLS ;
 S BFLAG=1
 D DEBTOR^A4AARPT Q:Y=-1
 D OPEN
 D START
 D CLOSE
 QUIT
...
START ;
 ...
 Q:'DEBTOR
 ; BFLAG = 1 TO LIST ONLY INCORRECT BILLS (DEFAULT)
 ; BFLAG = 0 TO LIST ALL BILLS
 ; FLAG1 = 0 SUMMARY FORMAT (DEFAULT)
 ; FLAG1 = 1 DETAIL  FORMAT 
 W !,?30,"BILL LISTING FOR DEBTOR ( ",DEBTOR," )",!!
PRTRPT1 ;
...
PBILLS ;
 ...
 ; S FLAG1=1
 D ^A4AARPT7
 I ((BAL'=RBAL)&(CS'=23))!(CS=40)!((BAL'=0)&(CS=39))!((RBAL<0)&(CAT'=26))!((RBAL'=0)&(CS=23))!((BAL'=0)&(CS=22)) D
   . S FLAG1=1
   . W @IOF,"****  INCORRECT BILL  ****"
...
PRTBILL ;
 ...
 ; I $Y+5>IOSL D:IOT="VTRM" PAUSE^A4AARPT3 Q:END  D PRTHDR1
 ; D:$Y>50 PRTHDR1
 D:FLAG1=1 ARPT1
 ...
 S TDATE=$P(^PRCA(430,BILL,0),U,10) S:TDATE="" TDATE=" "
...
PTRANS ;
 ...
 . I (IOT="VTRM")&(($Y+3)>IOSL) D PAUSE^A4AARPT3 Q:END  D PRTHDR1
...
TRANBILL W !,"BILL NUMBER :",?16,$P(^PRCA(430,BILL,0),"^"),"(",BILL,")",?40,"       TYPE : " S TYPE=$P(^PRCA(430,BILL,0),"^",2),TYPE=$P($G(^PRCA(430.2,+TYPE,0)),"^")
 ...
 W ?54,TYPE,!,"     STATUS :",?16,STATUS,?40,"       DATE :" S Y=DATE X ^DD("DD") W ?54,Y W !,?29," FILE 430 DATA"
 S F430N6=$G(^PRCA(430,BILL,6))
 W !!,"LETTER DATES: ",?15
 F I=1:1:3,8,9 S DATE=$P(F430N6,U,I) S Y=DATE X ^DD("DD") W $J(Y,12)
 W !,LINE
 D P430D^A4AARPT7
 Q
 ;
