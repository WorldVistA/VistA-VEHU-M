APGKOB1 ;PHOENIX/KLD; 10/8/98; TIU OBJECTS; 12/21/99  10:27 AM
ST Q
 ;
MAR() ;Marital status
...
APN() ;Address & phone for Next of Kin
 N REC,X S REC=$G(^DPT(DFN,.21))
 S X=$S($P(REC,U,3)]"":$P(REC,U,3),1:"NO STREET LISTED")
...
RDS() ;Rated disabilities (up to 10)
 ...
 Q "~@^TMP(""APGKOB1"","_$J_")"
 ;
TRA(N,T) ;Transfusions in last year (up to N)
 N C,DA,I,II,INV,X D K,NONE("TRANSFUSIONS") D AGO^APGKOB2
 S C=0,INV=9999999-X ;(DT-10000)
 S LRDFN=$G(^DPT(DFN,"LR")) I 'LRDFN Q "TRANSFUSIONS - NO LRDFN ON FILE"
 F I=0:0 S I=$O(^LR(LRDFN,1.6,I)) Q:'I!(I>INV)!(C>N)  D
 .S X=$G(^(I,0)) Q:X=""  D SET($$D(+X)_"   "_$P(^LAB(66,$P(X,U,2),0),U))
 .F II=0:0 S II=$O(^LR(LRDFN,1.6,I,2,II)) Q:'II  D
 ..D SET("  Comments: "_^(II,0))
 Q "~@^TMP(""APGKOB1"","_$J_")"
 ;
...
TL() ;Today's Lab results
 ...
 S C=0,LRDFN=$G(^DPT(DFN,"LR")),SD=9999999-DT-1,ED=SD+1
 I 'LRDFN D SET("INVALID LRDFN - CANNOT ACCESS LAB DATA") Q "~@^TMP(""APGKOB1"","_$J_")"
 D TL1 Q "~@^TMP(""APGKOB1"","_$J_")"
TLN(N) ;Last N*24 hours lab results
 N C,I,LRDFN,TEST,X D K,NONE("LAB RESULTS LAST "_(N*24)_" HRS"),NOW^%DTC
 S C=0,LRDFN=$G(^DPT(DFN,"LR")),SD=DT_"."_$P(%,".",2),SD=9999999-SD
 S X1=DT,X2=-N D C^%DTC S ED=(9999999-X-1)_"."_$P(SD,".",2)
 I 'LRDFN D SET("INVALID LRDFN - CANNOT ACCESS LAB DATA") Q "~@^TMP(""APGKOB1"","_$J_")"
 D TL1 Q "~@^TMP(""APGKOB1"","_$J_")"
 ;
TL1 F I=SD:0 S I=$o(^LR(LRDFN,"CH",I)) Q:'I!(I>ED)  D
 .F TEST=1:0 S TEST=$o(^LR(LRDFN,"CH",I,TEST)) Q:'TEST  S X=^(TEST) D
 ..D SET($P(^DD(63.04,TEST,0),U)_": "_$P(X,U)_"  "_$P(X,U,2))
 Q
 ;
BMI(N) ;Body Mass Index  |BMI;N;2Y|
 N AGO,C,DA,H,HT,I,WT,X
 S AGO=9999999-(DT-20000),C=0,X="BODY MASS INDEX" D K,NONE(X)
 F I=0:0 S I=$O(^GMR(120.5,"AA",DFN,8,I)) Q:'I!(I>AGO)!(C=N)  D
 ...
 S C=0 D SET(X) F I=1:1:N D:$D(WT(I))
...
RELIG() ;Patient's religious preference
 ...
 ;
OCC() ;Patient's occupation
 N X S X=$P($G(^DPT(DFN,0)),U,7)
 Q "OCCUPATION - "_$S(X="":"NONE FOUND",1:X)
 ;
K K ^TMP("APGKOB1",$J) Q
