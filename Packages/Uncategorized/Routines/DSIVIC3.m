DSIVIC3 ;DSS/LM - Insurance card RPC's ;05/17/2006 11:19
 ;;2.2;INSURANCE CAPTURE BUFFER;**1,2,4**;May 19, 2009;Build 7
...
X12(DSIVFLD,DSIVFILE,DSIVY,LABEL) ;convert X12 elements to readable data DSIV 2.0P2
 ...
 . S FILE=+$E(DSIVX12("SPECIFIER"),2,10)
 . D FILE^DID(FILE,"","NAME","DSIVF")
 . I $E(DSIVF("NAME"),1,3)="X12" S DSIVY=$$X12^IBCNERP2(FILE,DSIVY)
...
PIIV(DSIV,DFN,DSIVPATP) ;DSIV GET EIV PATIENT REPORT(from file 2.333 using buffer ien) DSIV*2.2*2
 ...
 ;                or 2.3226^1^.03^555-9090^COMMUNICATION NUMBER (file#^subfile entry#^field#^data^fieldname)
 ;
 S DSIV=$NA(^TMP("DSIVIC3",$J)) K @DSIV
 N DSIVDL,DSIVD,DSIVERR,CNT S CNT=1
 I '$G(DFN) S @DSIV@(0)="-1^Invalid Patient ien" Q
 I $G(DSIVPATP) D GETS^DIQ(2.312,DSIVPATP_","_DFN_",","**","IEN","DSIVD","DSIVERR") D RPT(.DSIVD) Q
 D LIST^DIC(2.312,","_DFN_",","@;.01;","I",,,,,,,"DSIVDL","DSIVERR")
 S DSIVPATP=0 F  S DSIVPATP=$O(DSIVDL("DILIST","ID",DSIVPATP)) Q:'DSIVPATP  D
 .K DSIVD D GETS^DIQ(2.312,DSIVPATP_","_DFN_",","**","IEN","DSIVD","DSIVERR") D RPT(.DSIVD)
 .Q
...
RPT(DSIVD) ;eIV DATA for one or more patient policies
 ...
 I +GRP S GRP=GRP_U_$$GET1^DIQ(355.3,GRP,.03) ;get correct group name DSIV*2.2*2T2
 S CNT=CNT+1,@DSIV@(CNT)="$$INS$$"_INS
 S CNT=CNT+1,@DSIV@(CNT)="$$GRP$$"_GRP
 S X=$NA(DSIVD(2.322)) F I=1:1 S X=$Q(@X) Q:X=""  S Y=@X,FLD=$QS(X,3) D
 .I $QS(X,4)="E" Q
 .I FLD=.01 Q
 .I Y["DSIVD(" Q
 .S CNT=CNT+1,LABEL="",DATA=$$X12(FLD,$QS(X,1),Y,.LABEL)
 .I $QS(X,1)=2.322 S @DSIV@(CNT)=$QS(X,1)_U_FLD_U_DATA_U_LABEL Q
 .S @DSIV@(CNT)=$QS(X,1)_U_+$QS(X,2)_U_FLD_U_DATA_U_LABEL
 .Q
