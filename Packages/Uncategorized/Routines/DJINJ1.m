DJINJ1 ;JA/WASH;INSERT AND LOOK UP ; 04 Sep 86  11:22 AM
 ;VERSION 1.36
 ...
 I '$D(DJY) S V(V)=$E(X,1,+DJJ(V)) D O S @$P(DJJ(V),U,2) X XY W @DJHIN X XY W V(V),@DJLIN S:DJ4["D"!(DJ4["S") X=DJXX X DJCP X:$D(^DJR(DJN,1,$N(^DJR(DJN,1,"A",V,0)),1)) ^(1) G NXT^DJINJ
P I DJ4["P" D P^DJINQ S @$P(DJJ(V),U,2) X XY G NXT^DJINJ:$D(Y)=0,TK^DJINJ
 S V(V)=$S(X="@":"",1:X) D O X DJCP I $D(^DJR(DJN,1,$N(^DJR(DJN,1,"A",V,0)),1)) X DJCP S:DJ4["D" X=DJXX X ^(1) S @$P(DJJ(V),U,2) X XY
 G:DJAT=.01&(V(V)="") Q^DJINJ G T4^DJINJ
Q1 S:'$D(X) X=DJXX D ^DJINQ G TK^DJINJ
T1 I $D(^DJR(DJN,1,$N(^DJR(DJN,1,"A",V,0)),1)) X DJCP X ^(1) S @$P(DJJ(V),U,2) X XY
 ...
 G LH^DJINJ
K1 G NXT^DJINJ:X=""&($D(DJDN)),LST^DJINJ:X=""&('$D(DJDN)) I $D(DJST),DJST=1 K ^UTILITY($J,"DJST"),DJST
 S:X'=" " DIC(0)="LMEQZ" S:X=" " DIC(0)="Z" X DJCP W X S:$D(DJDICS) DIC("S")=DJDICS D ^DIC K DIC("S") S DIC(0)="LMEQZ"
 R !,"Type <CR> to continue",X:DTIME S DJSV=V,DJFF=0 D N^DJDPL S V=DJSV
 I Y<0 S @$P(DJJ(V),U,2),X="" X XY G TK^DJINJ
 S (X,V(V))=$P(Y(0),U,1),(DA,W(V))=+Y,DJDNM=V(V) X DJCP S:'$D(DJST) DJST=1,D0=DA I DJST=1 G LOCK
C S:'DJP DJDN=+Y X DJCP K Y,DJLK D ^DJC2 X XY G NXT^DJINJ
ER K X W *7 G Q1
