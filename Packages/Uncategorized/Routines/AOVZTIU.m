AOVZTIU ;VAPA/640/LYL tiu utility routine  1/12/98
 ;;1.0;TEXT INTEGRATION UTILITIES;;Jun 20, 1997
 ;MODIFIED ^AOVTIU2 FOR TUBEFEEDING INFO
 ;
 ;
...
SUPFEED(DFN,TARGET) ; -- entry point for inpt supplemental feedings for tiu
 ...
 Q "~@"_$NA(@TARGET)
 ;
TUBFEED(DFN,TARGET)     ; -- entry point for inpt TUBE feedings for tiu
 N WARD,ADM,X,NO,L,K1,K,N,K2,Z,Y,Q,LINE
 S WARD=$G(^DPT(DFN,.1))
 S:WARD ADM=$G(^DPT("CN",WARD,DFN))
 I WARD=""!($G(ADM)="") Q "Not currently admitted"
 S X(0)=$G(^FHPT(DFN,"A",ADM,0))
 S NO=$P(X(0),"^",7),Y=$S('NO:"",1:^FHPT(DFN,"A",ADM,"TF",NO,0))
 S L=4 F K1=1:1:3 S K=0,N(K1)="" F K2=1:1:4 S Z=$P(Y,U,L+1),Q=$P(Y,U,L+2),L=L+2 I Z'="" S:'Q Q=1 S:N(K1)'="" N(K1)=N(K1)_"' " S N(K1)=N(K1)_Q_" "_$P(^FH(118.2,Z,0),"^",1)
 S LINE=0
 S N=0 F  S N=$O(N(N)) Q:N<1  S LINE=LINE+1,@TARGET@(LINE,0)=N(N)
 S @TARGET@(0)="^^"_LINE_"^"_LINE_"^"_DT_"^^"
 Q "~@"_$NA(@TARGET)
