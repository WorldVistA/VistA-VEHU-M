...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(2)=% S %=$P(%Z,U,5) S:%]"" DE(3)=% S %=$P(%Z,U,6) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
C1F1 N X,X1,X2 S DIXR=254 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 D
...
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;4",DV="Fa",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C2^ZZMAS5",DE(DW,"INDEX")=1
 G RE
...
C2F1 N X,X1,X2 S DIXR=255 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 D
...
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;5",DV="P5'a",DU="",DLB="STATE",DIFLD=.115
 S DE(DW)="C3^ZZMAS5",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C3F1 N X,X1,X2 S DIXR=256 D C3X1(U) K X2 M X2=X D C3X1("O") K X1 M X1=X
 D
...
C3F2 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;6",DV="F",DU="",DLB="ZIP CODE",DIFLD=.116
 S DE(DW)="C4^ZZMAS5",DE(DW,"INDEX")=1
 G RE
C4 G C4S:$D(DE(4))[0 K DB
 D ^ZZMAS7
C4S S X="" G:DG(DQ)=X C4F1 K DB
 D ^ZZMAS8
C4F1 N X,X1,X2 S DIXR=158 D C4X1(U) K X2 M X2=X D C4X1("O") K X1 M X1=X
 D
...
C4X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C4F2 S DIXR=257 D C4X2(U) K X2 M X2=X D C4X2("O") K X1 M X1=X
 D
...
C4X2(DION) K X
 ...
 Q
C4F3 Q
X4 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>5!($L(X)<5)!'(X?5N) X
 I $D(X),X'?.ANP K X
 Q
 ;
5 D:$D(DG)>9 F^DIE17 G ^ZZMAS9
