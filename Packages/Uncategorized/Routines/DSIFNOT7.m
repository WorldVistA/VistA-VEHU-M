DSIFNOT7 ;DSS/AMC - PATIENT PROFILE TEAM INFORMATION ;03/01/2009
...
TDATA(DFN,VALMCNT,SDATE,SDPRT,SDCOL) ;Team information - gather, format and optionally print.
 ...
 N SDTM,SDTMN,SDPO,SDPON,SDPR,SDPRN,PAGER,PHONE,CNT
 ;
...
PHONE(IEN) ;Get provider's pager and phone numbers.
 ...
 S (PAGER,PHONE)=""
 Q:'$G(IEN)
 S PAGER=$$GET1^DIQ(200,IEN,.137)
 S PHONE=$$GET1^DIQ(200,IEN,.132)
 Q
 ;
...
PRT ;Write assignment information
 N SDI S SDI=0
 F  S SDI=$O(^TMP("SDTEMP",$J,SDI)) Q:'SDI  D
 .S CNT=CNT+1,@FBOUT@(CNT)=^TMP("SDTEMP",$J,SDI) Q
 Q
 ;
