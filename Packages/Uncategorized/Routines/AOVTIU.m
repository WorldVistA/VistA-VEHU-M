...
OMED(DFN,TARGET) ; -- entry point for Outpatient rx Object
 ...
 . S RX(0)=^TMP("PSOO",$J,RX,0),RX(1)=^(1,0)
 . Q:$E($P(RX(0),U,5))'="A"&($E($P(RX(0),U,5))'="S")
 . Q:$E($P(^PSDRUG(+$P(RX(0),U,3),0),U,2))="X"
 . I RX(1)'="" S DA=RX(1) D SIG^AOVSIG(.DA) S RX(1)=DA
...
LAB(DFN,TEST,RANGE,NULL) ; -- Entry point called by lab tiu objects for
 ...
 I $A($E($P(AOVLAB,U,4),1))<65 S TEST=TEST_$$SPACE($J($P(AOVLAB,U,4),6,2),6)
...
DEL(TIUDA) ;
 ...
 S ^XMB(3.9,XMZ,2,7,0)="      Reason: "_$S(REASON=1:"ENTERED ON WORNG PATINET",REASON=2:"DUPLICATE NOTE",REASON=3:"MAJOR REVION NEEDED",1:"OTHER")
 S ^XMB(3.9,XMZ,2,8,0)=""
...
RXTODAY(DFN,TARGET) ; -- entry point for Outpatient rx Object issued today
 ...
 . S LINE=LINE+1,@TARGET@(LINE,0)=$E($P($P(RX(0),U,3),";",2),1,37) ;40
 . ;S LINE=LINE+1,@TARGET@(LINE,0)=
 .S ^(0)=^(0)_"   Sig: "_$TR($E(RX(1),1,77),"*")
 . W "."
...
FIX ;
 ...
 . S DR=$P(DFN(0),U,3)
 . ;
 . ; fixes visit date problem from uploaded notes
 . I DR,TYPE'="W" S DR=+$G(^AUPNVSIT(DR,0)) D:DR
 . . S DR="1301///"_DR,DIE="^TIU(8925,",DA=DFN
 . . D ^DIE
 . ;
 . ; removed because testing patch 21 should fix this *********
