A1B2CS ; GENERATED FROM 'A1B2 SPECIALTY STUFF' INPUT TEMPLATE(#1074), FILE 11500.61;01/31/91
 D DE G BEGIN
DE S DIE="^A1B2(11500.61,",DIC=DIE,DP=11500.61,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^A1B2(11500.61,DA,-1))<0
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="A1B2CS",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=1074,U="^"
...
C1 G C1S:$D(DE(1))[0 K DB S X=DE(1),DIC=DIE
 K ^A1B2(11500.61,"C",$E(X,1,30),DA)
 S X=DE(1),DIC=DIE
 K DIV S DIV=X,D0=DA,DIV(0)=D0 S Y(0)=X S X=$S($D(A1B2NTL):0,1:1) I X S X=DIV S Y(1)=$S($D(^A1B2(11500.61,D0,0)):^(0),1:"") S X=$P(Y(1),U,16),X=X S DIU=X K Y S X="" X ^DD(11500.61,.02,1,2,2.4)
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^A1B2(11500.61,"C",$E(X,1,30),DA)=""
 S X=DG(DQ),DIC=DIE
 K DIV S DIV=X,D0=DA,DIV(0)=D0 S Y(0)=X S X=$S($D(A1B2NTL):0,1:1) I X S X=DIV S Y(1)=$S($D(^A1B2(11500.61,D0,0)):^(0),1:"") S X=$P(Y(1),U,16),X=X S DIU=X K Y X ^DD(11500.61,.02,1,2,1.1) X ^DD(11500.61,.02,1,2,1.4)
 Q
X1 S DIC("S")="I $P(^(0),U,15)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;3",DV="RNJ3,0",DU="",DLB="LOS",DIFLD=.03
 S X=A1B2LOS
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 K:+X'=X!(X>999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
3 G 0^DIE17
