...
DE S DIE="^PRMQ(513.8,D0,1,",DIC=DIE,DP=513.801,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^PRMQ(513.8,D0,1,DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(4)=%
 I $D(^(5)) S %Z=^(5) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,2) S:%]"" DE(2)=% S %=$P(%Z,U,3) S:%]"" DE(6)=%
 I $D(^(7)) S %Z=^(7) S %=$P(%Z,U,4) S:%]"" DE(7)=% S %=$P(%Z,U,5) S:%]"" DE(8)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQEU1",DQ=1
1 S DW="5;1",DV="P200'I",DU="",DLB="ENTRY CREATED BY",DIFLD=11
 S DU="VA(200,"
 S X=$S($D(DUZ):DUZ,1:"")
 S Y=X
 ...
 G RD
X1 Q
2 S DW="5;2",DV="MDI",DU="",DLB="ENTRY DATE",DIFLD=12
 S Y="T"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X2 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S:'$D(PRMQPHY) Y="@1"
 Q
4 S DW="0;2",DV="P6'O",DU="",DLB="RESIDENT OR FELLOW",DIFLD=1
 S DQ(4,2)="S Y(0)=Y S Y=$S('$D(PRMQIEN):Y,Y]"""":$S($D(^DIC(16,Y,0)):$P(^(0),U),1:Y),1:"""")"
 S DU="DIC(6,"
 S X=PRMQPHY
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X4 Q
5 S DQ=6 ;@1
6 S DW="5;3",DV="S",DU="",DLB="HEALTH INSURANCE",DIFLD=.51
 S DU="1:YES;2:NO;"
 S X=PRMQIS
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X6 Q
7 S DW="7;4",DV="SI",DU="",DLB="TRANSFER",DIFLD=.2
 S DE(DW)="C7^PRMQEU1"
 S DU="1:YES;2:NO;"
 S X=PRMQTRAN
 S Y=X
...
C7S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X7 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW="7;5",DV="MD",DU="",DLB="ORIGINAL ADMISSION DATE",DIFLD=.21
 S X=PRMQOADM
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X8 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
9 D:$D(DG)>9 F^DIE17 G ^PRMQEU2
