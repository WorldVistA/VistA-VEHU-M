ZZMAS ; GENERATED FROM 'ZZMAS' INPUT TEMPLATE(#1336), FILE 2;06/26/22
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(3)=%
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,2) S:%]"" DE(4)=% S %=$P(%Z,U,3) S:%]"" DE(6)=% S %=$P(%Z,U,4) S:%]"" DE(7)=% S %=$P(%Z,U,5) S:%]"" DE(8)=% S %=$P(%Z,U,6) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="0;1",DV="RFXa",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X1 K:$L(X)>30!($L(X)<3) X I $D(X) S DG20NAME=X,(X,DG20NAME)=$$FORMAT^XLFNAME7(.DG20NAME,3,30) K:'$L(X) X,DG20NAME S:$D(X) DGNEWVAL=X
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;1",DV="FaXJ35",DU="",DIFLD=.111,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C2X1(DION) K X
 ...
 Q
C2F2 Q
X2 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>35!($L(X)<3) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;8",DV="P13'a",DU="",DIFLD=.08,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^ZZMAS"
 S DU="DIC(13,"
 G RE
...
C3F1 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;2",DV="Fa",DU="",DIFLD=.112,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X5 S:X="" Y=.114
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".11;3",DV="FaXJ30",DU="",DIFLD=.113,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C6^ZZMAS",DE(DW,"INDEX")=1
 G RE
...
C6F1 N X,X1,X2 S DIXR=254 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C6X1(DION) K X
 ...
 Q
C6F2 Q
X6 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;4",DV="FaXJ15",DU="",DIFLD=.114,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C7^ZZMAS",DE(DW,"INDEX")=1
 G RE
...
C7F1 N X,X1,X2 S DIXR=255 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
C7X1(DION) K X
 ...
 Q
C7F2 Q
X7 K:$L(X)>15!($L(X)<2) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;5",DV="*P5'a",DU="",DIFLD=.115,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C8^ZZMAS",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^ZZMAS2
C8F1 N X,X1,X2 S DIXR=256 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
...
C8X1(DION) K X
 ...
 Q
C8F2 Q
X8 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".11;6",DV="F",DU="",DIFLD=.116,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C9^ZZMAS",DE(DW,"INDEX")=1
 G RE
C9 G C9S:$D(DE(9))[0 K DB
 D ^ZZMAS3
C9S S X="" G:DG(DQ)=X C9F1 K DB
 D ^ZZMAS4
C9F1 N X,X1,X2 S DIXR=158 D C9X1(U) K X2 M X2=X D C9X1("O") K X1 M X1=X
 D
 ...
 G C9F2
C9X1(DION) K X
 S X(1)=$G(@DIEZTMP@("V",2,DIIENS,.116,DION),$P($G(^DPT(DA,.11)),U,6))
 S:'$G(EASDO2) X=$$ZIP^DGREGDD1(DA,X(1)),EASDO2=1
 S:$D(X)#2 X(2)=X
 S X=$G(X(1))
 Q
C9F2 S DIXR=257 D C9X2(U) K X2 M X2=X D C9X2("O") K X1 M X1=X
 D
 . D FC^DGFCPROT(.DA,2,.116,"KILL",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
 K X M X=X2 D
 . D FC^DGFCPROT(.DA,2,.116,"SET",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
 G C9F3
C9X2(DION) K X
 S X(1)=$G(@DIEZTMP@("V",2,DIIENS,.116,DION),$P($G(^DPT(DA,.11)),U,6))
 S X=$G(X(1))
 Q
C9F3 Q
X9 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>5!($L(X)<5)!'(X?5N) X
 I $D(X),X'?.ANP K X
 Q
 ;
10 D:$D(DG)>9 F^DIE17 G ^ZZMAS5
