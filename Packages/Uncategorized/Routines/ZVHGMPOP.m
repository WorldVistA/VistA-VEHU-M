ZVHGMPOP ;SLC/DAN Auto-populate database with vitals ; 12/2/10 8:12am
 ;
...
SWITCH ;takes the date input from the user and does a switcheroo so the program
 ...
 I Y'="" S ATIME=Y
 ;
 ;W !!,"Select patients to assign vitals results to.  Select numbers between",!,"0 and 950 with the number corresponding to the VeHU patient that you want to",!,"update.  For example, choosing number 12 will update twelve,patient.",!!
 ...
 ;[1/28/2015 AJC - 4 LINES ABOVE REPLACED WITH LINES BELOW
 NEW LONER,LIST
 DO EN^ZVHPAT
 IF $DATA(LIST) MERGE LONER=LIST ; vehu patients
 IF $DATA(^TMP("ZVHPAT",$J)) DO  ; select individual patients
 . NEW PATIENT SET PATIENT=""
 . FOR  SET PATIENT=$ORDER(^TMP("ZVHPAT",$J,PATIENT)) Q:'PATIENT  DO
 . . IF $DATA(^DPT(PATIENT,0)) SET LONER(PATIENT)=""
 ;[1/28/15 AJC - END CHANGES]
 ;
 ;
