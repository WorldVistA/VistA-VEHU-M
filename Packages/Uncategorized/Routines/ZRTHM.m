ZRTHM ;HVB/MTZ - MULTIPLE CPU RTHIST OUTPUT (DSM-11) ;7/7/88  09:28
 ;;6.0;
 W !,"   EDIT C2^DDPCIR:  r NODE  w NODE,ZCODE(ND)=N   (THEN COMMENT OUT THIS LINE)",! Q
 W !,?7,"Multiple CPU System Usage Histogram",!
 S ID=^SYS(0,"RUNNING"),ST=$V(44),%A=1 I ID=""!$V(ST+35) W !,"DDP is not available in the baseline system",! Q
 S %A=$V(ST+144)=0 I %A W !,"DDP is not available in this configuration" Q
 D CHKSYS^SYSROU Q:%A  S RV=-1 D CIRSTA^DDPCIR S N=0 F I=1:1 S N=$O(ZCODE(N)) Q:N=""  S CODE(I)=ZCODE(N),NDDP=I
B ;K (CODE,DT,DUZ,NDDP,U) ;   ***** EXCLUSIVE KILL *****
...
ZIS D ^%ZIS Q:POP=1  U IO W #
 S FIRST=FIRST-1 F Z=0:0 S FIRST=$N(^RTH(FIRST)) Q:$D(^(FIRST))  I FIRST<0 S QMSG="NO DATA" G QUIT
 ...
 G PRINT
GETLP S P=0,N=FIRST-1 F Z=0:0 S N1=N,N=$N(^["MGR",CODE(I)]RTH(N)) Q:N<0!(N>LAST)  D GET
 Q
GET S STIME(I,N)=^["MGR",CODE(I)]RTH(N,"STIME"),ET(I,N)=$P(^("ETIME"),",",2)-$P(STIME(I,N),",",2) S:ET(I,N)<0 ET(I,N)=ET(I,N)+86400
 ...
 Q
PRINT S N=-1 F Z=0:0 S N=$N(DISK(0,N)) Q:N<1  D PRUN
