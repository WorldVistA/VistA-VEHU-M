PRMQEU ; GENERATED FROM 'PRMQS2' INPUT TEMPLATE(#606), FILE 513.8;06/27/91
 D DE G BEGIN
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQEU",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=606,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S DIE("NO^")="OUTOK"
 Q
2 S D=0 K DE(1) ;2
 S DIFLD=2,DGO="^PRMQEU1",DC="69^513.801ID^1^",DV="513.801D",DW="0;1",DOW="ADMISSION OR TRANSFER DATE",DLB="Select "_DOW
 I $D(DSC(513.801))#2,$P(DSC(513.801),"I $D(^UTILITY(",1)="" X DSC(513.801) S D=$N(^(0)) G M2
 S D=$S($D(^PRMQ(513.8,DA,1,0)):$P(^(0),U,3,4),1:$N(^(0)))
M2 I D>0 S DC=DC_D I $D(^PRMQ(513.8,DA,1,+D,0)) S DE(2)=$P(^(0),U,1)
 G RE
R2 D DE
 G A
 ;
