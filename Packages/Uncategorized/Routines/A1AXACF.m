A1AXACF ; GENERATED FROM 'A1AX PACF' PRINT TEMPLATE (#1474) ; 06/10/88 ; (FILE 11830, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 S:'$D(DN) DN=1
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1474,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 W ?0 S X="ACTION PLAN AND SURVEY REPORT BY FACILITY" W #,?(IOM-$L(X))\2,X K DIP
 W ?11 S X="" S $P(X,"=",42)="" W !,?(IOM-$L(X))\2,X K DIP
...
A1 ;
 ...
 W ?77 W !,"VAMC:  ",A1AXF K DIP
 W ?88 W !,"REVIEWER:  ",A1AXO K DIP
 W ?99 I A1AXO="JCAHO" W !,"PROGRAM:  ",A1AXT K DIP
 W ?110 W !,"REVIEW DATE:  ",A1AXDT W ?(IOM-$L(A1AXDP)),A1AXDP K DIP
 W ?121 X DXS(2,9) K DIP
 S I(2)="""S""",J(2)=11830.02 F D2=0:0 Q:$N(^DIZ(11830,D0,"P",D1,"S",D2))'>0  X:$D(DSC(11830.02)) DSC(11830.02) S D2=$N(^(D2)) Q:D2'>0  D:$X>132 T Q:'DN  D A2
 G A2R
A2 ;
 D T Q:'DN  W ?6 W !,A1AXL K DIP
...
A3 ;
 ...
 W ?11 S X="("_A1AXT_")" W ?15,X,?23,"FLAG: "_A1AXFG K DIP
