MBAARPCL ;OIT-PD/PB - Scheduling RPCs;08/27/2014
...
RECALL(RESULTS)  ;pull a list of recalls for a facility MBAA RPC: MBAA RECALL FACILITY LIST
 ...
 K XDT,XX,CNT,DFN,LNAME,SSN,CLINIC,CIEN,NODE,ACCESS,COMMENT,USER,TESTAPP,FASTNON,PATRECDT,PROVIDER,APPTLEN,SCEPRT,RECALLDT,REMINDDT,SECPRT
 Q
...
EWL(RESULTS,DFN) ; gets the EWL data by patient as described in the output data MBAA RPC: MBAA WAIT LIST BY DFN
 ...
 S REC=0,CNT=0 F  S REC=$O(^SDWL(409.3,"B",DFN,REC)) Q:REC'>0  D  ;ICR#: 6046 SDWL(409.3
 ...
 .I $G(CLINIEN)'="" D
 ..S PTR=$P(^SDWL(409.32,CLINIEN,0),"^")  ;ICR#: 6047 SDWL(409.32
 ..S CLINNAME=$P(^SC(PTR,0),"^"),STOPPTR=$P(^SC(PTR,0),"^",7),STPCODE=$P(^DIC(40.7,STOPPTR,0),"^")  ;ICR#: 1024, 6044
 ...
 .K DESIRED,CLINIEN,PTR,CLINNAME,STOPPTR,STPCODE,PRIOR,ENRSTAT,SC,ENRSTAT1,PRIOR1,DTREMOVE,ZIP
 K REC,CNT,REC,DFN
 Q
