A1AXGACO ;SLL/ALBANY ISC; CO ACTION PLAN GROUP APPROVAL; 1/20/88
 ;;VERSION 1.0
 ;
...
SERV ;
 ...
 ;
 ;RECOM LOOP
 I $D(^DIZ(11830,AA,"P",AP,"S",AS,"R"))=0 W !,"NO REOCMMENDATION DATA ENTERED" S Y=-1 Q
...
PR ;
 ...
 I A1AXAPC=""!(A1AXAPC["D") S A1AXV(AR)=ARN S A1AXM=A1AXM+1
 Q
EXCEPT ;
 S A1AXM=A1AXM-1
 W !!,"TOTAL ",A1AXM," ACTION PLANS NOT YET CO APPROVED" Q:A1AXM=0  W ", LISTED AS FOLLOWS:"
 W !,"     " S X=0 F Q=0:0 S X=$O(A1AXV(X)) Q:X=""  W X_","
N S %=1,U="^" W !,"DO YOU WANT TO APPROVE ALL ACTION PLANS" D YN^DICN Q:%=1  W:%Y["?" !,"    ANSWER 'YES' OR 'NO'" Q:%Y["^"  G:%<1 N
...
CK ;CK VALIDITY OF ENTRY
 ...
 I A1AXFG=0 S XY=X_" NOT ON THE CO UNAPPROVED LIST, ENTRY DELETED!" W *7,!?10,XY G ENT
...
N1 S %=1 W !,"ARE YOU SURE" D YN^DICN Q:%=2  W:%Y["?" !,"    ANSWER 'YES' OR 'NO'" Q:%Y["^"  G:%<1 N1
 S A1AX=0 F Q=0:0 S A1AX=$O(A1AXV(A1AX)) Q:A1AX=""  S $P(^DIZ(11830,AA,"P",AP,"S",AS,"R",A1AXV(A1AX),7),"^",2)=DT S $P(^(9),"^",3)="A" D MSG
 W !!,"ACTION PLAN APPROVAL COMPLETE."
 I $P(^DIZ(11833,1,0),"^",14)["Y" W !,"MAIL MSG SENT TO SITE!"
 I $P(^DIZ(11833,1,0),"^",15)["Y" W !,"MAIL MSG SENT TO CO!"
 Q
MSG ;
 S D0=AA,D1=AP,D2=AS,D3=A1AXV(A1AX)
 D:$P(^DIZ(11833,1,0),"^",14)["Y" ENST^A1AXAPCO
 D:$P(^DIZ(11833,1,0),"^",16)["Y" ENCO^A1AXAPCO
 Q
EXIT ;
 K AA,AC,AP,AS,AR,ARN,I,J,M,Q,X,Y,ER,XY,A1AXV,A1AX,A1AXM,A1AXFG,A1AXO,A1AXDT,A1AXCOF,A1AXRD,A1AXCDT,A1AXRDT,A1AXFF,A1AXRD,A1AXCO,A1AXAPR,A1AXAPC,%,D0,D1,D2,D3,A1AXRDF,A1AXF
 K %X,%Y,DIYS,%Y1,%H,%I,C,K,XMKK,XMLOCK,XMQF,XMR,XMT,XMTEXT,XMZ,DIC
 Q
