A1AXRAO ; GENERATED FROM 'A1AX PRAO' PRINT TEMPLATE (#107) ; 01/28/88 ; (FILE 11830, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 S:'$D(DN) DN=1
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(107,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
...
A1 ;
 ...
 W ?66 W !,"REVIEWER: ",A1AXO W ?(IOM-$L(A1AXDP)),A1AXDP K DIP
 D T Q:'DN  W ?4 X DXS(2,9) K DIP
 W ?15 W !,"VAMC: ",A1AXF K DIP
 W ?26 W !,"REVIEW DATE: ",A1AXDT K DIP
