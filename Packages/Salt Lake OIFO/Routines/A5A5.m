A5A5 ;FIX USER/PERSON STRUCTURE CJS/SLC ; 8/27/86  3:05 PM ;
 ;;V~2~
...
SECOND W !!,"Finding all last name matches.  Answer yes if they're the same."
 W !,"Checking through the USER file." S DIC="^DIC(16,",DIC(0)="EM",E1=0,N="",A5="W ""   matching to Person "",N1"
...
END W !,"Ready to do the x-referencing" S %=1 D YN^DICN G LEND:%=2!(%=-1)
 ...
 F DA=0:0 S DA=$O(^DIC(6,DA)) Q:DA'>0  I $D(^DIC(16,DA,0)) W "." S J=$P(^(0),"^",1) I $L(J) S ^DIC(16,"A6",J,DA)=""
