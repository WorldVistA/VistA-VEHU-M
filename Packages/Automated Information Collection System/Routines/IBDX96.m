IBDX96 ; GENERATED FROM 'IBD CREATE FORM TRACKING' INPUT TEMPLATE(#2007), FILE 357.96;05/04/04
 D DE G BEGIN
DE S DIE="^IBD(357.96,",DIC=DIE,DP=357.96,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^IBD(357.96,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(3)=% S %=$P(%Z,U,5) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,8) S:%]"" DE(6)=% S %=$P(%Z,U,9) S:%]"" DE(7)=%
 I  S %=$P(%Z,U,10) S:%]"" DE(8)=% S %=$P(%Z,U,11) S:%]"" DE(9)=% S %=$P(%Z,U,14) S:%]"" DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBDX96",DQ=1
 ...
 M DIEZAR=^DIE(2007,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2007,U="^"
...
6 S DW="0;8",DV="F",DU="",DLB="FORM TYPE ID (EXTERNAL SOURCE)",DIFLD=.08
 ...
 G RD:X="@",Z
X6 Q
7 S DW="0;9",DV="F",DU="",DLB="EXTERNAL PRINTED FORM ID",DIFLD=.09
 S DE(DW)="C7^IBDX96"
 S X=EXID
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C7F1 Q
X7 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW="0;10",DV="P44'",DU="",DLB="CLINIC",DIFLD=.1
 S DU="SC("
 S X=$G(CLIN)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X8 Q
9 S DW="0;11",DV="S",DU="",DLB="PROCESSING STATUS",DIFLD=.11
...
X10 I $G(^DPT(DFN,"S",APPT,0)) S Y="@99"
 Q
11 S DW="0;14",DV="S",DU="",DLB="NO APPOINTMENT ENTRY",DIFLD=.14
 S DE(DW)="C11^IBDX96"
