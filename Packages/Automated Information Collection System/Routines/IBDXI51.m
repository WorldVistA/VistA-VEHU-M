...
DE S DIE="^IBE(357.5,D0,2,",DIC=DIE,DP=357.52,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.5,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(4)=% S %=$P(%Z,U,4) S:%]"" DE(8)=% S %=$P(%Z,U,5) S:%]"" DE(6)=% S %=$P(%Z,U,6) S:%]"" DE(23)=% S %=$P(%Z,U,7) S:%]"" DE(25)=% S %=$P(%Z,U,8) S:%]"" DE(21)=%
 I  S %=$P(%Z,U,9) S:%]"" DE(12)=%,DE(17)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBDXI51",DQ=1+D G B
...
X19 S IBP=X
 Q
20 S DQ=21 ;@9
21 S DW="0;8",DV="NJ3,0",DU="",DLB="LENGTH OF DATA",DIFLD=.08
 S X=$G(IBLEN(IBP))
 S Y=X
 G Y
X21 K:+X'=X!(X>150)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
22 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=22 D X22 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X22 S IBW=+X
 Q
23 S DW="0;6",DV="NJ3,0XO",DU="",DLB="STARTING ROW FOR DATA",DIFLD=.06
 S DQ(23,2)="S Y(0)=Y S:Y=+Y Y=Y+1"
 S X=+IBY
 S Y=X
 G Y
X23 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
24 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=24 D X24 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X24 S IBY=X+1
 Q
25 S DW="0;7",DV="NJ3,0XO",DU="",DLB="STARTING COLUMN FOR DATA",DIFLD=.07
 S DQ(25,2)="S Y(0)=Y S:Y=+Y Y=Y+1"
 S X=+IBX
 S Y=X
 G Y
X25 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
