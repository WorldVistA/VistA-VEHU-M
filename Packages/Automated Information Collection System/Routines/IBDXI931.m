...
DE S DIE="^IBE(357.93,D0,1,",DIC=DIE,DP=357.931,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.93,D0,1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(13)=% S %=$P(%Z,U,3) S:%]"" DE(15)=% S %=$P(%Z,U,4) S:%]"" DE(12)=% S %=$P(%Z,U,5) S:%]"" DE(9)=% S %=$P(%Z,U,6) S:%]"" DE(17)=% S %=$P(%Z,U,7) S:%]"" DE(19)=% S %=$P(%Z,U,8) S:%]"" DE(1)=%
 I  S %=$P(%Z,U,9) S:%]"" DE(4)=%,DE(7)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X16 S IBY=X+1 I (X<$$MINY^IBDFU1B)!(X>$$MAXY^IBDFU1B) D WARNING^IBDFU1B("input field")
 Q
17 S DW="0;6",DV="RNJ3,0XO",DU="",DLB="BUBBLE COLUMN",DIFLD=.06
 S DQ(17,2)="S Y(0)=Y S Y=+Y+1"
 S X=IBX
 S Y=X
 G Y
...
X18 S IBX=X+5 I (X<$$MINX^IBDFU1B)!((X+2)>$$MAXX^IBDFU1B) D WARNING^IBDFU1B("bubble")
 Q
19 S DW="0;7",DV="RNJ3,0XO",DU="",DLB="BUBBLE ROW",DIFLD=.07
 S DQ(19,2)="S Y(0)=Y S Y=+Y+1"
 S X=IBY
 S Y=X
 G Y
X19 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
20 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=20 D X20 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X20 S IBY=X I (X<$$MINY^IBDFU1B)!(X>$$MAXY^IBDFU1B) D WARNING^IBDFU1B("bubble")
 Q
