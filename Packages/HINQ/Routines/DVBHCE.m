DVBHCE ; GENERATED FROM 'DVBHINQ UPDATE' INPUT TEMPLATE(#832), FILE 2;06/22/22
 D DE G BEGIN
...
BEGIN S DNM="DVBHCE",DQ=1
 ...
 M DIEZAR=^DIE(832,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=832,U="^"
1 S DQ=2 ;@101
...
C22S S X="" G:DG(DQ)=X C22F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C22F1 N X,X1,X2 S DIXR=399 D C22X1(U) K X2 M X2=X D C22X1("O") K X1 M X1=X
 D
...
C36S S X="" G:DG(DQ)=X C36F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C36F1 N X,X1,X2 S DIXR=400 D C36X1(U) K X2 M X2=X D C36X1("O") K X1 M X1=X
 D
 . N DIEXARR M DIEXARR=X S DIEZCOND=1
 . S X=X2(1)=""
 . S DIEZCOND=$G(X) K X M X=DIEXARR Q:'DIEZCOND
