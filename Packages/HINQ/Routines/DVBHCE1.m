...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".32;20",DV="RSX",DU="",DIFLD=.32945,DLB="Service NNTL Episode"
 S DE(DW)="C1^DVBHCE1"
 S DU="Y:YES;N:NO;"
 G RE
...
X2 I $P(^DPT(D0,.32),U,20)'="Y" S Y="@33"
 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".32;16",DV="RDX",DU="",DIFLD=.3297,DLB="NNTL-EOD"
...
X3 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER2^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=3 D POS^DGINP
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".32;17",DV="RDX",DU="",DIFLD=.3298,DLB="NNTL-RAD"
...
X4 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER2^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,1,"MSNNTL") X I $D(X),$D(^DG(43,1)) S SD1=3 D PS^DGINP
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".32;15",DV="P23'X",DU="",DIFLD=.3296,DLB="NNTL-Bran. Ser."
...
C5S S X="" G:DG(DQ)=X C5F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C5F1 N X,X1,X2 S DIXR=401 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 D
...
X5 S DFN=DA K:X=$O(^DIC(23,"B","B.E.C.","")) X I $D(X) D SER2^DGLOCK S DGCOMBR=$G(Y) Q
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".32;14",DV="RP25'X",DU="",DIFLD=.3295,DLB="NNTL-Char. Ser."
 S DE(DW)="C6^DVBHCE1"
 S DU="DIC(25,"
 G RE
...
X6 S DFN=DA D SER2^DGLOCK
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".32;18",DV="FX",DU="",DIFLD=.3299,DLB="NNTL-Ser. Num."
 S DE(DW)="C7^DVBHCE1"
 G RE
...
X11 S DVBJC2=$S($D(^DPT(D0,.32)):$P(^(.32),U,3),1:"")
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW=".32;3",DV="*P21'Xa",DU="",DIFLD=.323,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^DVBHCE1"
 S DU="DIC(21,"
 G RE
...
X20 S DVBSCR=1 D ^DVBHS5 S DVBJ2=1
 Q
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW=".361;1",DV="SX",DU="",DIFLD=.3611,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C21^DVBHCE1"
...
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW=".361;2",DV="DX",DU="",DIFLD=.3612,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C23^DVBHCE1"
 S X="TODAY"
 S Y=X
 G Y
...
X23 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) D EK^DGLOCK
 Q
 ;
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW=".361;5",DV="FX",DU="",DIFLD=.3615,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C24^DVBHCE1"
 S X="HINQ"
 S Y=X
 G Y
...
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,DW=".3;6",DV="DX",DU="",DIFLD=.306,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S X="TODAY"
 S Y=X
 G Y
X26 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) D EK^DGLOCK
 ...
 ;
27 S D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DVBHCE2",DC="3^2.0361IP^E^",DV="2.0361M*P8'Xa",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
...
X30 S DVBSCR=1 D ^DVBHS5 S DVBJ2=1
 Q
31 S DW="TYPE;1",DV="RP391'a",DU="",DIFLD=391,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C31F1 N X,X1,X2 S DIXR=636 D C31X1(U) K X2 M X2=X D C31X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
...
C31F2 Q
X31 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="VET;1",DV="RSXa",DU="",DIFLD=1901,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C32^DVBHCE1"
 S DU="Y:YES;N:NO;"
 G RE
...
X34 I X="N" S Y="@2063"
 Q
35 S DQ=36 ;@2063
36 D:$D(DG)>9 F^DIE17,DE S DQ=36,DW=".36;1",DV="*P8'Xa",DU="",DIFLD=.361,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C36S S X="" G:DG(DQ)=X C36F1 K DB
 D ^DVBHCE7
C36F1 N X,X1,X2 S DIXR=852 D C36X1(U) K X2 M X2=X D C36X1("O") K X1 M X1=X
 D
