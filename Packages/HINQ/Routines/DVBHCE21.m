...
1 S DW=".362;15",DV="SX",DU="",DIFLD=.36225,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^DVBHCE21"
...
X6 I DVBRETT=""!("BMCROX"'[DVBRETT) S Y="@63",JP=JP+1
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".362;18",DV="SX",DU="",DIFLD=.36285,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X12 S X=DVBRETO I X=""!(X=0) S X="@"
 Q
13 S DW=".362;8",DV="NJ8,2X",DU="",DIFLD=.3628,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S X=X
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X18 S X=DVBOINC I X=""!(X=0) S X="@"
 Q
19 S DW=".362;9",DV="NJ8,2X",DU="",DIFLD=.3629,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S X=X
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
