...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(3)=% S %=$P(%Z,U,5) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DVBHCE17",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".11;2",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 2]",DIFLD=.112
 S DE(DW)="C1^DVBHCE17",DE(DW,"INDEX")=1
 G RE
...
C1F1 N X,X1,X2 S DIXR=253 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 D
...
C1X1(DION) K X
 ...
 Q
C1F2 Q
X1 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;3",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 3]",DIFLD=.113
 S DE(DW)="C2^DVBHCE17",DE(DW,"INDEX")=1
 G RE
...
C2F1 N X,X1,X2 S DIXR=254 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 D
...
X2 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X
 ...
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;4",DV="Fa",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C3^DVBHCE17",DE(DW,"INDEX")=1
 G RE
...
C3S S X="" G:DG(DQ)=X C3F1 K DB
 D ^DVBHCE18
C3F1 N X,X1,X2 S DIXR=255 D C3X1(U) K X2 M X2=X D C3X1("O") K X1 M X1=X
 D
...
X3 K:$L(X)>15!($L(X)<2) X
 ...
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;5",DV="*P5'a",DU="",DLB="STATE",DIFLD=.115
 S DE(DW)="C4^DVBHCE17",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C4S S X="" G:DG(DQ)=X C4F1 K DB
 D ^DVBHCE20
C4F1 N X,X1,X2 S DIXR=256 D C4X1(U) K X2 M X2=X D C4X1("O") K X1 M X1=X
 D
...
C4X1(DION) K X
 ...
 Q
C4F2 Q
X4 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 D:$D(DG)>9 F^DIE17 G ^DVBHCE21
