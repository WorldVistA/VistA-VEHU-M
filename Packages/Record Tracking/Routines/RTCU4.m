...
DE S DIE="^RTV(190.1,",DIC=DIE,DP=190.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(190.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,7) S:%]"" DE(4)=% S %=$P(%Z,U,8) S:%]"" DE(5)=% S %=$P(%Z,U,10) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="0;2",DV="RD",DU="",DLB="DATE/TIME REQUESTED",DIFLD=2
 S DE(DW)="C1^RTCU4"
 S X=RTNOW
 S Y=X
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;3",DV="P200'",DU="",DLB="USER REQUESTING RECORD",DIFLD=3
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 Q
3 S DW="0;6",DV="S",DU="",DLB="REQUEST STATUS",DIFLD=6
 ...
 G Z:X,RD
X3 Q
4 S DW="0;7",DV="D",DU="",DLB="DATE/TIME CURRENT STATUS",DIFLD=7
 S X=RTNOW
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X4 Q
5 S DW="0;8",DV="P200'",DU="",DLB="USER RESPONSIBLE FOR STATUS",DIFLD=8
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X5 Q
6 S DW="0;10",DV="P194.2'",DU="",DLB="PULL LIST",DIFLD=10
 S DE(DW)="C6^RTCU4"
 S DU="RTV(194.2,"
