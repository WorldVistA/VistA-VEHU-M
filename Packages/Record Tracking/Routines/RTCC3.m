...
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RT(DA,""))=""
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,1) S:%]"" DE(3)=% S %=$P(%Z,U,15) S:%]"" DE(4)=%
 I $D(^("COMMENT")) S %Z=^("COMMENT") S %=$P(%Z,U,1) S:%]"" DE(5)=%,DE(8)=%
...
X1 S %DT="X" D ^%DT S X=Y K:Y<1 X
 ...
 ;
2 S DQ=3 ;@18
3 S DW="CL;1",DV="*P190.1'",DU="",DLB="ASSOCIATED REQUEST",DIFLD=101
 S DU="RTV(190.1,"
 S X=$S($D(RTQ):RTQ,1:"")
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
4 S DW="CL;15",DV="S",DU="",DLB="LAST TIME SELECTED BY BARCODE?",DIFLD=115
 ...
 G Z:X,RD
X4 Q
5 S DW="COMMENT;1",DV="F",DU="",DLB="TEMPORARY COMMENT",DIFLD=75
