...
DE S DIE="^RTV(190.3,",DIC=DIE,DP=190.3,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(190.3,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,8) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(2)=% S %=$P(%Z,U,15) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="0;8",DV="R*P195.3'I",DU="",DLB="TYPE OF MOVEMENT",DIFLD=8
 S DU="DIC(195.3,"
 S X=$P(RTM,U,8)
 S Y=X
 ...
 G Z:X,RD
X1 Q
2 S DW="0;14",DV="*P195.9IX",DU="",DLB="ASSOCIATED BORROWER",DIFLD=14
 S DU="RTV(195.9,"
 S X=$P(RTM,U,14)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 Q
3 S DW="0;15",DV="S",DU="",DLB="SELECTION BY BARCODE?",DIFLD=15
 S DU="y:YES;n:NO;"
 S X=$P(RTM,U,15)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 S I(0,0)=D0 S Y(1)=$S($D(^RTV(190.3,D0,0)):^(0),1:"") S X=$P(Y(1),U,1),X=X S D(0)=+X S X=$S(D(0)>0:D(0),1:"")
 S DGO="^RTCM2",DC="^190^RT(" G DIEZ^DIE0
R4 D DE G A
 ;
5 G 0^DIE17
