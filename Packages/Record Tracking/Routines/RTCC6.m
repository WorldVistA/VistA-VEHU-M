RTCC6 ; ;12/16/91
 D DE G BEGIN
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^RT(DA,-1))<0
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,7) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(2)=%
 I $D(^("I")) S %Z=^("I") S %=$P(%Z,U,1) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="RTCC6",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="CL;7",DV="P200'",DU="",DLB="USER THAT CHARGED RECORD",DIFLD=107
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 ...
 G Z:X,RD
X1 Q
2 S DW="CL;8",DV="*P195.3'",DU="",DLB="TYPE OF MOVEMENT",DIFLD=108
 S DU="DIC(195.3,"
 S X=RTMV
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 S DIC("S")="I $D(D0),$P(^RT(D0,0),U,4)=$P(^DIC(195.3,Y,0),U,3)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
3 S DW="I;1",DV="D",DU="",DLB="INACTIVATION DATE",DIFLD=50
 S X=$S('$D(RTINACFL):0,1:RTINACFL),X=$S(X:"T-1",$P(RTMV0,"^")'["CHECK-IN":"",'$D(^DIC(195.2,+$P(RT0,"^",3),0)):0,$P(^(0),"^",15)="y":"T-1",1:"")
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X3 S %DT="X" D ^%DT S X=Y K:Y<1 X
 Q
 ;
