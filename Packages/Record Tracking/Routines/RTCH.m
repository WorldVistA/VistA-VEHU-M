RTCH ; GENERATED FROM 'RT HOME LOCATION' PRINT TEMPLATE (#570) ; 08/25/94 ; (FILE 190, MARGIN=132)
 G BEGIN
CP G CP^DIO2
C S DQ(C)=Y
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(570,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
