...
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,7) S:%]"" DE(1)=% S %=$P(%Z,U,12) S:%]"" DE(2)=%
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,5) S:%]"" DE(3)=% S %=$P(%Z,U,6) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(5)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;7",DV="RNJ2,0I",DU="",DLB="VOLUME NUMBER",DIFLD=7
 S X=RTVOL
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X1 Q
2 S DW="0;12",DV="F",DU="",DLB="CONTENT DESCRIPTOR",DIFLD=12
 S X=$S($D(RTACCN):RTACCN,1:"")
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X2 Q
3 S DW="CL;5",DV="R*P195.9X",DU="",DLB="CURRENT BORROWER/FILE ROOM",DIFLD=105
 S DE(DW)="C3^RTCR1"
 S DU="RTV(195.9,"
 S X=$P(RTINIT,U,5)
 S Y=X
...
C3S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="CL;6",DV="D",DU="",DLB="DATE/TIME CHARGED TO BORROWER",DIFLD=106
 S DE(DW)="C4^RTCR1"
 S X=RTNOW
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C4S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X4 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="CL;7",DV="P200'",DU="",DLB="USER THAT CHARGED RECORD",DIFLD=107
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
