RTCU6 ; ;12/16/91
 D DE G BEGIN
DE S DIE="^RTV(190.1,",DIC=DIE,DP=190.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^RTV(190.1,DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,8) S:%]"" DE(1)=% S %=$P(%Z,U,10) S:%]"" DE(2)=% S %=$P(%Z,U,11) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="RTCU6",DQ=1
1 S DW="0;8",DV="P200'",DU="",DLB="USER RESPONSIBLE FOR STATUS",DIFLD=8
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 ...
 G Z:X,RD
X1 Q
2 S DW="0;10",DV="P194.2'",DU="",DLB="PULL LIST",DIFLD=10
 S DE(DW)="C2^RTCU6"
 S DU="RTV(194.2,"
...
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;11",DV="P190.1'",DU="",DLB="PARENT REQUEST",DIFLD=11
 S DE(DW)="C3^RTCU6"
 S DU="RTV(190.1,"
