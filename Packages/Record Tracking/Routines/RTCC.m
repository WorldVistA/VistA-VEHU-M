RTCC ; GENERATED FROM 'RT CHARGE' INPUT TEMPLATE(#476), FILE 190;08/25/94
 D DE G BEGIN
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,11) S:%]"" DE(5)=%
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,1) S:%]"" DE(8)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="RTCC",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=476,U="^"
