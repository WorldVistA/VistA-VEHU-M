...
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(11)=% S %=$P(%Z,U,12) S:%]"" DE(9)=%
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,8) S:%]"" DE(1)=% S %=$P(%Z,U,15) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="CL;8",DV="*P195.3'",DU="",DLB="TYPE OF MOVEMENT",DIFLD=108
 S DU="DIC(195.3,"
 S X=$O(^DIC(195.3,"AA",+RTAPL,$S('$D(RTRANEW):"INITIAL CREATION",1:"TRANSFER INITIAL CREATION"),0))
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X1 Q
2 S DW="CL;15",DV="S",DU="",DLB="LAST TIME SELECTED BY BARCODE?",DIFLD=115
 S DU="y:YES;n:NO;"
 S X="n"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X9 K:$L(X)>20!($L(X)<2) X
 ...
 ;
10 S DQ=11 ;@10
11 S DW="0;6",DV="R*P195.9'X",DU="",DLB="HOME LOCATION",DIFLD=6
 S DE(DW)="C11^RTCR2"
 S DU="RTV(195.9,"
 G RE
...
C11S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X11 S DIC("V")="I $P(Y(0),U,4)=""L""",DIC("S")="I $D(D0),$D(^DIC(195.2,""AF"",Y,+$P(^RT(D0,0),U,3))) D DICS^RTDPA31" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
12 D:$D(DG)>9 F^DIE17 G ^RTCR3
