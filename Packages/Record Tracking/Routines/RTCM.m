RTCM ; GENERATED FROM 'RT MOVEMENT' INPUT TEMPLATE(#478), FILE 190.3;08/25/94
 D DE G BEGIN
DE S DIE="^RTV(190.3,",DIC=DIE,DP=190.3,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(190.3,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(3)=% S %=$P(%Z,U,5) S:%]"" DE(4)=% S %=$P(%Z,U,6) S:%]"" DE(5)=% S %=$P(%Z,U,7) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="RTCM",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=478,U="^"
