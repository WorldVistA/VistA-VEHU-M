ORPDMP ;SLC/AGP - PDMP Code ;Oct 14, 2020@15:32:38
...
STRTPDMP(ORRESULTS,ORUSER,ORCOSIGNER,DFN,ORVSTR) ;
 ...
 . S ^TMP(ORSUB,$J,0)="-1"
 . S ^TMP(ORSUB,$J,1)="Your VistA account does not have your domain email address defined. You need to have it defined to make a PDMP query as a delegate."
 . S ^TMP(ORSUB,$J,2)=" "
...
UPDATELOG(ORIENS,ORSTATUS,ORSHARED,ORQSTATS,ORVIEWED,ORERRINFO,ORSESSION,ORNOTE,ORNOTESTAT) ;
 ;
 N ORFDA
 ;
 I $G(ORSTATUS)'="" S ORFDA(101.621,ORIENS,.04)=ORSTATUS
...
VIEWEDREPORT(ORRESULT,ORLOGIENS,ORSTATUS,ORERRINFO) ;
 ;
 N ORD0,ORD1,ORI,ORLINE,ORLOGERR
 ;
 I $G(ORLOGIENS)="" D  Q
 ...
 ;
 D UPDATELOG(ORLOGIENS,"","","",ORSTATUS,"ORLOGERR")
 S ORRESULT=1
 ;
 Q
 ;
...
HASEMAIL(ORUSER) ;
 ...
 ;
 ;
 ; Return a user's domain email address (only look at #200,#205.5 as it's standardized by IAM)
GETEMAIL(ORRESULT,ORUSER) ;
 ...
 I $$LOW^XLFSTR(OREMAIL)["@domain" D  Q
 . S ORRESULT=OREMAIL
 ;
 Q
 ;
