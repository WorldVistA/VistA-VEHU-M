A4AARPT6 ;HINES-CIOFO/JJM - COMPUTES A4ATNA 3/1/99 15:30
...
START ;
 D DEBTOR^A4AARPT
 I Y=-1 D EXIT QUIT
PRTOPT ; ASK FOR DETAIL OR SUMMARY LIST
 S FLAG1=0
 S DIR("A")="DO YOU WANT A SUMMARY LIST ONLY",DIR(0)="Y",DIR("B")="YES" D ^DIR
 S:(X["N")!(X["n") FLAG1=1
ASKDEV ;
 S %ZIS="QM",%ZIS("B")=""
 D ^%ZIS
...
EN1 S:'$D(FLAG1) FLAG1=0
 S (A4ATL1,A4ATL2,A4ATL3,A4ATL4,A4ATL5)=0
 S:'$G(DEBTOR) DEBTOR=A4ADEB
 Q:'$G(DEBTOR)
 D CRT
 D PCLDFCS^A4AARPT5
 D PCLDFAR^A4AARPT5
 W !!,"ACCOUNT IS ",$S(DFLAG=1:"OUT OF BALANCE",1:"IN BALANCE"),!!
 D EOR6
 QUIT
CRT ; COMPUTE REPORT TOTALS
 S:'$G(DEBTOR) DEBTOR=A4ADEB
 Q:'$G(DEBTOR)
 N TRANS,DATE1,DATE2,TIME1,TIME2,BS
 N LST1,LST2,LST3,LST4,LST5,LST6
 S (RBAL,NATOTAL,SRBAL,A4APSB)=0,BS=" "
 ...
 F BILL=0:0 S BILL=$O(^PRCA(430,"C",DEBTOR,BILL)) Q:'BILL  D
   ...
   . D PRTTRAN2
   . S:(CS=16)!(CS=42) A4ATOA=A4ATOA+BAL
   . F TRANS=0:0 S TRANS=$O(^PRCA(433,"C",BILL,TRANS)) Q:'TRANS  D PRTTRAN3
 S A4ATL6=A4ATOA
 S A4ATL7=A4APSB
 S A4ATL8=NATOTAL
...
PRTTRAN2 ;
 ...
 I NAF=("*")&(SFLAG="") S NATOTAL=NATOTAL+TAMT S A4ATNA=NATOTAL
 I NAF'=("*")&(SFLAG="") S SRBAL=SRBAL+TAMT S A4APSB=SRBAL
 S:NAF'="*" LST1=LST1+TAMT
 I SFLAG="" S RBAL=RBAL+TAMT S A4AFRB=RBAL
 S TYPE=$P($G(^PRCA(430.3,+$P(^PRCA(430,BILL,0),U,8),0)),U,2)
