SDESCKNSTEP ;ALB/BLB,TAW - VISTA SCHEDULING RPCS ;Jun 25, 2021@15:22
 ;;5.3;Scheduling;**788,790,792,800**;Aug 13, 1993;Build 23
...
SETAPPTCKNSTEP(SDECY,IEN,STATUS,EAS) ; File check-in status and date/time in 409.843,.01/1
 ...
 I '$G(IEN) S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,14)  D BUILDER Q 
 I '$D(^SDEC(409.84,IEN,0))  S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,15)  D BUILDER Q  ;invalid appt IEN
 I $G(STATUS)="" S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,38)  D BUILDER Q 
 I '$D(^SDEC(409.842,STATUS,0))  S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,37)  D BUILDER Q
 I $$GET1^DIQ(409.84,IEN,.17,"E")="C" S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,31)  D BUILDER Q  ;appt status is "C" (canceled)
 ...
 I STATPOINTER=STATUS S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,32)  D BUILDER Q  ; duplicate status entry, will not file
...
GETAPPTCKNSTEPS(SDECY,IEN) ; Get a list of checkin statuses associated with an appointment IEN
  ...
  .D BUILDER
  I '$D(SDCHECKIN("ApptCheckInSteps")) S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,33) D BUILDER Q  ;no statuses found, will not pass back list
  Q
SETCKNSTEP(SDECY,STATUS) ;create new status entry in check-in step status file
 N FDA,SDCHECKIN,IEN,ENTRY
 I $G(STATUS)="" S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,38)  D BUILDER Q
 I $L(STATUS)<3!($L(STATUS)>30) S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,39)  D BUILDER Q
 I STATUS'?.E1A.E S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,41)  D BUILDER Q
 S IEN=0,ENTRY=""
 F  S IEN=$O(^SDEC(409.842,IEN)) Q:'IEN!(STATUS=ENTRY)  D
 .S ENTRY=$$GET1^DIQ(409.842,IEN,.01,"E")
 I STATUS=ENTRY S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,32)  D BUILDER Q  ; duplicate status entry, will not file
 S FDA(409.842,"+1,",.01)=STATUS
 D UPDATE^DIE(,"FDA",,"ERR") K FDA,ERR
 I '$D(ERR) S SDCHECKIN("CheckInSteps",0,"FilingSuccess")=1 D BUILDER Q
 I $D(ERR) D ERRLOG^SDESJSON(.SDCHECKIN,34)  D BUILDER
 Q
EDITCKNSTEP(SDECY,IEN,STATUS) ; edit entry in check-in step status file
 N FDA,SDCHECKIN,ENTRY,SIEN
 I '$G(IEN) S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,40)  D BUILDER Q
 I $G(STATUS)="" S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,38)  D BUILDER Q
 I STATUS'="@",$L(STATUS)<3!($L(STATUS)>30) S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,39)  D BUILDER Q
 I STATUS'="@",STATUS'?.E1A.E S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,41)  D BUILDER Q
 I '$D(^SDEC(409.842,IEN,0)) S SDCHECKIN("ApptCheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,37)  D BUILDER Q
 S SIEN=0,ENTRY=""
 ...
 I '$D(ERR) S SDCHECKIN("CheckInSteps",0,"FilingSuccess")=1 D BUILDER Q
 I $D(ERR) D ERRLOG^SDESJSON(.SDCHECKIN,35)  D BUILDER
 Q
GETCKNSTEP(SDECY,IEN) ;get status from check-in step status file
 ...
 I $G(STATUS)="" S SDCHECKIN("CheckInSteps")="" D ERRLOG^SDESJSON(.SDCHECKIN,37)  D BUILDER Q
 D BUILDER
 Q
GETCKNSTEPS(SDECY) ;
 N SDCHECKIN,IEN,NUM
...
CKNBUILDER(IEN,STATUS,DATETIME,TAG) ;
 ...
 .S SDCHECKIN("ApptCheckInSteps",1,"EASTrackingNumber")=$G(EAS)
