SDAMXLD ; GENERATED FROM 'SDAMVLD' PRINT TEMPLATE (#1189) ; 01/04/99 ; (FILE 409.65, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1189,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
