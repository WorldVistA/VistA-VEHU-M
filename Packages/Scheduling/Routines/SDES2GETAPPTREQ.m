SDES2GETAPPTREQ ;ALB/BWF,JAS,BWF,JAS,LAB,BLB,LAB,RN,AGW - SDES2 GET APPT REQ RPCS ; JUL 10,2026
 ;;5.3;Scheduling;**873,877,878,880,890,893,908,922,909,941,951**;Aug 13, 1993;Build 5
...
GETREQBYTYPEVET(JSONRETURN,SDCONTEXT,SDINPUT) ;GET APPT REQ BY TYPE VET
 N ERRORS,REQUEST,COUNT,REQDATE,REQUESTIEN,MAXNUMREQS
 S REQUEST=$NA(^TMP("SDES2GETAPPTREQ",$J,"REQUEST")) K @REQUEST
 S JSONRETURN=$NA(^TMP("SDES2GETAPPTREQ",$J,"JSON")) K @JSONRETURN
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("Request",1)="" D ENCODE^XLFJSON("ERRORS",.JSONRETURN) Q
 I $G(SDCONTEXT("USER DUZ"))'="" N DUZ D DUZ^XUP(SDCONTEXT("USER DUZ"))
 S MAXNUMREQS=$G(SDINPUT("MAX REQ"))
 S:(MAXNUMREQS="")!(MAXNUMREQS=0) MAXNUMREQS=400
 I MAXNUMREQS'?1.4N D  Q
 .D ERRLOG^SDES2JSON(.ERRORS,52," Max records must be a 1 to 4 digit number")
 .S ERRORS("Request",1)="" D ENCODE^XLFJSON("ERRORS",.JSONRETURN)
 .Q
 I (MAXNUMREQS'<1001) D  Q
 .D ERRLOG^SDES2JSON(.ERRORS,52," Max records must be less than 1001")
 .S ERRORS("Request",1)="" D ENCODE^XLFJSON("ERRORS",.JSONRETURN)
 .Q
 S COUNT=0
 S REQUEST=$NA(^TMP("SDES2GETAPPTREQ",$J,"REQUEST")) K @REQUEST
 S JSONRETURN=$NA(^TMP("SDES2GETAPPTREQ",$J,"JSON")) K @JSONRETURN
 S REQDATE=0 F  S REQDATE=$O(^SDEC(409.85,"E","O",REQDATE)) Q:'REQDATE!(MAXNUMREQS=COUNT)  D
 .S REQUESTIEN=0 F  S REQUESTIEN=$O(^SDEC(409.85,"E","O",REQDATE,REQUESTIEN)) Q:'REQUESTIEN!(MAXNUMREQS=COUNT)  D
 ..I '$D(^SDEC(409.85,"TYPE","VETERAN",REQUESTIEN)) Q
 ..S COUNT=COUNT+1
 ..D GETREQUESTTMP^SDES2UTILAREQTMP(.REQUEST,REQUESTIEN,COUNT)
 ..Q
 .Q
 D ENCODE^XLFJSON(.REQUEST,.JSONRETURN)
 K @REQUEST
 Q
 ;
...
GETREQUEST(REQUEST,REQUESTIEN) ;
 N ERR,IENS,FN,REQDATA,NUM,CLINSTOPIEN,CLINCREDIEN,DFN,SENSITIVE,CONTACTIEN,PRIOGROUP
 S FN=409.85,NUM=""
