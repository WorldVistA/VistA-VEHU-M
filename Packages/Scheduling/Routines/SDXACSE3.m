...
DE S DIE="^SDV(D0,""CS"",",DIC=DIE,DP=409.51,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^SDV(D0,"CS",DA,""))=""
 I $D(^("PR")) S %Z=^("PR") S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(3)=% S %=$P(%Z,U,5) S:%]"" DE(5)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="SDXACSE3",DQ=1
1 S DW="PR;3",DV="*P409.71'X",DU="",DLB="PROCEDURE 3",DIFLD=23
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,5)
 S Y=X
...
X2 I $P(SDCPT(SDJ),U,6)']"" S Y="@99"
 Q
3 S DW="PR;4",DV="*P409.71'X",DU="",DLB="PROCEDURE 4",DIFLD=24
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,6)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X4 I $P(SDCPT(SDJ),U,7)']"" S Y="@99"
 Q
5 S DW="PR;5",DV="*P409.71'X",DU="",DLB="PROCEDURE 5",DIFLD=25
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,7)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
