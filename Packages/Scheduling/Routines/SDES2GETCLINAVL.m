...
CLINICAVAIL(RESULT,SDCONTEXT,SDINPUT) ;
 N CLINLIST,ERRORS,CLINAVAIL,STARTDTFM,ENDDTFM,PRIMARYSTOP,SECSTOPLIST,NUMBERRET
 K ^TMP("SDES2CLINAVAIL",$J)
 S CLINLIST=$NA(^TMP("SDES2CLINAVAIL",$J,"CLINLIST")) K @CLINLIST
 ...
 D GETAVAIL(.CLINAVAIL,.CLINLIST,STARTDTFM,ENDDTFM,$G(SDINPUT("STATION")),NUMBERRET)
 I '$D(CLINAVAIL) S CLINAVAIL("ClinicAvailability",1)=""
...
VALIDATE(ERRORS,SDINPUT,STARTDTFM,ENDDTFM,NUMBERRET) ;
 ...
 S NUMBERRET=$G(SDINPUT("NUMBER RETURNED"))
 I '$G(NUMBERRET) S (NUMBERRET,SDINPUT("NUMER RETURNED"))=100
...
GETAVAIL(RETURN,CLINLIST,START,END,STATION,NUMBER) ;
 ...
 .S GLOB=$NA(^TMP("SDES2CLINAVAIL",$J,CLIN)) K @GLOB
 .S BADATE=0
 ...
 ..D RESAB^SDEC57(GLOB,CLIN,AVAILDATE,AVAILDATE_"."_2359) Q:'$D(^TMP("SDES2CLINAVAIL",$J,CLIN))
 ..S LOOP=0 F  S LOOP=$O(^TMP("SDES2CLINAVAIL",$J,CLIN,LOOP)) Q:'LOOP!(DONE)  D
 ...S APPTDAT=$G(^TMP("SDES2CLINAVAIL",$J,CLIN,LOOP))
 ...
 ..I TOTALAVAILCNT>=NUMBER S DONE=1 Q
 Q
...
VALNUM(ERRORS,NUMBERRET) ;
 I $G(NUMBERRET)<1!($G(NUMBERRET)>100) D ERRLOG^SDESJSON(.ERRORS,382)
 Q
VISN(INSTNUM) ;
 N IEN,VISNPTR,VISN
 S VISN=0
