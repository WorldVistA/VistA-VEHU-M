SDESBLKANDMOVE ;ALB/MGD/TAW - BLOCK AND MOVE ;Dec 06, 2021
 ;;5.3;Scheduling;**797,799,800,801**;Aug 13, 1993;Build 13
...
APPTBLOCKMOVE(RETURN,APPTIEN,TORES,TODTNET,EASTRCKNGNMBR) ;
 ...
 N FROMDTFM,FROMDTNET,TIMESCALEDIFF,FROMRES,SDSLOT2BLK,FROMTIMESCALE,SDORGCLIEN,TOENDDTFM
 ...
 I 'POP D CHKAVAILABILITY(TORES,SDTOCLIEN,TODTNET,"T",TODTFM)
 I 'POP D CHKAVAILABILITY(FROMRES,SDORGCLIEN,FROMDTNET,"F",FROMDTFM)
...
LOADAPPTDATA ;Load variables used throughout B&M
 ...
 S SDPID=$$GET1^DIQ(409.831,TORES,.2,"E")
 ; Load Resource & Clinic info
 S SDDATA44SLFROM=$G(^SC(SDORGCLIEN,"SL"))
 S CLINBEGFROM=$$PADCLTIME^SDESUTIL($P(SDDATA44SLFROM,U,3))
 S SDDATA44SLTO=$G(^SC(SDTOCLIEN,"SL"))
 S CLINBEGTO=$$PADCLTIME^SDESUTIL($P(SDDATA44SLTO,U,3))
...
SEGARRAY(CLIEN,TIMESCALE,FROMDTFM,CLINBEG,CLINID) ; Create local array to hold segments for validation comparisons
 N PATRNIEN,SDDOW,SDINDX,SDENDTIME,SDSLDATA,STOP,TINDX
