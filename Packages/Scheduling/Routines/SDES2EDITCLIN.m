SDES2EDITCLIN ;ALB/BWF,MGD/BLB,BWF,TJB,JAS,MGD,TJB,JHC - SDES2 EDIT CLINIC ;JUN 12, 2026
 ;;5.3;Scheduling;**853,857,864,866,871,885,890,893,895,914,909,949**;Aug 13, 1993;Build 4
...
EDITCLINIC(RESULTS,SDCONTEXT,SDCLINIC) ;
 ...
 I $D(ERRORS) S ERRORS("ClinicEdit",1)="",ERRORS("Clinic",1)="" D BUILDJSON^SDES2JSON(.RESULTS,.ERRORS) Q
 ...
 D VALSTARTHOUR^SDES2VAL44(.ERRORS,$G(SDCLINIC("HOUR CLINIC DISPLAY BEGINS")),.FILEDATA)
 I $D(ERRORS) S ERRORS("ClinicUpdate",1)="",ERRORS("Clinic",1)="" D BUILDJSON^SDES2JSON(.RESULTS,.ERRORS) Q
 ;
 ...
 I $D(ERRORS) D  Q
 .S ERRORS("ClinicEdit",1)="",ERRORS("Clinic",1)="" D BUILDJSON^SDES2JSON(.RESULTS,.ERRORS)
 ;
 ...
 ;
 N SDRETURN
 D BLDCLNREC^SDES2CLININFO(.SDRETURN,$G(SDCLINIC("CLINIC IEN")))
 S SDRETURN("ClinicEdit","ID")=SDIEN
 D BUILDJSON^SDES2JSON(.RESULTS,.SDRETURN)
 Q
FILECLINDATA(ERRORS,FILEDATA,SDIEN) ;
 ...
 I $D(FILEDATA("HOUR CLINIC DISPLAY BEGINS")) S FDA(44,SDIENS,1914)=$G(FILEDATA("HOUR CLINIC DISPLAY BEGINS"))
 I $D(FDA) D
