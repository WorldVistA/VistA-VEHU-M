SDES2VAL44 ;ALB/BWF,MGD/BLB/TAW,TJB,JHC - SDES2 Clinic validation utilities ;JUN 16, 2026
 ;;5.3;Scheduling;**853,857,864,866,871,890,914,930,949**;Aug 13, 1993;Build 4
...
VALPATFNAME(ERRORS,SDPATNAME,FDATA) ;
 ...
 Q
 ;
VALMEETATFACIL(ERRORS,SDMEETATFAC,FDATA) ;
 ...
 Q
 ;
VALDIRPATSCHED(ERRORS,SDPATSCHED,FDATA) ;
 ...
 Q
 ;
VALDISPAPPT(ERRORS,SDCLINIC,FDATA) ;
 N SDDISPAPPT
...
VALDIV(ERRORS,SDCLINIC,FDATA) ;
 ...
 Q
 ;
VALINCPERHOUR(ERRORS,SDCLINIC,FDATA) ;
...
VALPROFILE(ERRORS,SDCLINIC,FDATA) ; required - cannot be deleted with '@' or "" - can be changed
 ...
 I SDREQPROFILE?1N S FDATA("REQUIRE ACTION PROFILES")='SDREQPROFILE
 Q
 ;
VALNOSHOWLET(ERRORS,SDCLINIC,FDATA) ;
 ...
 S SDNOSHOWLET=$$LETTER(.NOSHOWERRORS,SDNOSHOWLET)
...
VALPREAPTLET(ERRORS,SDCLINIC,FDATA) ;
 ...
 S SDPREAPTLET=$$LETTER(.PREAPTERRORS,SDPREAPTLET)
...
VALCANLET(ERRORS,SDCLINIC,FDATA) ;
 ...
 S SDCANLET=$$LETTER(.CANLETERRORS,SDCANLET)
...
VALAPTCANLET(ERRORS,SDCLINIC,FDATA) ;
 ...
 S SDAPTCANLET=$$LETTER(.CANLETERRORS,SDAPTCANLET)
...
VALINOUTTIME(ERRORS,SDCLINIC,FDATA) ;
 ...
 Q
 ;
VALSTARTHOUR(ERRORS,SDSTARTHR,FDATA) ;
 I SDSTARTHR'=""&(+SDSTARTHR'=SDSTARTHR) D ERRLOG^SDES2JSON(.ERRORS,119) Q
 S SDSTARTHR=$S(SDSTARTHR="":8,+SDSTARTHR>=0:+SDSTARTHR,1:-1)
 I (SDSTARTHR<0)!(SDSTARTHR>16) D ERRLOG^SDES2JSON(.ERRORS,119) Q
...
CONDAMISCHECK(ERRORS,SDPRIMAMIS,SDCREDITAMIS) ;
 ...
 Q
 ;
LETTER(ERRORS,LETTER) ;Look up the letter IEN
 I LETTER'="@",LETTER'="" D
