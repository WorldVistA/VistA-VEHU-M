SDES2VAL44 ;ALB/BWF - SDES2 Clinic validation utilities ;JUL 18, 2023
...
VALSERVICE(ERRORS,SDCLINIC,FDATA) ; required, cannot be deleted with @ or "" - can be changed
 N SDSERVICE,SDSERVICEEXT
 S SDSERVICE=$G(SDCLINIC("SERVICE"))
 I SDSERVICE="@" D ERRLOG^SDES2JSON(.ERRORS,229,"Service") Q
 I SDSERVICE="" D ERRLOG^SDES2JSON(.ERRORS,92) Q
 S SDSERVICE=$E(SDSERVICE)
 S:SDSERVICE="N" SDSERVICE=0
 S SDSERVICEEXT=$$SOCINT2EXT^SDES2UTIL(44,9,SDSERVICE)
 I SDSERVICEEXT="" D ERRLOG^SDES2JSON(.ERRORS,93) Q
 S FDATA("SERVICE")=SDSERVICE
 Q
 ;
...
VALDIV(ERRORS,SDCLINIC,FDATA) ; required field, cannot be deleted with @ or "" - can be changed
 ...
 I +SDDIVISION,$D(^DG(40.8,SDDIVISION,0)) Q
 S SDDIVISION=$O(^DG(40.8,"B",$E(SDDIVISION,1,30),""),-1)
...
VALAPPTTYPE(ERRORS,SDCLINIC,FDATA) ;
 ...
 I 'SDAPPTTYP D ERRLOG^SDES2JSON(.ERRORS,83)
...
VALAPPTLENGTH(ERRORS,SDLENGTHOFAPT,FDATA) ; required, cannot be deleted with '@' or "" - can be changed
 ...
 .I (SDLENGTHOFAPT#10=0)!(SDLENGTHOFAPT#15=0) Q
 .D ERRLOG^SDES2JSON(.ERRORS,116) Q
