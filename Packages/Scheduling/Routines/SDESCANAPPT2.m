SDESCANAPPT2 ;ALB/DJS,LAB,MGD - SCHEDULING CANCEL APPOINTMENTS RPC ;Mar 3, 2023
 ;;5.3;Scheduling;**838,842**;Aug 13, 1993;Build 17
...
CANCELAPPT2(JSONRETURN,APPTIEN,CLINICIEN,DFN,CANBYCLINORPAT,CANCELREASON,NOTE,CANCELHASH,EAS) ;
 ...
 N RECALLREQIEN,RECALLREQLINK,RESOURCE,OLDRECALLPTR,RETURN,RECALLRET,CANCELREASONIEN,APPTLENGTH
 ;
 ...
 ;
 S IS40984CANCELLED=$$CANCEL40984(.ERRORS,$G(APPTIEN),$G(CANCELREASON),$G(CANBYCLINORPAT),$G(EAS))
 ;
 S IS44CANCELLED=$$CANCEL44(.ERRORS,$G(CLINICIEN),$G(APPTSTARTTIME),$G(DFN),$G(APPTIEN))
 ;
 S IS2CANCELLED=$$CANCEL2(.ERRORS,$G(DFN),$G(APPTSTARTTIME),$G(CANBYCLINORPAT),$G(CANCELREASON),$G(NOTE),$G(APPTIEN),$G(CLINICIEN))
 ;
 I $D(ERRORS) S ERRORS("Appointment",1)="" M RETURN=ERRORS D BUILDJSON(.JSONRETURN,.RETURN) Q
 ;
 ...
 .D REMOVEMRTCAPTIEN(REQUESTIEN,APPTIEN)
 ...
 .D REQSET^SDESCONSULTUPD(REQUESTIEN,PROVIEN,"",2,CANBYCLINORPAT,NOTE,APPTSTARTTIME,RESOURCE,$G(DFN))
 .; update contact main sequence upon re-open
 .D UPDCONTSEQ^SDESCONTACTS($G(DFN),$G(REQUESTIEN))
 ;
