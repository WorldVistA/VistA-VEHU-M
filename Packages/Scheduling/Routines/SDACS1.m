SDACS1 ;ALB/BOK - EXTERNAL PACKAGE CALL TO ADD STOP CODES &/OR PROCEDURES ;3/18/92  14:24
 ;;5.3;Scheduling;;Aug 13, 1993
 ;;MAS VERSION 5.1;;**7**;
 ;Continued from SDACS0
 S SDAPTYPR=$S(SDCOMP:2,SDAPTYP=10:1,1:0)
...
DUPCHK I $D(SDC)=11,$D(^SDV("ADT",DFN,$P(SDATE,"."))) S SDFDT=^($P(SDATE,".")) I $D(^SDV(SDFDT,0)) F SDB=0:0 S SDB=$O(^SDV(SDFDT,"CS",SDB)) Q:SDB'>0  S SDA=+^(SDB,0),SDA=$P(^DIC(40.7,+SDA,0),"^",2) I $D(SDC(SDA)) K SDC(SDA)
 ...
 F SDA=0:0 S SDA=$O(SDC(SDA)) Q:SDA'>0!(SDSCCT'<15)  S SDAX="",DIE="^SDV(",DA=SDFDT,DR="10///"_SDA,DR(2,409.51)="2////^S X=DUZ;S SDAX=DA;5////^S X=SDAPTYP;11////^S X=1"_$S(SDAPTYPR:";6////^S X=SDAPTYPR",1:"") D ^DIE D
 .S SDERR=0,SDCTR=SDCTR+1,SDSCCT=SDSCCT+1
 .D:SDAX AE^SDAMEVT1(DA,SDAX,"1^added as a result of activity in another package")
...
Q I SDCPTCT>6!(SDSCCT'<15) D EXCESS
 ...
 ;D:SDCTYPE="S" ADD^SDAMEVT2(DFN,SDFDT)
 ;D:SDCTYPE="C"!(SDCTYPE="B") ADD^SDAMEVT2(DFN,DA(1))
CLEAN K SD900,SDA,SDAPTYP,SDAPTYPR,SDA1,SDAX,SDB,SDCOMP,SDCPT,SDCPTCT,SDCTR,SDD,SDECPT,SDEMSG,SDF,SDFDT,SDFLAG,SDJ,SDP,SDSCCT,SDVNODE,SDY,DA,DIC,DIE,DR,IJ,IX,J,VAEL,VAERR,X,Y K:SDMSG=0 SDMSG Q
...
CHECK S SDECPT=$S($P(SDCPT(SDJ),U)'=900:1,$P(SDCPT(SDJ),U,2)']"":1,'$D(^SC(+$P(SDCPT(SDJ),U,2),0)):1,$P(^(0),U,3)'="C":1,1:0) I SDECPT K SDCPT(SDJ) S SDEMSG="Invalid data contained in CPT string - not " D ERR:"CB"[SDMSG Q
 ...
 Q  ;Return to CPT+3
CHK1 ;
 N DA S DA(1)=SDATE D SCREEN^SDAMBAE3 I '$T S SDECPT=1,SDEMSG="Invalid or Inactive CPT code "_Y_" - not " D ERR:"CB"[SDMSG
 Q
...
FILE ;
 ...
 L +@SDVNODE:2 I $T D ^DIE L -@SDVNODE K DR,DQ,DE,DIC,X S DA(1)=SDA1,DA=+SDY D AE^SDAMEVT1(DA(1),DA,"1^added as a result of activity in another package") Q
