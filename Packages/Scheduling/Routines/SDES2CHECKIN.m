SDES2CHECKIN ;ALB/LAB,TJB,JAS,TJB,JDJ - SDES VISTA SCHEDULING SDES2 CHECKIN ;May 28,2026@10:00
 ;;5.3;Scheduling;**866,878,909,947**;Aug 13, 1993;Build 3
...
CHECKIN(SDRETURN,SDCONTEXT,SDPARAM) ;Check in appointment
 ...
 N C,DGMT,DGNOCOPF,DSIHCL,DSIHFS,SDIHFS,DSPV150,EVN,HL16,HL7STRG,HLA,HLECH,HLFS
 N HLQ,IEN,IVMZ10F,J,PIVOT,R,REF,S,SDCLINSTOP,SDMODE,VA,VACNTRY,VAERR,XQXFLG,XX,Z0,ZTSK
 S SDECNOEV=1 ;Don't execute protocol
 ;
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("Checkin",1)="",ERRORS("Appointment",1)="" D BUILDJSON^SDES2JSON(.SDRETURN,.ERRORS) Q
 ...
 I $D(ERRORS) S ERRORS("Checkin",1)="",ERRORS("Appointment",1)="" D BUILDJSON^SDES2JSON(.SDRETURN,.ERRORS) Q
 D BUILDAPPTOBJ^SDES2BLDAPPTOBJ(.RETURN,SDPARAM("APPOINTMENT IEN"),1,.ERRORS)
 I $D(ERRORS) M ERRORS=RETURN D BUILDJSON^SDES2JSON(.SDRETURN,.ERRORS) Q
