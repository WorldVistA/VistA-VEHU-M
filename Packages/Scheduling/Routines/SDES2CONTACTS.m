SDES2CONTACTS ;ALB/LAB,MCB/TJB,JAS,MCB,JAS,RN - VISTA SCHEDULING CONTACT SDES2 CONTACT ATTEMPTS ;June 26,2026
 ;;5.3;Scheduling;**860,873,878,887,917,909,949**;Aug 13, 1993;Build 4
...
ADDCONTACT(RESULT,SDCONTEXT,CONTACT) ; RPC: SDEC2 ADD CONTACT ATTEMPT
 ...
 ;
 D:(CONTACTIEN'="") UPDATECONTACT(.RESULT,.FDA,CONTACTIEN,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,.SDCONTEXT)
 D:(CONTACTIEN="") CREATECONTACT(.RESULT,.FDA,DFN,CONTACTIEN,CLINIC,PREFDATE,REQTYPE,APPTREQTYPE,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,.SDCONTEXT)
...
BLDNEWCONT(FDA,IENS,DFN,CLINIC,PREFDATE,REQTYPE,APPTREQTYPE) ; build the new contact attempt record.
 ...
 Q
 ;
BLDNEWATTMPT(FDA,IENS,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,SEQUENCE,SDCONTEXT) ;add a new entry to existing contact attempt record.
 ...
 S FDA(409.863,IENS,100)=$G(SDCONTEXT("ACHERON AUDIT ID"))
 S FDA(409.863,IENS,101)=$G(XWB(2,"RPC"))
 S FDA(409.863,IENS,102)=$G(SDCONTEXT("APPID"))
 Q
 ;
...
REQUESTINFO(ERRORS,CONTACT,APPTREQTYPE,REQUESTIEN,REQTYP,DFN,CLINIC,PREFDATE,CONTACTIEN) ;
 ...
 Q
 ;
CREATECONTACT(RESULT,FDA,DFN,CONTACTIEN,CLINIC,PREFDATE,REQTYPE,APPTREQTYPE,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,SDCONTEXT) ;
 ;create a new contact attempt record
 D BLDNEWCONT(.FDA,"+1,",DFN,CLINIC,PREFDATE,REQTYPE,APPTREQTYPE)
 S SUBIENS="+2,+1,"
 D BLDNEWATTMPT(.FDA,SUBIENS,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,1,.SDCONTEXT)
 Q
 ;
UPDATECONTACT(RESULT,FDA,CONTACTIEN,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,SDCONTEXT) ;
 ...
 D BLDNEWATTMPT(.FDA,SUBIENS,DTTMOFCONTACT,CONTACTTYPE,COMMENTS,DTTMENTERED,LEFTMSG,SEQUENCE,.SDCONTEXT)
 Q
 ;
