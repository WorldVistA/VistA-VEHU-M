SDES2CONTACTS ;ALB/LAB - VISTA SCHEDULING CONTACT SDES2 CONTACT ATTEMPTS ;JUL 26, 2023
...
ADDCONTACT(RESULT,SDCONTEXT,CONTACT) ; RPC: SDEC2 ADD CONTACT ATTEMPT
 ...
 I '$D(NEWCONTACT) S NEWCONTACT("Contact",1)="" D BUILDJSON^SDES2JSON(.RESULT,.CONTACTS) Q
...
VALIDATE(ERRORS,REQTYPE,DTTMOFCONTACT,COMMENTS,DTTMENTERED,LEFTMSG,CONTACTTYPE,CLINIC) ;validate input array values
 ...
 S DTTMENTERED=$$VALISODTTM^SDES2VALISODTTM(.ERRORS,DTTMENTERED,CLINIC,1,387,388) ;
 Q
 ;
...
GETCONTACTS(RESULT,SDCONTEXT,REQARRAY) ; RPC: SDES2 GET CONTACT ATTEMPT
 N REQTYPE,REQIEN,ERRORS,APPTREQTYPE,REQIEN,REQTYPE,DFN,CLINIC,PREFDATE,SERVICE,RESULTS,CONTACTS,CONTACTIEN
 D ASSIGNREQVAR(.REQARRAY,.REQTYPE,.REQIEN)
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("Contact",1)="" D BUILDJSON^SDES2JSON(.RESULTS,.ERRORS) Q
 ;
 D VALFIELD^SDES2VALUTIL(,.ERRORS,409.86,2.1,REQTYPE,,0,395,396)
