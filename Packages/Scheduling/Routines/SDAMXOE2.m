SDAMXOE2 ; ;08/24/93
 D DE G BEGIN
DE S DIE="^SCE(",DIC=DIE,DP=409.68,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^SCE(DA,-1))<0
...
2 S DW="0;9",DV="FI",DU="",DLB="EXTENDED REFERENCE",DIFLD=.09
 ...
 G Z:X,RD
X2 Q
3 S DW="0;10",DV="RP409.1'I",DU="",DLB="APPOINTMENT TYPE",DIFLD=.1
 S DU="SD(409.1,"
 S X=$G(SDVSIT("TYP"))
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X3 Q
4 S DW="0;11",DV="RP40.8'I",DU="",DLB="MEDICAL CENTER DIVISION",DIFLD=.11
 S DU="DG(40.8,"
 S X=$G(SDVSIT("DIV"))
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
