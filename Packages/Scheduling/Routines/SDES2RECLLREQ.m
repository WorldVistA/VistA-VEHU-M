SDES2RECLLREQ ; ALB/TJB,TJB,JDJ,JAS - VISTA SCHEDULING CREATE/UPDATE RECALL REQUESTS ; Nov 25, 2024
 ;;5.3;Scheduling;**866,881,890,895**;Aug 13, 1993;Build 11
...
CREATERECREQ(RETN,SDCONTEXT,SDINPUT) ;CREATE recall request
 N CAFDA,ERRORS,SDRECREQ,SDFDA,SDMSG,SDIEN
 ...
 D BLDCOMAUD(.CAFDA,.SDCONTEXT,.SDINPUT,SDIEN(1),"")
 D UPDATE^DIE("","CAFDA","","CAERR") K CAFDA
...
UPDRECALLREQ(RETN,SDCONTEXT,SDINPUT) ;RECALLIEN,DFN,ACCNO,SDCMT,FASTING,APPTP,RRPROVIEN,CLINIEN,APPTLEN,DATE,RECPPDT,DAPTDT,USERIEN,SECPDT,EAS) ;update recall request
 N CAFDA,ERRORS,LASTNOTE,SDRECREQ,SDFDA,SDMSG,SDIEN
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 ...
 I $D(SDMSG) D ERRLOG^SDES2JSON(.ERRORS,134),RETURNERROR(.ERRORS,.SDRECREQ,.RETN,SDINPUT("RECALL IEN")) Q
 D BLDCOMAUD(.CAFDA,.SDCONTEXT,.SDINPUT,SDINPUT("RECALL IEN"),LASTNOTE)
 D UPDATE^DIE("","CAFDA","","CAERR") K CAFDA
...
BLDREC(SDFDA,SDINPUT,SDCONTEXT) ;build and file record
 ...
 Q
 ;
BLDCOMAUD(CAFDA,SDCONTEXT,SDINPUT,RECALLIEN,LASTNOTE) ;
 ...
 S CAFDA(403.57,"+1,"_RECALLIEN_",",1)=$S($G(SDCONTEXT("USER DUZ"))'="":SDCONTEXT("USER DUZ"),1:DUZ)
 S CAFDA(403.57,"+1,"_RECALLIEN_",",2)=$TR(NEWNOTE,"^"," ")
 Q
 ;
 ; SDINPUT("RECALL IEN")
 ; SDINPUT("DFN")
