SDES ;ALB/KML,MGD,ANU,RRM,LAB,RRM,BWF,BLB,LAB - VISTA SCHEDULING RPCS ;MAY 18,2022
 ;;5.3;Scheduling;**788,790,792,794,797,799,800,801,803,804,805,807,809,813,814,815,818**;Aug 13, 1993;Build 9
...
APTREQCREATE(JSON,ARIEN,DFN,EDT,INST,TYPE,CLIN,USER,REQBY,PROV,DAPTDT,COMM,ENPRI,MAR,MAI,AMRAN,PATCONT,SVCCON,SVCCOP,MRTCDT,STOP,APTYP,PATSTAT,MULTAPTMDE,PARENT,NLT,PRER,ORDN,VAOSGUID,EAS,PCMT,INSTIEN,PATDATEPREFS,ARSTOPSEC) ;
 S ARIEN=""  ;There will never be an IEN when calling this RPC
 D ARSET Q
APTREQUPDATE(JSON,ARIEN,DFN,EDT,INST,TYPE,CLIN,USER,REQBY,PROV,DAPTDT,COMM,ENPRI,MAR,MAI,AMRAN,PATCONT,SVCCON,SVCCOP,MRTCDT,STOP,APTYP,PATSTAT,MULTAPTMDE,PARENT,NLT,PRER,ORDN,VAOSGUID,EAS,PCMT,INSTIEN,PATDATEPREFS,ARSTOPSEC) ;
 S ARIEN=$G(ARIEN,"")
...
ARSET ;Called from APTREQCREATE and APTREQUPDATE tags
 ...
 S EAS=$G(EAS),PCMT=$G(PCMT),INSTIEN=$G(INSTIEN),ARSTOPSEC=$G(ARSTOPSEC)
 ...
 D ARSET^SDESAPTREQSET(.JSON,ARIEN,DFN,EDT,INST,TYPE,CLIN,USER,REQBY,PROV,DAPTDT,COMM,ENPRI,MAR,MAI,AMRAN,PATCONT,SVCCON,SVCCOP,MRTCDT,STOP,APTYP,PATSTAT,MULTAPTMDE,PARENT,NLT,PRER,ORDN,VAOSGUID,EAS,PCMT,INSTIEN,.PATDATEPREFS,ARSTOPSEC)
 Q
...
GETMISSIONELG(SDECY,APPTREQIEN,PID,APPTREQTYP,SDEAS,APPTDATE) ;
 D GETMISSIONELG^SDESMISSIONELG(.SDECY,$G(APPTREQIEN),$G(PID),$G(APPTREQTYP),$G(SDEAS),$G(APPTDATE)) Q
