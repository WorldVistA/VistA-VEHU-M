...
DE S DIE="^SDV(D0,""CS"",",DIC=DIE,DP=409.51,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^SDV(D0,"CS",DA,-1))<0
 I $D(^("PR")) S %Z=^("PR") S %=$P(%Z,U,1) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="SDXACSE6",DQ=1
1 S DW="PR;1",DV="*P409.71'X",DU="",DLB="PROCEDURE 1",DIFLD=21
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,3)
 S Y=X
...
X2 I $P(SDCPT(SDJ),U,4)']"" S Y="@99"
 Q
3 D:$D(DG)>9 F^DIE17 G ^SDXACSE7
