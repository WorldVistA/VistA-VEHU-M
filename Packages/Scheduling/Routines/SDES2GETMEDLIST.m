SDES2GETMEDLIST ;ALB/JAS - SDES2 GET PATIENT'S MEDICATIONS LIST ; 08 Aug 2023  3:12 PM
...
GETMEDLIST(JSONRETURN,SDCONTEXT,PARAMS) ; Get Patient's Medications List
 ;
 N DTRANGE,SDDFN,SDERRORS,SDENDDT,SDSTRTDT,SDRETURN,VALRET
 ;
 ; Validate SDCONTEXT
 ;
 ...
 D VALFILEIEN^SDES2VALUTIL(.VALRET,.SDERRORS,2,$G(SDCONTEXT("PATIENT DFN")),1,0,1,2)
 I 'VALRET,$D(SDERRORS) M SDRETURN=SDERRORS S SDRETURN("Medication",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.SDRETURN) Q
 S SDDFN=SDCONTEXT("PATIENT DFN")
 ;
 ; Validate PARAMS
 ;
