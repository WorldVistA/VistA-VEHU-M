SDINACT ;ALB/TMP - INACTIVATE A CLINIC ; 30 APR 85  9:02 am 
 ;;MAS VERSION 5.1;
 S:'$D(DTIME) DTIME=300 I '$D(DT) D DT^SDUTL
 S SDAY="Sun^Mon^Tues^Wednes^Thurs^Fri^Satur",SDZQ=1
 D DT^DICRW S DIC="^SC(",DIC(0)="AEMZQ",DIC("A")="Select CLINIC NAME: ",DIC("S")="I $P(^(0),""^"",3)=""C""" D ^DIC K DIC("A"),DIC("S") G:Y<0 END S SC=+Y,SDX="",SDX1=9999999
...
D S %DT="AEFX",%DT("A")="Enter Date Clinic is to be Inactivated: " D ^%DT K %DT G:Y'>0 END S SDDATE=Y I Y<DT W "??",!,*7,"Inactivate date must be greater than or equal to today's date" G D
 ...
 S POP=0 F I=SDDATE-.0001:0 S I=$N(^SC(SC,"S",I)) Q:I'>0!(POP)!(SDDATE'<SDX1&(SDX1))  F I1=0:0 S I1=$N(^SC(SC,"S",I,1,I1)) Q:I1'>0  I $P(^(I1,0),"^",9)'="C" S POP=1 Q
 ...
 F I=0:1:6 S J=$N(^SC(SC,"T"_I,SDN(I))) D GOT
OVR F I=SDDATE-.0001:0 S I=$N(^SC(SC,"ST",I)) Q:I'>0!(I>SDX1)  K ^(I)
 F I=SDDATE-.0001:0 S I=$N(^SC(SC,"T",I)) Q:I'>0!(I>SDX1)  K ^(I)
 F I=SDDATE-.0001:0 S I=$N(^SC(SC,"OST",I)) Q:I'>0!(I>SDX1)  K ^(I)
 S ^SC(SC,"I")=SDDATE_"^"_$P(^SC(SC,"I"),"^",2)
 W !!,"Clinic will be inactivated effective " S Y=SDDATE D DTS^SDUTL W Y G END
 ;
CHECK W *7,!,"This clinic is to be inactivated as of " S SDX=+^("I"),Y=SDX D DTS^SDUTL W Y S SDX1=+$P(^("I"),"^",2),Y=SDX1 I Y D DTS^SDUTL W " and reactivated as of ",Y
...
CHG1 K SDN S X1=SDDATE,X2=6 D C^%DTC S SDNL=X,X=SDDATE D DOW^SDM0 S SDN(Y)=X
 ...
 F I=0:0 S X2=1,X1=X D C^%DTC Q:X>SDOL  D DOW^SDM0 S:$D(^SC(SC,"T"_Y))&($N(^SC(SC,"T"_Y,0))'=9999999) ^SC(SC,"T"_Y,SDN(Y),1)=$S($D(^SC(SC,"T"_Y,X,1)):^(1),1:""),^(0)=SDN(Y) D A1,A
 I SDDATE<SDX F I=0:1:6 F J=SDNL:0 S J=$N(^SC(SC,"T"_I,J)) Q:J'>0!(J'<SDX)  K ^SC(SC,"T"_I,J)
 Q
A1 S:'$D(^SC(SC,"T"_Y,9999999,1)) ^(1)="",^(0)=9999999 K:(SDN(Y)-X) ^SC(SC,"T"_Y,X)
 Q
A I $N(^SC(SC,"T"_Y,SDN(Y)))>0 S SD=$N(^SC(SC,"T"_Y,SDN(Y))) S:^SC(SC,"T"_Y,SD,1)]"" ^SC(SC,"T"_Y,SDN(Y),1)=^SC(SC,"T"_Y,SD,1),^(0)=SDN(Y),^SC(SC,"T"_Y,SD,1)=""
 I SDX'>SDDATE,$N(^SC(SC,"ST",SDX-.1))>0 F Z=SDX-.1:0 S Z=$N(^SC(SC,"ST",Z)) Q:Z'>0!(SDX1&(Z'<SDX1))  K ^SC(SC,"ST",Z)
 K SD,Z Q
GOT S SD=$N(^SC(SC,"T"_I,0))
 I J>0,SD'=9999999 S ^SC(SC,"T"_I,SDN(I),1)=^SC(SC,"T"_I,J,1),^(0)=SDN(I) K ^SC(SC,"T"_I,J) F J1=J:0 S J1=$N(^SC(SC,"T"_I,J1)) Q:J1'>0  K ^SC(SC,"T"_I,J1)
