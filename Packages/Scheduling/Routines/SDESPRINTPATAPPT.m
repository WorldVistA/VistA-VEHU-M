SDESPRINTPATAPPT ;ALB/LAB - SDES Print basic appointments ;Feb 9, 2022
...
PRINTPATAPPTS(SDPATIENTAPPTS,SDDFN,SDBEG,SDEND,EAS) ; PRINT PATIENT APPOINTMENTS
 N HASDFNERRORS,HASBEGDTERRORS,HASENDDTERRORS,HASERRORS,ERRORS,SDAPPOINTARRAY,PRINTFIELDS,RETURN
...
VALIDATEPTDFN(ERRORS,SDDFN) ;Validate Patient DFN
 N ERRORFLAG
 I SDDFN="" S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,1)
 I SDDFN'="",('$D(^DPT(SDDFN,0))) S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,2)
 Q $D(ERRORFLAG)
 ;
VALIDATEBEGDT(ERRORS,SDBEG) ;Validate Begin date - converted in SDES so -1 if invalid
 N ERRORFLAG
 I SDBEG=-1 S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,11)
 Q $D(ERRORFLAG)
 ;
VALIDATEENDDT(ERRORS,SDEND) ;validate End date - converted in SDES so -1 if invalid
 N ERRORFLAG
 I SDEND=-1 S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,12)
 Q $D(ERRORFLAG)
 ;
GETAPPOINTMENTS(SDPATIENTAPPTS,SDDFN,SDBEGIN,SDEND) ;select appointments
 ...
 I COUNT=0 D
 . S SDPATIENTAPPTS("PatientAppointments")=""
 . S COUNT=1
 Q COUNT
 ;
