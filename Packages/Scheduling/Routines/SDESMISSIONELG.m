SDESMISSIONELG ;ALB/ANU - VISTA SCHEDULING RPCS GET MISSION ELIGIBILITY ; Mar 09, 2022@15:20
 ...
 Q
 ;
GETMISSIONELG(ELGRETURN,APPTREQIEN,PID,APPTREQTYP,SDEAS) ; Get Mission Eligbility
 ; This RPC gets User name, Keys and Scheduling Options for a given User.
 ; Input:
 ; ELGRETURN  - [required] - Successs or Error message
 ...
 ;
 N HASVLDERRORS,RETURN,HASFIELDS,ELGFIELDSARRAY,SDCLNIEN,REQDATE,REQENDDT,SDAPPTAVAIL,SDGETCLAVL,VLDERRORS,SDSTOPCD,SDSERVICE
 N SDPRIM,STOPCDSTR,SDI,REQDATENET,REQENDDTNET
 N SDCLRESIEN,PIDDATE
 S (RETURN,VLDERRORS,ELGFIELDSARRAY,HASFIELDS)=""
 ;
 S HASVLDERRORS=$$VALIDATE(.VLDERRORS,APPTREQIEN,PID,APPTREQTYP,SDEAS)
 I HASVLDERRORS M RETURN=VLDERRORS
 I 'HASVLDERRORS S HASFIELDS=$$GETMSNELG(.ELGFIELDSARRAY,REQDATE,REQENDDT,PIDDATE,.SDAPPTAVAIL)
 ...
 Q
 ;
VALIDATE(ERRORS,APPTREQIEN,PID,APPTREQTYP,SDEAS) ; Validate Appointment Request IEN, Request Date, PID Date
 ...
 I "APPT,RTC,CONSULT,PROCEDURE,PTCSCH"'[APPTREQTYP S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,3) Q $D(ERRORFLAG)
 I APPTREQTYP="" S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,3) Q $D(ERRORFLAG)
 I ((APPTREQTYP="APPT")!(APPTREQTYP="RTC")) D
 . I APPTREQIEN'="",'$D(^SDEC(409.85,+APPTREQIEN,0)) S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,60) Q
 . S SDCLNIEN=$$GET1^DIQ(409.85,APPTREQIEN,8,"I") ;Appointment resource/clinic
 . S REQDATE=$$GET1^DIQ(409.85,APPTREQIEN,1,"I") ;Request Start Date
 ;
 I ((APPTREQTYP="CONSULT")!(APPTREQTYP="PROCEDURE")) D
 . I APPTREQIEN'="",'$D(^GMR(123,+APPTREQIEN,0)) S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,60) Q
 . S SDCLNIEN=$$GET1^DIQ(123,APPTREQIEN,.04,"I") ;Appointment resource/clinic
 . S REQDATE=$$GET1^DIQ(123,APPTREQIEN,3,"I") ;Request Start Date
 ;
 I APPTREQTYP="PTCSCH" D
 . I APPTREQIEN'="",'$D(^SD(403.5,+APPTREQIEN,0)) S ERRORFLAG=1 D ERRLOG^SDESJSON(.ERRORS,60) Q
 . S SDCLNIEN=$$GET1^DIQ(403.5,APPTREQIEN,4.5,"I") ;Appointment resource/clinic
 ...
 S SDPRIM="N"
 S STOPCDSTR=""
 F SDI=2:1 S STOPCDSTR=$P($T(STOPCDLST+SDI),";;",2,3) Q:$P(STOPCDSTR,";")="QUIT"  D
 .I STOPCDSTR[SDSTOPCD S SDPRIM="Y" Q
 ;
 ...
 ;
 ; Determine Appointment availability is within Request starte date and Request end date
 K SDTMPARY
 ...
 Q $D(ERRORFLAG)
 ;
GETMSNELG(ELGARRAY,REQDATE,REQENDDT,PIDDATE,SDAPPTAVAIL) ; GET Eligibility
 N HASDATA
...
BUILDJSON(JSONRETURN,INPUT) ; Build JSON format
 ...
 ;
CLEANUP ; Cleanup 
 K HASVLDERRORS,RETURN,HASFIELDS,ELGFIELDSARRAY,SDCLNIEN,REQENDDT,SDAPPTAVAIL,SDGETCLAVL,SDSTOPCD,SDTMPARY
 K ERRORFLAG
 K HASDATA
 K JSONERROR
 Q
STOPCDLST ;
 ; List of Stop codes (Primary/Mental Health)
 ;;322^323^338^341^342^350^502^513^523^527^528^530^533^534^536
 ;;539^545^546^550^560^565^566^576^577^579^584
 ;;QUIT
