...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(3)=% S %=$P(%Z,U,2) S:%]"" DE(4)=% S %=$P(%Z,U,3) S:%]"" DE(5)=% S %=$P(%Z,U,4) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(7)=% S %=$P(%Z,U,12) S:%]"" DE(8)=%
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".52;5",DV="RSXR",DU="",DIFLD=.525,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^DVBAXA5",DE(DW,"INDEX")=1
...
X1 S DFN=DA D POWV^DGLOCK I $D(X) D SV^DGLOCK
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".32;3",DV="*P21'XaR",DU="",DIFLD=.323,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^DVBAXA5"
 S DU="DIC(21,"
 G RE
...
X2 S DFN=DA D POS^DGLOCK1
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;1",DV="FaXJ35R",DU="",DIFLD=.111,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^DVBAXA5",DE(DW,"INDEX")=1
 G RE
...
C3F1 N X,X1,X2 S DIXR=251 D C3X1(U) K X2 M X2=X D C3X1("O") K X1 M X1=X
 D
...
C3X1(DION) K X
 ...
 Q
C3F2 Q
X3 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>35!($L(X)<3) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".11;3",DV="FaXJ30",DU="",DIFLD=.113,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C5^DVBAXA5",DE(DW,"INDEX")=1
 G RE
...
C5F1 N X,X1,X2 S DIXR=254 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 D
...
X5 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X D:$D(X) UP^DGHELP
 ...
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".11;4",DV="FaXJ15R",DU="",DIFLD=.114,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C6F1 N X,X1,X2 S DIXR=255 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
X6 K:$L(X)>15!($L(X)<2) X D:$D(X) UP^DGHELP
 ...
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;5",DV="*P5'aR",DU="",DIFLD=.115,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 D ^DVBAXA6
C7F1 N X,X1,X2 S DIXR=256 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
X7 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;12",DV="FXOaR",DU="",DIFLD=.1112,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(8,2)="S Y(0)=Y D ZIPOUT^VAFADDR"
 S DE(DW)="C8^DVBAXA5",DE(DW,"INDEX")=1
 G RE
C8 G C8S:$D(DE(8))[0 K DB
 D ^DVBAXA7
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^DVBAXA8
C8F1 N X,X1,X2 S DIXR=157 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
...
C8X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C8F2 S DIXR=252 D C8X2(U) K X2 M X2=X D C8X2("O") K X1 M X1=X
 D
