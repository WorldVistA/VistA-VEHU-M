DVBCUTIL ;ALB/GTS-557/THM;C&P UTILITY ROUTINE ; 7/31/26 3:51pm
 ;;2.7;AMIE;**17,126,143,149,184,256**;Apr 10, 1995;Build 19
...
SVC(DFN,DVBCIE) ;Retrieve Last Military Service Data Info
 ...
 Q DVBMSE
VBUTTON(DVBRTN,DVBSRV) ;CAPRI-28984 BG 7/31/26
 ;RPC DVBA CAPRI VISTA BUTTON
 K DIC,DIE,DA,DR,X,Y,DO,^TMP("DVBBTN")
 N DVBPAR,DVBDTTM,DVBSAVESEQ,DVBBTN,DVBCNT,DVBRET,DVBCT,DVBRT
 S DVBDTTM=$$NOW^XLFDT
 S DVBSAVESEQ=$P($G(^DVB(396.9992,DUZ,1,0)),"^",3)
 S DA(1)=DUZ,(DA,X)=DVBSAVESEQ+1
 S (DLAYGO,DIC)="^DVB(396.9992,"_DA(1)_",1,",DIC(0)="LZ"
 S DIC("DR")=".01////"_DVBDTTM_";.02////"_DVBSRV
 D FILE^DICN
 I Y=-1 S DVBRT="-1^New Entry not Built"
 K DIC,DIE,DA,DR,X,Y,DO
 I $D(^XTV(8989.51,"B","DVBAB CAPRI VISTA GUI MSG"))<9 S ^TMP("DVBBTN",$J,1)="-1^No button message to display",DVBRTN=$NA(^TMP("DVBBTN",$J)) Q
 D GETWP^XPAR(.DVBBTN,"PKG","DVBAB CAPRI VISTA GUI MSG")
 S DVBCT=0,DVBCNT=0 F  S DVBCT=$O(DVBBTN(DVBCT)) Q:DVBCT=""  D
 .I DVBCNT=0 D
 ..S DVBCT=0,DVBRET(DVBCNT)=$G(DVBRT)
 ..M ^TMP("DVBBTN",$J,DVBCNT)=DVBRET(DVBCNT) S DVBCNT=DVBCNT+1,DVBCT=DVBCT+1
 ..Q
 .S DVBRET(DVBCNT)=DVBBTN(DVBCT,0)
 .S DVBRET(DVBCNT)=DVBBTN(DVBCT,0) M ^TMP("DVBBTN",$J,DVBCNT)=DVBRET(DVBCNT)
 .S DVBCNT=DVBCNT+1
 .Q
 S DVBRTN=$NA(^TMP("DVBBTN",$J))
 Q
AASBURPT ;AMIE AUDIT SUMMARY BY USER REPORT (396.9992) CAPRI-28984 JCS 08/05/2026
 ;
 N DVBDUZ,DVBIDX,DVBLC,DVBTEMP,DVBNAM,DVBDAT,DVBSRV,DVBBL
 S (DVBDUZ,DVBIDX,DVBTEMP)="",DVBLC=0
 S $P(DVBBL," ",80)=""
 D AASBURPTH
 K ^TMP("DVBAAR",$J)
 F  S DVBDUZ=$O(^DVB(396.9992,DVBDUZ)) Q:DVBDUZ=""  D
 . S DVBIDX=0
 . F  S DVBIDX=$O(^DVB(396.9992,DVBDUZ,1,DVBIDX)) Q:'+(DVBIDX)  D
 .. S DVBNAM=$$GET1^DIQ(200,DVBDUZ_",",".01","","","DVBERR")
 .. I DVBNAM="" S DVBNAM="Unknown ("_DVBDUZ_")"
 .. S DVBDAT=$$FMTE^XLFDT($G(^DVB(396.9992,DVBDUZ,1,DVBIDX,0)),2)
 .. S DVBSRV=$G(^DVB(396.9992,DVBDUZ,1,DVBIDX,1))
 .. S ^TMP("DVBAAR",$J)=$G(^TMP("DVBAAR",$J))+1
 .. S ^TMP("DVBAAR",$J,^TMP("DVBAAR",$J))=DVBNAM_U_DVBDAT_U_DVBSRV
 ;
 S DVBIDX="" F  S DVBIDX=$O(^TMP("DVBAAR",$J,DVBIDX)) Q:DVBIDX=""  D
 . S DVBTEMP=^TMP("DVBAAR",$J,DVBIDX)
 . W !,$E($P(DVBTEMP,U,1),1,19),?20,$P(DVBTEMP,U,2),?38,$P(DVBTEMP,U,3)
 I '$D(^TMP("DVBAAR",$J)) W !!,"No data to report at this time"
 K ^TMP("DVBAAR",$J)
 Q
AASBURPTH ;AMIT AUDIT HEADER CAPRI-28984 JCS 08/05/2026
 W !,?28,"CAPRI VISTA BUTTON USAGE",!
 W !,"Name",?20,"Date/Time",?38,"Server"
 W !,"-------------------",?20,"-----------------",?38,"---------------------------------------"
 Q
