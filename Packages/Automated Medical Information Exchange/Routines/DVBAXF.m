DVBAXF ; GENERATED FROM 'DVBA FINALIZED REPORT' PRINT TEMPLATE (#1044) ; 05/21/98 ; (FILE 396, MARGIN=132)
 G BEGIN
CP G CP^DIO2
C S DQ(C)=Y
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1044,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
