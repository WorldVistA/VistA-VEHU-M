...
X1 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".13;1",DV="FaXJ17",DU="",DIFLD=.131,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^DVBAXA9"
 G RE
...
C2F1 Q
X2 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".13;2",DV="FaXJ17",DU="",DIFLD=.132,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^DVBAXA9"
 G RE
