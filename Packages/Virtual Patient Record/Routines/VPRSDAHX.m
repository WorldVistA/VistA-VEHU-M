VPRSDAHX ;SLC/MKB -- SDA Hx utilities ;7/29/22  14:11
 ;;1.0;VIRTUAL PATIENT RECORD;**30,37**;Sep 01, 2011;Build 10
...
HFM ; -- get V Health Factors, for Family History
 ...
 .. S DA=0 F  S DA=$O(^PXRMINDX(9000010.23,"PI",+$G(DFN),ITEM,DATE,DA)) Q:DA<1  S VPRN=VPRN+1,DLIST(VPRN)=DA Q:VPRN'<DMAX  ;p37 removed _U_ITEM from the DLIST set
 Q
 ;
HFS ; -- get V Health Factors, for Social History
 ...
 . S DATE=DSTRT F  S DATE=$O(^PXRMINDX(9000010.23,"PI",+$G(DFN),ITEM,DATE)) Q:DATE<1!(DATE>DSTOP)  D  Q:VPRN'<DMAX
 .. S DA=0 F  S DA=$O(^PXRMINDX(9000010.23,"PI",+$G(DFN),ITEM,DATE,DA)) Q:DA<1  S VPRN=VPRN+1,DLIST(VPRN)=DA Q:VPRN'<DMAX  ;p37 removed_U_ITEM from the DLIST set
 Q
