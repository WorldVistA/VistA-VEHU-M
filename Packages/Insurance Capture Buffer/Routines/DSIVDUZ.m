...
OUT Q:$D(FUN) DSIV Q
 ;
ACT(DSIV,XDUZ,DSISCR,FUN) ;  RPC: DSIV ACTIVE USER
 ; validate that user is an active user
 ; RETURN: user's DUZ value
 S DSIV=$$ACT^DSIVDUZ1 G OUT
 ;
CK(XDUZ) ; basic check for valid DUZ
 ; Return 1 if valid DUZ
 Q $$CK^DSIVDUZ1(XDUZ)
 ;
DIV(DSIV,XDUZ,SITE,FUN) ;  RPC: DSIV USER DEF DIV
 ...
 S DSIV=$$DIV^DSIVDUZ1 G OUT
 ;
 ;ID(DSIV,XDUZ,FLAGS,FUN) ; RPC: DSIV USER ID
 ...
 ;;         "v" - VA#
 ;;         "V" - VPID
 ;; Return DSIV(n) = p1^p2^p3^p4 for n=1,2,3,... where
 ...
 ;D ID^DSIVDUZ1 G OUT
 ;
LIST(DSIV,VAL,SCR) ;  RPC: DSIV ACTIVE USER LIST
 ...
 Q
 ;
 ;PER(DSIV,USER,DATE,FUN) ; RPC: DSIV ACTIVE PERSON CLASS
 ...
 ;S DSIV=$$PER^DSIVDUZ1 G OUT
 ;
 ;PROV(DSIV,XDUZ,RDV,FUN) ; rpc: DSIV ACTIVE CPRS PROVIDER
