DSIVDDR0 ; BLJ/DSS-Clones of Fileman calls ;11/24/2013 11:57
 ;;2.2;INSURANCE CAPTURE BUFFER;**11,12**;Jul 09, 2008;Build 13
 ;Copyright 1995-2014, Document Storage Systems, Inc., All Rights Reserved
 ;
 ;Cloned from DSICDDR0
 ;
 ; DBIA#  SUPPORTED
 ; -----  -----------------------
 ;  2051  ^DIC:  FIND, LIST
 ...
 ;  2050  MSG^DIALOG
 ;
FILE(DSIVP,FILE,IENS,FLAGS,DATA) ; rpc: DSIV DDR FILER
 ; DO NOT USE AFTER 11/21/2002 - use FILE^DSIVFM04
 ;    DSIVFM04 input format is different
 ;
 ...
 D FILE^DSIVFM04(.DSIVP,$G(FILE),$G(IENS),FLAGS,.DSIN)
 G CLEANUP
 ;
FIND(DSIVP,FILE,IENS,FIELD,NUMBER,VALUE,INDEX,SCREEN,FLAGS) ;
 ; RPC: DSIV DDR FINDER
 ;
 ; Input: FILE: File number
 ;        IENS: IENS in file
 ...
 .S ^TMP("DSIV",$J,0)="-1^"_Y_$G(^TMP("DIERR",$J,"1","TEXT","1"))
 .K ^TMP("DILIST",$J)
 .Q
 I $D(^TMP("DILIST",$J)) D
 .S ^TMP("DSIV",$J,0)=$G(^TMP("DILIST",$J,0))
 .S SEQ=0
 ...
 ..Q
 .Q
 K ^TMP("DILIST",$J) S DSIVP=$NA(^TMP("DSIV",$J))
 G CLEANUP
 ;
GET(DSIVP,FILE,IENS,FIELD,FLAGS) ; RPC: DSIV DDR GETS ENTRY DATA
 ;  FILE = File number
 ...
 ;         @DSIVP@(n) = FileNumber^IENS^FieldNumber^Data^I or E
 ;
 ...
 K ^TMP("DSIV",$J),^TMP("DSIV1",$J) S DSIVP=$NA(^TMP("DSIV",$J))
 D INIT("GET")
 I FIELD["**" S @DSIVP@(1)="-1^Getting submultiples not supported." Q
 S:FLAGS="" FLAGS="E"
 S X="-1^Only a single field number allowed with the W flag"
 I FLAGS="W",FIELD'=+FIELD S @DSIVP@(1)=X Q
 S X="-1^The W flag must be used by itself"
 I FLAGS["W",FLAGS'="W" S @DSIVP@(1)=X Q
 I FLAGS="W" S FLAGS=""
 D GETS^DIQ(FILE,IENS,FIELD,FLAGS,$NA(^TMP("DSIV1",$J)))
 S SUB=0,(ROOT,STOP)=$NA(^TMP("DSIV1",$J)),$E(STOP,$L(STOP))=","
 F  S ROOT=$Q(@ROOT) Q:ROOT'[STOP  D
 .S DATA=@ROOT,Y=$QL(ROOT)
 .S FILE=$QS(ROOT,3),FIELD=$QS(ROOT,5),X=$QS(ROOT,Y)
 .;  check for WP data
 .I FLAGS="" S:X=+X&(Y=6) SUB=SUB+1,@DSIVP@(SUB)=DATA Q
 .Q:DATA[U  S:+X=X X="E"
 .S SUB=SUB+1,Z=FILE_U_IENS_U_FIELD_U
 .S L=$L(Z)+$L(DATA) I L<250 S Z=Z_DATA
 .I L>249 S Z=Z_$E(DATA,1,250-$L(Z))_"..."
 .S:DATA'="" Z=Z_U_X
 .S @DSIVP@(SUB)=Z
 .Q
 I '$D(@DSIVP) S @DSIVP@(1)="-1^No data retrieved"
 K ^TMP("DSIV1",$J)
 G CLEANUP
 ;
GET2(DSIVP,FILE,IENS,FIELD,FLAGS) ; RPC: DSIV DDR GETS EXTENDED DATA
 ;  FILE = File number
 ;  IENS = Internal entry number string
 ; FIELD = Field numbers delimited by either ^ or ;
 ; FLAGS = Flags to control processing:
 ...
 .S SUB=SUB+1,Z=FILE_U_IENS_U_FIELD_U
 .S L=$L(Z)+$L(DATA) I L<250 S Z=Z_DATA
 .I L>249 S @DSIVP@(SUB)=Z_$E(DATA,1,250-$L(Z)),SUB=SUB+1,Z=FILE_U_IENS_U_FIELD_U_"$$CONT$$"_U_$E(DATA,251-$L(Z),$L(DATA))
 .S:DATA'="" Z=Z_U_X
 .S @DSIVP@(SUB)=Z
 .Q
 I '$D(@DSIVP) S @DSIVP@(1)="-1^No data retrieved"
 ...
 G CLEANUP
 ;
KILL(DSIVP,FILE,IENS) ; RPC: DSIV DDR DELETE ENTRY
 ; Input: FILE: File number
 ;        IENS: IENS
 ;
 ...
 S FDA(FILE,IENS,.01)="@"
 D FILE^DIE("","FDA")
 S X=$$MSG S:X<0 X="0^"_$P(X,U,2) S DSIVP(1)=X
 G CLEANUP
 ;
LIST(DSIVP,FILE,IENS,FIELD,FLAGS,NUMBER,FROM,PARTIAL,INDEX,SCREEN,IDENTIFY) ;
 ; RPC: DSIV DDR LISTER
 ; INPUT : FILE : File Number
 ...
 S DSIVP=$NA(^TMP("DSIV",$J))
 S ROOT=$NA(^TMP("DSIV1",$J))
 K @DSIVP,@ROOT
 D INIT("LIST")
 S:NUMBER="" NUMBER="*"
 D LIST^DIC(FILE,IENS,FIELD,FLAGS,NUMBER,FROM,PARTIAL,INDEX,SCREEN,IDENTIFY,ROOT)
 S TMP=$NA(^TMP("DSIV1",$J,"DILIST","ID")),STOP=$P(TMP,")")
 F  S TMP=$Q(@TMP) Q:TMP'[STOP  D
 .S DATA=@TMP,SEQ=$QS(TMP,5),FLD=$QS(TMP,6)
 .S X=$G(^TMP("DSIV1",$J,"DILIST",2,SEQ))
 .S ^TMP("DSIV",$J,SEQ,FLD)=FILE_U_X_U_FLD_U_DATA
 K ^TMP("DSIV1",$J)
 G CLEANUP
 ;
UPDATE(DSIVP,FILE,DATA) ; RPC: DSIV DDR UPDATE FILE
 N IENS G U1
 ;
UPDATE1(DSIVP,FILE,IENS,DATA) ; RPC: DSIV DDR UPDATE SUBFILE
 ; Input: FILE : File number
 ;        DATA : Array of
 ;           ("Field number")=Data
 ;
 ; Return: DSIVP: Successful Create="1^New Entry Number"
...
U1 N X,FDA,IENS1
 ...
 S X=$$MSG S:X>0 X=X_$G(IENS1(1)) S DSIVP(1)=X
 G CLEANUP
 ;
WP(DSIVP,FILE,IENS,FIELD,FLAGS,DSIVD) ; RPC: DSIV DDR WP FILER
 ; Input : FILE: file number
 ;         IENS: IENS
 ;         FIELD: field number
 ;         FLAGS: Flags
 ;         DSIVD: Word processing text to be saved
 ;                If DSIVD="@", delete the wp field data
 ; 
 ; Return: Successful save: "1^"
 ;       unsuccessful save: "-1^Error text"
 ;
 N X,Y D INIT("WP") S X=$S($G(DSIVD)="@":"@",1:"DSIVD")
