...
DIV() ; Return default division for user
 ...
 S X=$$DIV4^XUSER(.DSIVX,XDUZ)
 I 'X,'$G(SITE) Q $$ERR(8)
 I 'X Q $$SITE^VASITE
 S (A,I,X)=0
 F  S I=$O(DSIVX(I)) Q:'I  Q:DSIVX(I)  S A=A+1 S:DSIVX(I)="" X=I
...
LIST ; Return a list of active users only for a lookup value
 N I,X,Y,Z,DSI,DSICNT,DSIVX,DSILIST,DSIRET,DSISCR,DSIX,ERR,INPUT
 S DSIRET=$NA(^TMP("DSIV",$J)) K @DSIRET
 S DSILIST=$NA(^TMP("DSIVDUZ",$J)) K @DSILIST
 ...
 S INPUT(3)="VAL^"_VAL
 S INPUT(4)="SCREEN^I $$ACT^DSIVDUZ(,+Y,.DSISCR,1)>0"
 D FIND^DSIVFM05(.DSILIST,.INPUT)
 S (DSI,DSICNT)=0
 ...
 .S DSIX=Z
 .S X=$$DIV^DSIVDUZ(,+DSIX,1,1) I X>0 S $P(DSIX,U,9)=$P(X,U,2)
 .S DSICNT=DSICNT+1,@DSIRET@(DSICNT,0)=DSIX
 .Q
 I '$D(@DSIRET) S @DSIRET@(1)=$$ERR(11)
LOUT S DSIV=DSIRET K @DSILIST
 Q
 ;
...
SCR() ;  dic screen for users
 ...
 .I X="PARM" D  Q
 ..S DSIX="USR~"_Y_"~"_Z,X=$$GET1^DSIVXPR(,DSIX,1)
 ..I +X=-1 S DSIRET=X
 ..Q
