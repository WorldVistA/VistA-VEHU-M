DSIVIC ;DSS/LM - Insurance card RPC's ;12/14/2004 [1/25/05 11:02am]
 ;;2.2;INSURANCE CAPTURE BUFFER;**4,11,12**;May 19, 2009;Build 13
...
GRPPOL(RESULT,X,FLDS,VDT)  ;RPC: DSIV GROUP INSURANCE PLAN
 ...
 ; FLDS=Field list, defaults to "@;2.01;.01I;.01"
 ; VDT=Visit Date.Time, defaults to DT
 ; 
 ; Returns matching values in local array
 ; 
 S RESULT(1)="-1^Group Plan name required" Q:'$L($G(X))
 S FLDS=$G(FLDS,"@;2.01;.01I;.01"),VDT=$G(VDT,$G(DT,$$DT^XLFDT))
 N DSIVICR,DSIVICM
 D FIND^DIC(355.3,,FLDS,"MQ",$$UC(X),,,,,"DSIVICR","DSIVICM")
 I '$D(DSIVICR) S RESULT(1)="-1"_U_$G(DSIVICM("DIERR",1,"TEXT",1)) Q
 S RESULT(1)="0^No match found"
 N CNT,I S CNT=1,I="" F  S I=$O(DSIVICR("DILIST",2,I)) Q:I=""  D
 .S RESULT(CNT)=DSIVICR("DILIST",2,I)
 .N J S J=""
 .F  S J=$O(DSIVICR("DILIST","ID",I,J)) Q:J=""  D
 ..I ($L(RESULT(CNT))+$L(DSIVICR("DILIST","ID",I,J)))<255 S RESULT(CNT)=RESULT(CNT)_U_DSIVICR("DILIST","ID",I,J) Q
 ..S CNT=CNT+1
 ..S RESULT(CNT)="$$APPEND$$"_U_DSIVICR("DILIST","ID",I,J)
 .S CNT=CNT+1
 Q
...
FIND(RESULT,DFN,FLDS,NUM) ;RPC: DSIV FIND ENTRIES
 ...
 I +$G(NUM) S PARM(7)="NUMBER"_U_NUM ;DSIV*2.2*12  2/9/2016 AJ
