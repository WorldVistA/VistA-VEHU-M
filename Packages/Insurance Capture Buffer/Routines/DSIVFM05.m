DSIVFM05 ;DSS/BPD - RPC FOR LIST^DIC ; 10/9/14 10:19am
 ...
 ;
 ;RETURN
 ;  Return DSIV=$NA(^TMP("DSIV",$J,"DILIST"))
 ;    ^TMP("DSIV",$J,"DILIST",n,0)=p1^p2^p3^... for n=1,2,3,...
 ;    where p1 = record ien
 ...
 ;          etc. for p4,p5,p6,...
 ;  If no matches found, @DSIV@(1,0) = -1^No matches found
 ;  If errors, one or more lines may be returned
 ;    @DSIV@(1,0) = -1^Problems encountered
 ;    @DSIV@(n,0) = detailed text of problem if available
 ;                  where n = 2,3,4,5,...
 ;
FIND(DSIV,INPUT) ; RPC: DSIV FM FIND
 N I,X,Y,Z,DIERR,DSIERR,ERR,FIELDS,FILE,FLAGS,IENS,INDEX,NUMBER
 N ROU,VAL,SCREEN
 K ^TMP("DSIV",$J) S DSIV=$NA(^TMP("DSIV",$J)) K @DSIV
 I '$D(INPUT) D ERR(1) Q
 ;
 ...
 S ROU=ROU_".SCREEN,,DSIV,""DSIERR"")"
 G DOIT
 ;
LIST(DSIV,INPUT) ; RPC: DSIV FM LIST
 N I,X,Y,Z,DIERR,DSIERR,ERR,FIELDS,FILE,FLAGS,FROM,IENS,INDEX,NUMBER
 N PART,ROU,SCREEN
 S DSIV=$NA(^TMP("DSIV",$J)) K @DSIV
 I '$D(INPUT) D ERR(1) Q
 ;
 ;  get fileman variables
 ...
 S:$G(INDEX)'="" ROU=ROU_"INDEX" S ROU=ROU_","
 S ROU=ROU_".SCREEN,,DSIV,""DSIERR"")"
DOIT X ROU S DSIV=$NA(^TMP("DSIV",$J,"DILIST"))
 I $D(DIERR) S X=$$MSG^DSIVFM01("VE",,,,"DSIERR") D ERR(X) Q
 K @DSIV@(0) I '$D(@DSIV) D ERR(7)
 Q
 ;
 ;   --------------------  subroutines  --------------------
ERR(X,Y) ;
 ...
 S Y=1+$O(^TMP("DSIV",$J,"DILIST","A"),-1)
 I Y=1 S ^TMP("DSIV",$J,"DILIST",1,0)="-1^Problems encountered" S Y=2
 S ^TMP("DSIV",$J,"DILIST",Y,0)=X
 S DSIV=$NA(^TMP("DSIV",$J,"DILIST"))
 Q
 ;
...
FLG(Z) ;  set FLAGS parameter
 ...
 S:FLAGS'="" FLAGS=$$CNVT^DSIVUTL(FLAGS,Z,"U")
