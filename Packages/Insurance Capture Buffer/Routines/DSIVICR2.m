DSIVICR2 ;DSS/KC - NIGHTLY EXCEPTION REPORT ;07/03/2012 2:21
 ;;2.2;INSURANCE CAPTURE BUFFER;**1,4,6,7,11**;May 19, 2009;Build 16
...
QUIT K ^XTMP("DSIVICR2;"_$J)
 Q
 ;
DQ N DSIVIC,I,DSIVI,DSIVICD,DSIVICP,DSIVAY,DSIVAYM,DSIVAYN,DSIVEXP,DSIVTOT,DSIVPCNT,X,CNT
 K DSIVD,^TMP("DILIST",$J),^TMP("DSIVICD",$J)
 S DSISCR="I $P(^(0),U,3)=""C""",CNT=0
 D LIST^DIC(44,,"@",,,,,,DSISCR) ;All Clinics
 S DSIVTOT=+^TMP("DILIST",$J,0)
 F I=1:1 Q:'$D(^TMP("DILIST",$J,2,I))  S $P(@DSIVICR@(1),U,8)=(((I/DSIVTOT)*.80)*100)\1 S DSICL=+$G(^TMP("DILIST",$J,2,I)) D:DSICL
