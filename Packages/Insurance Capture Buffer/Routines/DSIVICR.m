DSIVICR ;DSS/LM - Insurance card RPC's ;07/16/2012 11:24
 ;;2.2;INSURANCE CAPTURE BUFFER;**7,11**;May 19, 2009;Build 16
...
POLL(RESULT,DSIVHNDL,NUMS,MORE) ;RPC: DSIV POLLED DATA
 ...
 ;     or -1^Error message
 ;
 N CNT,I,R,X,DSIVICN,DSIVICEN,NEXT,QUIT,STDT,ETDT
 ...
 I '$P(X,U,2) S @RESULT@(1)="0^Results not ready^"_$P(X,U,8) Q
 ...
 F I=1:1 S DSIVICN=$Q(@DSIVICN) S NEXT=$Q(@DSIVICN) Q:'(DSIVICN[DSIVICEN)!QUIT  Q:(CNT=(NUMS-1))&($E(@NEXT,1)="$")  D
...
LOOP(SDT,IENST) ;loop to get data
 ...
 S IENST=0,SDTEMP=SDT
 F  S SDTEMP=$O(^DSI(19625,DSIVIX,SDTEMP)) Q:'SDTEMP!(SDTEMP>EDT)  S TOTAL=$G(TOTAL)+1
 S PCNT=0
 F  S SDT=$O(^DSI(19625,DSIVIX,SDT)) Q:'SDT!(SDT>EDT)!QUIT  D
 .S PCNT=$G(PCNT)+1,$P(@RET@(1),U,8)=((PCNT/TOTAL)*100)\1
