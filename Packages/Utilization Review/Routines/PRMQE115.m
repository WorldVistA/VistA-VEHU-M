...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQE115",DQ=1
1 S DW="3;4",DV="P3'",DU="",DLB="BY WHOM DXLS ENTERED",DIFLD=8
 S DU="DIC(3,"
 S X=$S(DUZ:DUZ,1:"")
 S Y=X
...
X4 I $D(^PRMQ(513.85,1,0)),$P(^(0),U,3)]"" S NRS=$P(^(0),U,3),NRS=$S(NRS=1:.5,NRS=2:.75,NRS=3:5,1:.45)
 Q
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 I NRS=5 S X1=PRMQADTT,X2=NRS D C^%DTC I $D(X) S PRMQNSRD=X
 Q
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X6 I NRS'=5,$D(DRGCAL) S PRMQALOS=$P(DRGCAL,U,8) K PRMQNSRD I $D(PRMQALOS),PRMQALOS S X1=PRMQADTT,X2=NRS*PRMQALOS+.5\1 D C^%DTC I $D(X) S PRMQNSRD=X
 Q
7 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=7 D X7 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X7 S PRMQY=Y S:$D(PRMQNSRD) Y=PRMQNSRD X:$D(PRMQNSRD) ^DD("DD") S:$D(PRMQNSRD) PRMQNSRD=$P(Y,"@",1) S Y=PRMQY
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S PRMQPMT=$S($D(^PRMQ(513.8,D0,1,D1,5)):$P(^PRMQ(513.8,D0,1,D1,5),"^",17),1:"EMPTY")
 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 S PRMQY=X S:PRMQPMT="" PRMQPMT="EMPTY",PRMQY=""
 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 I PRMQPMT'="EMPTY" S Y=PRMQPMT X ^DD("DD") S (PRMQPMT,PRMQY)=Y S Y="^"
 Q
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 S %=1 W !!,"NEXT SCHEDULED REVIEW DATE IS: "_PRMQPMT_".",!!,"DO YOU WANT TO CALCULATE A NEW DRG-BASED VALUE" D YN^DICN S PRMQYY=% I %'=1 S Y="@128"
 Q
12 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=12 D X12 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X12 S Y=$P(PRMQY,".") X ^DD("DD") S Y="^"
 Q
13 D:$D(DG)>9 F^DIE17 G ^PRMQE116
