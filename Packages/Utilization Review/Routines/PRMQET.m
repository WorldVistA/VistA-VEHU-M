PRMQET ; GENERATED FROM 'PRMQS1' INPUT TEMPLATE(#600), FILE 513.8;06/27/91
 D DE G BEGIN
DE S DIE="^PRMQ(513.8,",DIC=DIE,DP=513.8,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^PRMQ(513.8,DA,-1))<0
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQET",DQ=1
 S:$D(DTIME)[0 DTIME=999 S D0=DA,DIEZ=600,U="^"
...
X2 S DIE("NO^")="OUTOK"
 Q
3 S D=0 K DE(1) ;2
 S DIFLD=2,DGO="^PRMQET1",DC="69^513.801ID^1^",DV="513.801D",DW="0;1",DOW="ADMISSION OR TRANSFER DATE",DLB="Select "_DOW
 I $D(DSC(513.801))#2,$P(DSC(513.801),"I $D(^UTILITY(",1)="" X DSC(513.801) S D=$N(^(0)) G M3
 S D=$S($D(^PRMQ(513.8,DA,1,0)):$P(^(0),U,3,4),1:$N(^(0)))
M3 I D>0 S DC=DC_D I $D(^PRMQ(513.8,DA,1,+D,0)) S DE(3)=$P(^(0),U,1)
 S X=PRMQDAT
 S Y=X
