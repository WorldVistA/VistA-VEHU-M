...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="PRMQE19",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,D=0 K DE(1) ;4
 S DIFLD=4,DGO="^PRMQE110",DC="3^513.803PA^2^",DV="513.803MP80.1'",DW="0;1",DOW="OPERATION/PROCEDURE CODE",DLB="Select "_DOW
 S DU="ICD0("
 G RE:D I $D(DSC(513.803))#2,$P(DSC(513.803),"I $D(^UTILITY(",1)="" X DSC(513.803) S D=$N(^(0)) G M1
 S D=$S($D(^PRMQ(513.8,D0,1,DA,2,0)):$P(^(0),U,3,4),1:$N(^(0)))
M1 I D>0 S DC=DC_D I $D(^PRMQ(513.8,D0,1,DA,2,+D,0)) S DE(1)=$P(^(0),U,1)
 G RE
R1 D DE
 S D=1 G 1+1
 ;
