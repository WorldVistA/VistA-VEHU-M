ZIBCFP ;ALB/ARH - PRINT AUTHORIZED BILLS IN ORDER ; 6-DEC-94 [ 01/23/95  4:13 PM ]
...
BEG S DIR("A")="Begin printing bills",DIR("?")="Enter 'Y'es to begin printing of all authorized bills."
 ...
 W !!?10,"Turn on Capture -   <ALT>F1",!! H 60 D QTASK
 W !!?10,"Turn off Capture - <ALT>F1",!! H 120 K IBX,IBXX
 ;I $D(ZTSK) W !," ... queued"
 ;
END K DIR,IBY,IBZ,IBS,IBPAR1,IBFT,Y,X,DIRUT ; end of interactive part
 Q
 ;
...
SORT ;sort authorized bills by form type and requested sort order (notice that bill addendums only print for HCFA 1500's)
 ...
 K IBIFN,IBFT,IBY,IBPAT,IBZIP,IBINS,IBS1,IBS2,IBS3,IBS
 ;
QUEUE ; starts a queued job for each form type that an authorized bill was found for
 ...
 . D QBILL^ZIBCFP
 K IBY,IBFTP,IBJ ; end of first queued part
 Q
 ;
 ;
...
ROUT(IBFT,IBPNT,IBIFN) ; sub procedure so can protect variables with new
 ...
 S IBXX=IBX I $$FTN^IBCU3(+IBFT)="UB-92" D EN^IBCF3 W @IOF G RE
