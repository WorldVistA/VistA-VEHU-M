ZIBOSTUS ;ALB/SGD - MCCR BILL STATUS REPORT ;25 MAY 88 14:19
 ;;Version 2.0 ; INTEGRATED BILLING ;; 21-MAR-94
 ;
 ;MAP TO DGCROST
 ;
 ;***
 ;S XRTL=$ZU(0),XRTN="ZIBOSTUS-1" D T0^%ZOSV ;start rt clock
...
SORT ; chose the date type to sort on
 ...
 S DIR("A")="SORT BY",DIR("B")=1,DIR("?")="^D HELP2^ZIBOSTUS"
...
DATE1 S %DT="EPX" W !,"Go to "_IBDTP_" DATE: TODAY// " R X:DTIME S:X=" " X=IBBEG G Q:(X["^") S:X="" X="TODAY" D ^%DT G DATE1:Y<0 S IBEND=Y I IBEND<IBBEG W *7," ??",!,"ENDING DATE must follow BEGINNING DATE." G DATE1
 ...
 ;S DGPGM="QUEUED^ZIBOSTUS",DGVAR="IBDTP^IBBST^IBHD2^IBBEG^IBEND^DUZ" D ZIS^DGUTQ G Q:POP
 ;
 S %ZIS="QM" D ^%ZIS G:POP Q
 I $D(IO("Q")) K IO("Q") D  G Q
 .S ZTRTN="QUEUED^ZIBOSTUS",ZTDESC="IB - Bill Status Report",ZTSAVE("IB*")=""
 .D ^%ZTLOAD K ZTSK D HOME^%ZIS
 ;
 U IO
 ;***
 ;I $D(XRT0) S:'$D(XRTN) XRTN="ZIBOSTUS" D T1^%ZOSV ;stop rt clock
QUEUED ; entry point if queued
 ;***
 ;S XRTL=$ZU(0),XRTN="ZIBOSTUS-2" D T0^%ZOSV ;start rt clock
 ;
 ; K ^TMP($J)
...
STATS ; prints statistics
 S IBHDR3="REPORT STATISTICS" D HEAD^ZIBOSTS1
 S IBST1="RATE TYPE  : "
 S IBST2="BILL STATUS: "
 F I="IBST1","IBST2" N IBTOT D  W:'IBCRT !!!!
 .S IBCAT="" F  W ! S IBCAT=$O(@I@(IBCAT)) Q:IBCAT=""  D
 ..I IBCRT,($Y>(IOSL-2)) D HEAD^ZIBOSTS1
