ZTRTHT ;HVB/MTZ - TIME SERIES RTHIST OUTPUT (DSM-11) ;5/11/93  08:11
 ;;7.1;KERNEL;;May 11, 1993
 ;K (DT,DTIME,DUZ,U) ;***** EXCLUSIVE KILL *****
 W !,?7,"Time series histogram",!
 D KILL S DOMAIN="" I $D(^%ZOSF("MASTER")) S X=^("MASTER"),DOMAIN=@("^["""_$P(X,",")_""","""_$P(X,",",2)_"""]XMB(""NAME"")")
...
ZIS D ^%ZIS Q:POP'=0  U IO W #
 S P=0,N=FIRST-1 F Z=0:0 S N1=N,N=$O(^RTH(N)) S:$S<800 PF=1 Q:N=""!(N>LAST)!(PF=1)  D GET
 G PRINT
...
PRINT Q:'$D(CONF)  W !,"ZRTHT",?19,CONF,?57,DOMAIN,!!,"START DATE@TIME",?21,"RUN",?28,"%IDLE",?36,"%DISK",?44,"GLOREF",?52,"JOBS",!
 ...
 D LINE1,LINE0 W # X ^%ZIS("C")
KILL K %H,CNT,COM,CONF,DISK,DIV,DOMAIN,ET,FIRST,GLOREF,H,HITS,I,IDLE,INC,JOBS,LAST,M,N,N1,P,PF,POP,RTN,STAR,STARS,STIME,TAB,TIM,UCIP,X,Y,Z Q
 Q
