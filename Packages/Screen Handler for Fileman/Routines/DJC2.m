DJC2 ;FDJW,JA/WASH;FILL V() AFTER SELECTION ; 30 Jun 86  6:42 PM
 ;VERSION 1.36
 S DJ14=-1,DJ9=V,DJ8=DJN
 S:'$D(DJST) DJST=1 I DJST=1 S ^UTILITY($J,"DJST",1,"DA")=W(DJ9),^UTILITY($J,"DJST",1,"SC")=DJN,^UTILITY($J,"DJST",1,"LOC")="",^UTILITY($J,"DJST",1,"DIC")=DIC,^UTILITY($J,"DJST",1,"DD")=DJDD,^UTILITY($J,"DJST",1,"GN")=""
 F V=DJF-.01:0 S V=$N(DJJ(V)) Q:V<0!(V>DJL)  S DJ16=$P(DJJ(V),U,4) D:DJ16["M" M D:DJ16'["M" S
...
SQ3 I DJ16["C" X $P(^DD(DJDD,DJ7,0),U,5,99) S V(V)=X G:DJ16["D" SQ1
 ...
 ;I DJ16["W" S N=0 S A=DIC_DA_","_+DJ7_",0)" S N=$N(@A),V(V)=^(N,0)
E X XY I V(V)'="" D O S V(V)=$E(V(V),1,+DJJ(V)) W @DJHIN X XY W V(V),@DJLIN S $P(DJDB," ",DJJ(V)-$L(V(V)))=" " W:$D(DJDB) DJDB K DJDB
 I DJ7=.01&(DJ16'["M") S ^UTILITY($J,"DJST",DJST,"TITLE")=$P(^DD(DJDD,.01,0),"^",1)_":"_V(V),^UTILITY($J,"DJST",DJST-1,"KEY")=V(V)
 Q
P ;
...
COMP F DJK=0:1:DJST-2 S @("D"_DJK)=^UTILITY($J,"DJST",DJK+1,"DA")
